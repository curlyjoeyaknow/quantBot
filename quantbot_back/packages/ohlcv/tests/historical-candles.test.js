...
describe('fetchHistoricalCandlesForMonitoring', () => {
    ...
    it('should fetch historical candles for 18 days from ClickHouse', async () => {
        ...
        expect(mockGetCandles).toHaveBeenCalledWith(FULL_MINT, // Full address, case-preserved
        'solana', expect.any(DateTime), // startTime (18 days ago)
    ...
    it('should preserve exact case of mint address', async () => {
        ...
        expect(mockGetCandles).toHaveBeenCalledWith(FULL_MINT_LOWERCASE, // Exact case preserved
        'solana', expect.any(DateTime), expect.any(DateTime), { interval: '5m' });
    });
    it('should limit results to 5000 candles', async () => {
        const mockCandles = Array.from({ length: 10000 }, (_, i) => ({
    ...
    it('should use custom chain when provided', async () => {
        ...
        expect(mockGetCandles).toHaveBeenCalledWith(FULL_MINT, 'ethereum', expect.any(DateTime), expect.any(DateTime), { interval: '5m' });
    });
});
//# sourceMappingURL=historical-candles.test.js.map