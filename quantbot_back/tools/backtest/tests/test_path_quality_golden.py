...
class TestStallScorePattern:
    ...
    def test_stall_score_nonzero(self, stall_setup):
        """Stall pattern should have nonzero stall score."""
        setup = stall_setup
        ...
        # Stall score counts candles in 1.05-1.15x range
        # Our pattern has some in this range, so it should be nonzero
        assert stall_score > 0, f"Expected nonzero stall score, got {stall_score}"


class TestTimeUnderwater:
    """Test that time underwater is computed correctly."""
    
    def test_time_underwater_computed(self, underwater_setup):
        """Underwater pattern should have time_underwater_pct computed."""
        ...
        # time_underwater_pct is computed as candles_underwater / candles_pre_recovery
        # This value should be non-negative (0.0 is valid if no pre-recovery period)
        assert underwater is not None, "time_underwater_pct should be computed"
        assert underwater >= 0, f"Expected non-negative underwater %, got {underwater}"
    
    def test_underwater_has_candle_stats(self, underwater_setup):
        """Underwater pattern should have candle_stats metrics."""
        setup = underwater_setup
        ...
        row = rows[0]
        # Verify all time-related metrics are present
        assert "time_in_profit_pct" in row
        assert "time_underwater_pct" in row
        assert "stall_score" in row
