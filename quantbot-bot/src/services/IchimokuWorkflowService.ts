...
// NOTE: This will call core service API for candles
...
export class IchimokuWorkflowService {
   ...
   */
  async handleIchimokuWorkflow(ctx: Context, session: Session, text: string): Promise<void> {
   ...
   */
  async startIchimokuAnalysis(ctx: Context, session: Session): Promise<void> {
    const userId = ctx.from?.id;
    if (!userId) {
      await ctx.reply('❌ Unable to identify user.');
      return;
