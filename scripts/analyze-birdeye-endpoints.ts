...
async function main() {
  ...
  try {
    ...
    if (endpointResults.rows.length === 0) {
      ...
      console.log(`  ${'Endpoint'.padEnd(50)} ${'Calls'.padEnd(10)} ${'Success'.padEnd(10)} ${'Failed'.padEnd(10)} ${'Avg Latency'.padEnd(15)} ${'Credits'}`);
      ...
      console.log('  ' + '-'.repeat(110));
      console.log(`  ${'TOTAL'.padEnd(50)} ${totalCalls.toString().padEnd(10)} ${''.padEnd(10)} ${''.padEnd(10)} ${''.padEnd(15)} ${totalCredits.toFixed(0)}`);
    }
