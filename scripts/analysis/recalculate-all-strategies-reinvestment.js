...
const optimize_strategies_1 = require("./optimize-strategies");
...
async function recalculateAllStrategies() {
    ...
    for (let i = 0; i < topStrategiesJson.length; i++) {
        ...
        const strategyParams = strategyData.params;
        ...
        if (strategyData.trades && strategyData.trades.length > 0) {
            for (const trade of strategyData.trades) {
                trades.push({
                    ...
                    timestamp: luxon_1.DateTime.fromISO(trade.alertTime || ''),
                });
            }
        }
        else {
            ...
            for (const call of uniqueCalls.slice(0, 238)) {
                try {
                    ...
                    if (candles.length < 10)
                        continue;
                    const result = (0, optimize_strategies_1.simulateStrategyWithParams)(candles, strategyParams);
                    trades.push({
                        tokenAddress,
    ...
    for (let i = 0; i < results.length; i++) {
        const r = results[i];
        console.log(`${(i + 1).toString().padStart(4)} | ` +
...
//# sourceMappingURL=recalculate-all-strategies-reinvestment.js.map