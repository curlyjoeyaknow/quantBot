...
exports.getLatestResultsFile = getLatestResultsFile;
...
function getLatestResultsFile() {
    ...
    if (!fs.existsSync(exportDir)) {
        logger_1.logger.error('Export directory does not exist', { exportDir });
        return null;
    }
    ...
    if (files.length === 0) {
        logger_1.logger.error('No scored results files found');
        return null;
    }
    return files[0].path;
}
...
function loadResults(filePath) {
    try {
        ...
        if (!Array.isArray(results)) {
            logger_1.logger.error('Results file is not an array');
            return [];
        }
        return results.filter((r) => r !== null && r.score !== null && r.score !== undefined);
    }
    catch (error) {
        logger_1.logger.error('Error loading results', { error: error.message });
        return [];
    }
}
/**
...
function formatScore(score) {
    if (score >= 80)
        return `\x1b[32m${score.toFixed(2)}\x1b[0m`; // Green
    if (score >= 60)
        return `\x1b[33m${score.toFixed(2)}\x1b[0m`; // Yellow
    return `\x1b[31m${score.toFixed(2)}\x1b[0m`; // Red
}
/**
 * Display results in table format
 */
function displayTable(results, limit = 50) {
...
function filterResults(results, options) {
    ...
    if (options.caller) {
        filtered = filtered.filter(r => r.callerName?.toLowerCase().includes(options.caller.toLowerCase()));
    }
    return filtered;
}
/**
...
async function main() {
    ...
    logger_1.logger.info('Displaying results', {
    ...
    if (args.includes('--export')) {
        const exportPath = path.join(process.cwd(), 'data', 'exports', 'brook-analysis', `filtered-results-${Date.now()}.json`);
...
if (require.main === module) {
    main().catch(error => {
        logger_1.logger.error('Error viewing results', error);
        process.exit(1);
    });
}
//# sourceMappingURL=view-scored-results.js.map