...
function simulateCloudCrossStrategy(
  ...
  for (let i = 52; i < candles.length; i++) {
    ...
    if (previousIndicators?.ichimoku && indicators.ichimoku) {
      ...
      const wasBelowOrInCloud =
        previousIndicators.ichimoku.isBearish || previousIndicators.ichimoku.inCloud;
  ...
  for (let i = entryIndex; i < candles.length; i++) {
    ...
    if (previousIndicators?.ichimoku && indicators.ichimoku) {
      const wasAboveOrInCloud =
        previousIndicators.ichimoku.isBullish || previousIndicators.ichimoku.inCloud;
...
async function compareStrategies() {
  ...
  const tkResult: StrategyResult = {
    ...
    winRate:
      tenkanKijunTrades.length > 0
        ? (tenkanKijunTrades.filter((t) => t.pnl > 1.0).length / tenkanKijunTrades.length) * 100
        : 0,
    avgPnlPerTrade:
      tenkanKijunTrades.length > 0
        ? (tenkanKijunTrades.reduce((sum, t) => sum + (t.pnl - 1.0), 0) /
            tenkanKijunTrades.length) *
          100
        : 0,
    finalPortfolio: 0,
    compoundFactor: 0,
    first6Hours: {
      ...
      winRate:
        tkFirst6h.length > 0
          ? (tkFirst6h.filter((t) => t.pnl > 1.0).length / tkFirst6h.length) * 100
          : 0,
      avgPnl:
        tkFirst6h.length > 0
          ? (tkFirst6h.reduce((sum, t) => sum + (t.pnl - 1.0), 0) / tkFirst6h.length) * 100
          : 0,
    ...
    remainingPeriod: {
      ...
      winRate:
        tkRemaining.length > 0
          ? (tkRemaining.filter((t) => t.pnl > 1.0).length / tkRemaining.length) * 100
          : 0,
      avgPnl:
        tkRemaining.length > 0
          ? (tkRemaining.reduce((sum, t) => sum + (t.pnl - 1.0), 0) / tkRemaining.length) * 100
          : 0,
  ...
  const cloudResult: StrategyResult = {
    ...
    winRate:
      cloudCrossTrades.length > 0
        ? (cloudCrossTrades.filter((t) => t.pnl > 1.0).length / cloudCrossTrades.length) * 100
        : 0,
    avgPnlPerTrade:
      cloudCrossTrades.length > 0
        ? (cloudCrossTrades.reduce((sum, t) => sum + (t.pnl - 1.0), 0) / cloudCrossTrades.length) *
          100
        : 0,
    finalPortfolio: 0,
    compoundFactor: 0,
    first6Hours: {
      ...
      winRate:
        cloudFirst6h.length > 0
          ? (cloudFirst6h.filter((t) => t.pnl > 1.0).length / cloudFirst6h.length) * 100
          : 0,
      avgPnl:
        cloudFirst6h.length > 0
          ? (cloudFirst6h.reduce((sum, t) => sum + (t.pnl - 1.0), 0) / cloudFirst6h.length) * 100
          : 0,
    ...
    remainingPeriod: {
      ...
      winRate:
        cloudRemaining.length > 0
          ? (cloudRemaining.filter((t) => t.pnl > 1.0).length / cloudRemaining.length) * 100
          : 0,
      avgPnl:
        cloudRemaining.length > 0
          ? (cloudRemaining.reduce((sum, t) => sum + (t.pnl - 1.0), 0) / cloudRemaining.length) *
            100
          : 0,
  ...
  console.log(
    `  First 6 Hours:  ${tkResult.first6Hours.trades} trades | ${tkResult.first6Hours.winRate.toFixed(1)}% win rate | ${tkResult.first6Hours.avgPnl >= 0 ? '+' : ''}${tkResult.first6Hours.avgPnl.toFixed(2)}% avg PnL`
  );
  console.log(
    `  Remaining:     ${tkResult.remainingPeriod.trades} trades | ${tkResult.remainingPeriod.winRate.toFixed(1)}% win rate | ${tkResult.remainingPeriod.avgPnl >= 0 ? '+' : ''}${tkResult.remainingPeriod.avgPnl.toFixed(2)}% avg PnL`
  );

  console.log('\nCloud Cross:');
  console.log(
    `  First 6 Hours:  ${cloudResult.first6Hours.trades} trades | ${cloudResult.first6Hours.winRate.toFixed(1)}% win rate | ${cloudResult.first6Hours.avgPnl >= 0 ? '+' : ''}${cloudResult.first6Hours.avgPnl.toFixed(2)}% avg PnL`
  );
  console.log(
    `  Remaining:     ${cloudResult.remainingPeriod.trades} trades | ${cloudResult.remainingPeriod.winRate.toFixed(1)}% win rate | ${cloudResult.remainingPeriod.avgPnl >= 0 ? '+' : ''}${cloudResult.remainingPeriod.avgPnl.toFixed(2)}% avg PnL`
  );
