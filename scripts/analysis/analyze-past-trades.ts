  ...
  for (const file of tradeFiles) {
    ...
    for (const record of records) {
      ...
      allTrades.push({
        ...
        portfolioBefore: parseFloat(
          record['Portfolio_Before_SOL'] || record['portfolioBefore'] || '0'
        ),
        portfolioAfter: parseFloat(
          record['Portfolio_After_SOL'] || record['portfolioAfter'] || '0'
        ),
        maxMultiplierReached: parseFloat(
          record['Max_Multiplier_Reached'] || record['maxMultiplierReached'] || '0'
        ),
        holdDurationMinutes: parseFloat(
          record['HoldDuration_Minutes'] || record['holdDurationMinutes'] || '0'
        ),
        timeToAthMinutes: parseFloat(
          record['TimeToAth_Minutes'] || record['timeToAthMinutes'] || '0'
        ),
        timestamp,
      });
    }
...
function analyzeTrades(trades: Trade[]) {
  ...
  const avgPnL =
    totalTrades > 0 ? trades.reduce((sum, t) => sum + t.pnlMultiplier, 0) / totalTrades : 0;
  ...
  const bestTrade = trades.reduce(
    (best, t) => (t.pnlMultiplier > best.pnlMultiplier ? t : best),
    trades[0] || { pnlMultiplier: 0 }
  );

  const worstTrade = trades.reduce(
    (worst, t) => (t.pnlMultiplier < worst.pnlMultiplier ? t : worst),
    trades[0] || { pnlMultiplier: Infinity }
  );

  const avgHoldDuration =
    totalTrades > 0 ? trades.reduce((sum, t) => sum + t.holdDurationMinutes, 0) / totalTrades : 0;

  const avgTimeToAth =
    totalTrades > 0 ? trades.reduce((sum, t) => sum + t.timeToAthMinutes, 0) / totalTrades : 0;
  ...
  const portfolioGrowth =
    initialPortfolio > 0 ? ((finalPortfolio - initialPortfolio) / initialPortfolio) * 100 : 0;
...
function printAnalysis(stats: ReturnType<typeof analyzeTrades>, trades: Trade[]) {
  ...
  console.log(
    `Winning Trades:      ${stats.winningTrades} (${((stats.winningTrades / stats.totalTrades) * 100).toFixed(1)}%)`
  );
  console.log(
    `Losing Trades:       ${stats.losingTrades} (${((stats.losingTrades / stats.totalTrades) * 100).toFixed(1)}%)`
  );
  ...
  console.log(
    `Net P/L:             ${stats.totalProfit >= 0 ? '+' : ''}${stats.totalProfit.toFixed(4)} SOL`
  );
  ...
  console.log(
    `Portfolio Growth:   ${stats.portfolioGrowth >= 0 ? '+' : ''}${stats.portfolioGrowth.toFixed(2)}%`
  );
  console.log('');
  ...
  if (stats.bestTrade.pnlMultiplier > 0) {
    ...
    console.log(
      `  PnL: ${stats.bestTrade.pnlMultiplier.toFixed(4)}x (${((stats.bestTrade.pnlMultiplier - 1) * 100).toFixed(2)}%)`
    );
  ...
  if (stats.worstTrade.pnlMultiplier < Infinity) {
    ...
    console.log(
      `  PnL: ${stats.worstTrade.pnlMultiplier.toFixed(4)}x (${((stats.worstTrade.pnlMultiplier - 1) * 100).toFixed(2)}%)`
    );
  ...
  console.log('─'.repeat(80));
  console.log(
    `Avg Hold Duration:   ${(stats.avgHoldDuration / 60).toFixed(2)} hours (${stats.avgHoldDuration.toFixed(0)} minutes)`
  );
  console.log(
    `Avg Time to ATH:     ${(stats.avgTimeToAth / 60).toFixed(2)} hours (${stats.avgTimeToAth.toFixed(0)} minutes)`
  );
  console.log('');

  console.log('🔗 CHAIN DISTRIBUTION');
  console.log('─'.repeat(80));
  const sortedChains = Array.from(stats.chainCounts.entries()).sort((a, b) => b[1] - a[1]);
  sortedChains.forEach(([chain, count]) => {
    const percentage = (count / stats.totalTrades) * 100;
    console.log(
      `  ${chain.padEnd(10)} ${count.toString().padStart(5)} trades (${percentage.toFixed(1)}%)`
    );
  });
  console.log('');
  ...
  Object.entries(stats.pnlRanges).forEach(([range, count]) => {
    ...
    console.log(
      `  ${range.padEnd(10)} ${count.toString().padStart(5)} trades (${percentage.toFixed(1)}%) ${bar}`
    );
  });
  console.log('');
  ...
  if (stats.monthlyBreakdown && stats.monthlyBreakdown.length > 0) {
    ...
    console.log('─'.repeat(80));
    console.log(
      `${'Month'.padEnd(20)} ${'Trades'.padStart(8)} ${'Investment'.padStart(12)} ${'Return'.padStart(12)} ${'Profit'.padStart(12)} ${'ROI'.padStart(10)} ${'Win Rate'.padStart(10)} ${'Portfolio Growth'.padStart(18)}`
    );
    console.log('─'.repeat(80));
    ...
    stats.monthlyBreakdown.forEach((month) => {
      ...
      const profitStr = month.profit >= 0 ? `+${month.profit.toFixed(4)}` : month.profit.toFixed(4);
      const roiStr = month.roi >= 0 ? `+${month.roi.toFixed(2)}%` : `${month.roi.toFixed(2)}%`;
      const winRateStr = `${month.winRate.toFixed(1)}%`;
      const portfolioGrowthStr =
        month.portfolioGrowth >= 0
    ...
    console.log('─'.repeat(80));
    const cumulativeROI =
      cumulativeInvestment > 0 ? (cumulativeProfit / cumulativeInvestment) * 100 : 0;
    const cumulativeROIStr =
      cumulativeROI >= 0 ? `+${cumulativeROI.toFixed(2)}%` : `${cumulativeROI.toFixed(2)}%`;
