...
const helius_client_1 = require("../../src/api/helius-client");
const candles_1 = require("../../src/simulation/candles");
async function fetchAllTransactionsForAddress(address) {
    const all = [];
    let before;
    ...
    const pageSize = 100;
    for (;;) {
        const batch = await helius_client_1.heliusRestClient.getTransactionsForAddress(address, {
            limit: pageSize,
            before,
        });
        if (!batch.length)
...
function deriveTicksFromTx(tx, mint) {
    ...
    for (const [account, deltaTokens] of tokenDelta.entries()) {
        ...
        if (!deltaTokens || !deltaLamports)
            continue;
        // User buys: deltaTokens > 0, deltaLamports < 0
        // User sells: deltaTokens < 0, deltaLamports > 0
        if (deltaTokens > 0 && deltaLamports >= 0)
            continue;
        if (deltaTokens < 0 && deltaLamports <= 0)
            continue;
        const tokens = Math.abs(deltaTokens);
        const sol = Math.abs(deltaLamports) / 1000000000; // lamports -> SOL
        if (!tokens || !sol)
            continue;
...
function buildOneMinuteCandles(ticks) {
    if (!ticks.length)
        return [];
...
async function main() {
    ...
    const candles5m = (0, candles_1.aggregateCandles)(candles1m, '5m');
    // Summaries
...
//# sourceMappingURL=build-pumpfun-ohlcv-from-helius.js.map