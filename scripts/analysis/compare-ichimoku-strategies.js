 ...
 */
function simulateTenkanKijunStrategy(candles, alertTime) {
    ...
    const entryPeriod = entryTime < sixHourMark ? 'first6h' : 'remaining';
    let remaining = 1.0;
    let pnl = 0;
 ...
 */
function simulateCloudCrossStrategy(candles, alertTime) {
    if (candles.length < 52) {
        return null;
    }
    // Calculate indicators
 ...
 */
function calculateReinvestmentPerformance(trades, initialPortfolio = 100, stopLossPercent, maxRiskPerTrade = 0.02) {
    ...
    for (const trade of sortedTrades) {
        const tradeDate = luxon_1.DateTime.fromISO(trade.alertTime);
    ...
    const sortedWeeks = Array.from(tradesByWeek.entries()).sort((a, b) => luxon_1.DateTime.fromISO(a[0]).toMillis() - luxon_1.DateTime.fromISO(b[0]).toMillis());
...
async function compareStrategies() {
    ...
    const tenkanKijunTrades = [];
    const cloudCrossTrades = [];
        ...
        try {
            ...
            if (tkResult) {
                tenkanKijunTrades.push({
                    ...
                    entryTime: luxon_1.DateTime.fromMillis(tkResult.entryTime).toISO() || '',
                    exitTime: luxon_1.DateTime.fromMillis(tkResult.exitTime).toISO() || '',
                    pnl: tkResult.pnl,
            ...
            if (cloudResult) {
                cloudCrossTrades.push({
                    tokenAddress,
                    alertTime: call.timestamp || call.alertTime || '',
                    entryTime: luxon_1.DateTime.fromMillis(cloudResult.entryTime).toISO() || '',
                    exitTime: luxon_1.DateTime.fromMillis(cloudResult.exitTime).toISO() || '',
                    pnl: cloudResult.pnl,
    ...
    console.log('-'.repeat(80));
    console.log(`${tkResult.strategy.padEnd(20)} | ` +
    ...
    console.log(`${cloudResult.strategy.padEnd(20)} | ` +
    ...
    await new Promise((resolve, reject) => {
        (0, csv_stringify_1.stringify)(csvRows, { header: true }, (err, output) => {
            if (err)
                reject(err);
            else {
...
//# sourceMappingURL=compare-ichimoku-strategies.js.map