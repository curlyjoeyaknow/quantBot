...
const OUTPUT_JSON_PATH = path.join(__dirname, 'real_ohlcv_simulation_results.json');
const OUTPUT_CSV_PATH = path.join(__dirname, 'real_ohlcv_simulation_results.csv');
...
function simulateTradeWithRealOHLCV(call, strategyRules) {
    ...
    console.log(`\n🔄 Processing ${tokenAddress} at ${alertTimestamp}`);
    ...
    let tradeResult = {
        ...
        alertTimestamp: alertTimestamp.toString(),
        ...
        exitPrice: 0,
        exitTimestamp: null,
        candleData: candles.slice(0, 10) // Store first 10 candles for verification
    };
...
async function runRealOHLCVSimulation() {
    console.log('🚀 Running REAL OHLCV simulation...');
    ...
    for (let i = 0; i < validRecords.length; i++) {
        ...
        const tradeResult = await simulateTradeWithRealOHLCV(call, strategyRules);
    ...
    console.log('\n🎉 === REAL OHLCV SIMULATION COMPLETE ===');
...
// Run the simulation
runRealOHLCVSimulation().catch(console.error);
