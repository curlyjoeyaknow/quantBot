...
const RISK_MANAGEMENT_RULES = {
    ...
    maxTradesPerCaller: 50, // Limit total trades per caller
    maxWeeksToSimulate: 16, // Only simulate 16 weeks (4 months)
    minPositionSize: 10, // Minimum $10 per trade
    maxPositionSize: 500, // Maximum $500 per trade (cap growth)
};
...
async function simulateRealisticProfitTargetVariations() {
    const inputFile = '/home/memez/quantBot/final_complete_filtered_ca_drops.csv';
    const allResults = {};
    
    console.log('Testing REALISTIC PROFIT TARGET VARIATIONS (July 30 - November, 2025)...');
    ...
    console.log(`Time Period: July 30 - November, 2025 (~4 months)`);
    console.log(`Max Trades Per Caller: ${RISK_MANAGEMENT_RULES.maxTradesPerCaller}`);
    console.log(`Max Weeks: ${RISK_MANAGEMENT_RULES.maxWeeksToSimulate}`);
    console.log(`Position Size Cap: $${RISK_MANAGEMENT_RULES.maxPositionSize}`);
    
    // Test each profit target strategy
    for (const [strategyKey, strategy] of Object.entries(PROFIT_TARGET_STRATEGIES)) {
        console.log(`\n=== Testing ${strategy.name} ===`);
        
        const results = await simulateStrategyRealistic(inputFile, strategy);
        ...
        topPerformers.forEach((performer, index) => {
            console.log(`${index + 1}. ${performer.caller}: ${formatPercentage(performer.totalReturn)} return, Final Value: ${formatLargeNumber(performer.finalPortfolioValue)}`);
        });
        ...
        console.log(`Average Return: ${formatPercentage(totalReturn)}`);
        console.log(`Combined Final Portfolio Value: ${formatLargeNumber(finalPortfolioValue)}`);
    }
    
    // Create comparison report
    createRealisticComparisonReport(allResults);
    
    return allResults;
}

async function simulateStrategyRealistic(inputFile, strategy) {
    return new Promise((resolve, reject) => {
        ...
        fs.createReadStream(inputFile)
            ...
            .on('end', () => {
                ...
                Object.entries(callerData).forEach(([caller, calls]) => {
                    const simulation = simulateCallerRealistic(caller, calls, strategy);
...
function simulateCallerRealistic(caller, calls, strategy) {
    ...
    // Limit to max weeks
    const limitedWeeklyCalls = weeklyCalls.slice(0, RISK_MANAGEMENT_RULES.maxWeeksToSimulate);
    
    console.log(`  ${caller}: Processing ${limitedWeeklyCalls.length} weeks of data (limited from ${weeklyCalls.length})`);
    
    // Process each week
    limitedWeeklyCalls.forEach((weekCalls, weekIndex) => {
        ...
        // Process up to 5 calls per week (more realistic)
        const callsToProcess = weekCalls.slice(0, 5);
        
        callsToProcess.forEach((call, callIndex) => {
            // Stop if we've reached max trades per caller
            if (trades.length >= RISK_MANAGEMENT_RULES.maxTradesPerCaller) {
                return;
            }
            
            const trade = simulateTradeRealistic(call, callIndex, strategy, portfolioValue);
            if (trade) {
                trades.push(trade);
        ...
        console.log(`    End of Week ${weekIndex + 1}: Portfolio Value: ${formatLargeNumber(portfolioValue)}`);
    });
...
function simulateTradeRealistic(call, tradeIndex, strategy, currentPortfolioValue) {
    // Skip if no address or invalid data
    if (!call['Address'] || call['Address'] === 'N/A') {
        return null;
    }
    
    // Calculate position size as 2.5% of current portfolio value, but cap it
    const uncappedPositionSize = currentPortfolioValue * RISK_MANAGEMENT_RULES.positionSizePercent;
    const positionSize = Math.max(
        Math.min(uncappedPositionSize, RISK_MANAGEMENT_RULES.maxPositionSize),
    ...
    let trade = attemptInitialEntryRealistic(call, actualEntryPrice, positionSize, holdTime, tradeIndex, false, strategy);
    ...
    if (trade && trade.exitReason === 'stop_loss') {
        ...
        trade = attemptReentryRealistic(call, actualReentryPrice, positionSize, holdTime, tradeIndex, true, strategy);
    }
    
    return trade;
}

function attemptInitialEntryRealistic(call, entryPrice, positionSize, holdTime, tradeIndex, isReentry, strategy) {
...
function attemptReentryRealistic(call, reentryPrice, positionSize, holdTime, tradeIndex, isReentry, strategy) {
    return attemptInitialEntryRealistic(call, reentryPrice, positionSize, holdTime, tradeIndex, true, strategy);
}

function createRealisticComparisonReport(allResults) {
    console.log('\n=== REALISTIC PROFIT TARGET STRATEGY COMPARISON ===');
    console.log('Time Period: July 30 - November, 2025 (~4 months)');
    console.log('REALISTIC CONSTRAINTS: Max 50 trades per caller, Max 16 weeks, Position cap $500');
    ...
    // Create data table with realistic figures
    console.log('\n📊 REALISTIC PROFIT TARGET STRATEGY COMPARISON TABLE');
    console.log('='.repeat(140));
    console.log('| Strategy'.padEnd(30) + '| Avg Return'.padEnd(20) + '| Total Final Value'.padEnd(25) + '| Total Trades'.padEnd(15) + '| Re-entry Rate'.padEnd(15) + '| Best Performer'.padEnd(25) + '|');
    console.log('='.repeat(140));
    
    comparison.forEach((strategy, index) => {
        const strategyName = strategy.strategy.length > 29 ? strategy.strategy.substring(0, 26) + '...' : strategy.strategy;
        const avgReturn = formatPercentage(strategy.avgReturn);
        const totalValue = formatLargeNumber(strategy.totalFinalValue);
        const totalTrades = strategy.totalTrades.toString();
        const reentryRate = strategy.reentryRate + '%';
        const bestPerformer = strategy.bestPerformer.length > 24 ? strategy.bestPerformer.substring(0, 21) + '...' : strategy.bestPerformer;
        
        console.log('| ' + strategyName.padEnd(28) + '| ' + avgReturn.padEnd(18) + '| ' + totalValue.padEnd(23) + '| ' + totalTrades.padEnd(13) + '| ' + reentryRate.padEnd(13) + '| ' + bestPerformer.padEnd(23) + '|');
    });
    ...
    console.log('| Strategy'.padEnd(30) + '| 2x Trades'.padEnd(12) + '| 3x Trades'.padEnd(12) + '| 5x Trades'.padEnd(12) + '| 10x Trades'.padEnd(12) + '|');
    console.log('='.repeat(90));
    
    comparison.forEach((strategy) => {
        const strategyName = strategy.strategy.length > 29 ? strategy.strategy.substring(0, 26) + '...' : strategy.strategy;
        console.log('| ' + strategyName.padEnd(28) + '| ' + strategy.tradesAt2x.toString().padEnd(10) + '| ' + strategy.tradesAt3x.toString().padEnd(10) + '| ' + strategy.tradesAt5x.toString().padEnd(10) + '| ' + strategy.tradesAt10x.toString().padEnd(10) + '|');
    });
    
    console.log('='.repeat(90));
    
    // Save detailed comparison
    fs.writeFileSync('/home/memez/quantBot/realistic_profit_target_comparison.json', JSON.stringify({
        strategies: comparison,
        riskManagement: RISK_MANAGEMENT_RULES,
        timePeriod: "July 30 - November, 2025 (~4 months)",
        constraints: "Max 50 trades per caller, Max 16 weeks, Position cap $500",
        summary: {
    ...
    console.log('\nRealistic comparison saved to realistic_profit_target_comparison.json');
    
    // Create CSV for easy analysis
    const csvWriter = createCsvWriter({
        path: '/home/memez/quantBot/realistic_profit_target_comparison.csv',
        header: [
    ...
    csvWriter.writeRecords(comparison)
        .then(() => {
            console.log('Realistic comparison CSV saved to realistic_profit_target_comparison.csv');
        })
        .catch(console.error);
}

// Run the realistic profit target variations simulation
simulateRealisticProfitTargetVariations()
    .then((allResults) => {
        console.log('\n=== REALISTIC PROFIT TARGET VARIATIONS COMPLETE ===');
        console.log(`Tested ${Object.keys(allResults).length} different profit target strategies with REALISTIC CONSTRAINTS`);
        console.log('Time Period: July 30 - November, 2025 (~4 months)');
        console.log('Position Size: 2.5% of portfolio value per trade (capped at $500)');
        console.log('Constraints: Max 50 trades per caller, Max 16 weeks');
        ...
        console.log(`Best Total Final Value: ${formatLargeNumber(bestStrategy.totalFinalValue)}`);
    })
    .catch(console.error);