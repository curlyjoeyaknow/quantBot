...
async function simulateCleanedProfitTargetVariations() {
    const inputFile = '/home/memez/quantBot/cleaned_ca_drops.csv';
    const allResults = {};
    
    console.log('Testing PROFIT TARGET VARIATIONS on CLEANED DATA (Aug 23 - Sep 26, 2025)...');
    ...
    console.log(`Time Period: August 23 - September 26, 2025 (35 days)`);
    
    // Test each profit target strategy
    for (const [strategyKey, strategy] of Object.entries(PROFIT_TARGET_STRATEGIES)) {
        console.log(`\n=== Testing ${strategy.name} ===`);
        
        const results = await simulateStrategyCleaned(inputFile, strategy);
    ...
    createCleanedProfitTargetComparisonReport(allResults);
    
    return allResults;
}

async function simulateStrategyCleaned(inputFile, strategy) {
    return new Promise((resolve, reject) => {
        ...
        fs.createReadStream(inputFile)
            ...
            .on('end', () => {
                ...
                Object.entries(callerData).forEach(([caller, calls]) => {
                    const simulation = simulateCallerWithStrategyCleaned(caller, calls, strategy);
...
function simulateCallerWithStrategyCleaned(caller, calls, strategy) {
    ...
    // Process up to 50 calls per caller
    const callsToProcess = calls.slice(0, 50);
    
    callsToProcess.forEach((call, index) => {
        const trade = simulateTradeWithStrategyCleaned(call, index, strategy);
        if (trade) {
            trades.push(trade);
...
function simulateTradeWithStrategyCleaned(call, tradeIndex, strategy) {
    ...
    let trade = attemptInitialEntryWithStrategyCleaned(call, actualEntryPrice, positionSize, holdTime, tradeIndex, false, strategy);
    ...
    if (trade && trade.exitReason === 'stop_loss') {
        ...
        trade = attemptReentryWithStrategyCleaned(call, actualReentryPrice, positionSize, holdTime, tradeIndex, true, strategy);
    }
    
    return trade;
}

function attemptInitialEntryWithStrategyCleaned(call, entryPrice, positionSize, holdTime, tradeIndex, isReentry, strategy) {
...
function attemptReentryWithStrategyCleaned(call, reentryPrice, positionSize, holdTime, tradeIndex, isReentry, strategy) {
    return attemptInitialEntryWithStrategyCleaned(call, reentryPrice, positionSize, holdTime, tradeIndex, true, strategy);
}

function createCleanedProfitTargetComparisonReport(allResults) {
    console.log('\n=== CLEANED DATA PROFIT TARGET STRATEGY COMPARISON ===');
    console.log('Time Period: August 23 - September 26, 2025 (35 days)');
    console.log('No "Unknown" callers - all data properly attributed!');
    ...
    console.log('\nRanking by Total Return (CLEANED DATA):');
    ...
    fs.writeFileSync('/home/memez/quantBot/cleaned_profit_target_comparison.json', JSON.stringify({
        strategies: comparison,
        riskManagement: RISK_MANAGEMENT_RULES,
        timePeriod: "August 23 - September 26, 2025 (35 days)",
        summary: {
    ...
    console.log('Cleaned comparison saved to cleaned_profit_target_comparison.json');
    
    // Create CSV for easy analysis
    const csvWriter = createCsvWriter({
        path: '/home/memez/quantBot/cleaned_profit_target_comparison.csv',
        header: [
    ...
    csvWriter.writeRecords(comparison)
        .then(() => {
            console.log('Cleaned comparison CSV saved to cleaned_profit_target_comparison.csv');
        })
        .catch(console.error);
}

// Run the cleaned profit target variations simulation
simulateCleanedProfitTargetVariations()
    .then((allResults) => {
        console.log('\n=== CLEANED PROFIT TARGET VARIATIONS COMPLETE ===');
        console.log(`Tested ${Object.keys(allResults).length} different profit target strategies on cleaned data`);
        console.log('Time Period: August 23 - September 26, 2025 (35 days)');
        console.log('No "Unknown" callers - all data properly attributed!');
