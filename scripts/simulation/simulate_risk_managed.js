...
async function simulateWithRiskManagement() {
    const results = [];
    const inputFile = '/home/memez/quantBot/filtered_ca_drops.csv';
    
    console.log('Simulating with PROPER RISK MANAGEMENT...');
        ...
        fs.createReadStream(inputFile)
            ...
            .on('end', () => {
                ...
                Object.entries(callerData).forEach(([caller, calls]) => {
                    console.log(`\nSimulating ${caller} (${calls.length} calls) with risk management...`);
                ...
                const csvWriter = createCsvWriter({
                    path: '/home/memez/quantBot/risk_managed_simulations.csv',
                    header: [
                ...
                csvWriter.writeRecords(results)
                    .then(() => {
                        console.log('\nRisk-managed simulations saved to risk_managed_simulations.csv');
                        
                        // Create detailed analysis
                        createRiskManagementAnalysisReport(results);
...
function simulateCallerWithRiskManagement(caller, calls) {
    ...
    const portfolioReturn = totalReturn; // Same as total return since we're using portfolio size
...
function createRiskManagementAnalysisReport(results) {
    console.log('\n=== RISK-MANAGED STRATEGY PERFORMANCE ANALYSIS ===');
    console.log(`Portfolio Size: $${RISK_MANAGEMENT_RULES.portfolioSize}`);
    console.log(`Max Risk Per Trade: ${RISK_MANAGEMENT_RULES.maxRiskPerTrade * 100}%`);
    console.log(`Max Position Size: ${RISK_MANAGEMENT_RULES.maxPositionSize * 100}% of portfolio`);
    ...
    console.log('\n=== RISK-MANAGED STRATEGY STATISTICS ===');
    ...
    fs.writeFileSync('/home/memez/quantBot/risk_managed_analysis.json', JSON.stringify(comparison, null, 2));
    console.log('\nDetailed analysis saved to risk_managed_analysis.json');
}

// Run the risk-managed simulation
simulateWithRiskManagement()
    .then((results) => {
        console.log(`\nRisk-managed simulation complete! Analyzed ${results.length} callers.`);
        
        // Print top performers
        const topPerformers = results.sort((a, b) => b.portfolioReturn - a.portfolioReturn).slice(0, 3);
        console.log('\nTop 3 Performers with Risk Management:');
