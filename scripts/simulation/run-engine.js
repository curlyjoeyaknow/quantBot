...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        switch (arg) {
            ...
            case '-j':
                if (!next)
                    throw new Error('--maxConcurrency requires a number');
...
async function loadConfigFromFile(configPath) {
    ...
    try {
        ...
        return (0, simulation_1.loadSimulationConfig)(raw);
    }
    catch (error) {
...
async function main() {
    ...
    const summaries = await (0, simulation_1.runSimulationConfig)(config);
    console.log('\n=== Simulation Summary ===');
    for (const summary of summaries) {
        console.log(`${summary.scenarioName}: ${summary.successes}/${summary.totalTargets} succeeded, ${summary.failures} failed`);
    }
}
main().catch((error) => {
