...
// RISK MANAGEMENT RULES (WEEKLY REBALANCING WITH COMPOUNDING)
const RISK_MANAGEMENT_RULES = {
    initialPortfolioSize: 3500, // $3,500 USD initial portfolio
    positionSizePercent: 0.05, // 5% of portfolio value per trade
...
function simulateTradeWithWeeklyRebalancing(call, tradeIndex, strategy, currentPortfolioValue) {
    ...
    // Calculate position size as 5% of current portfolio value
...
function createWeeklyRebalancedComparisonReport(allResults) {
    ...
    console.log('Time Period: July 30 - November, 2025 (~120+ days)');
    console.log('WEEKLY REBALANCING with 5% of portfolio value per trade!');
    ...
    console.log('\nRanking by Total Final Portfolio Value (WEEKLY REBALANCED):');
    comparison.forEach((strategy, index) => {
        console.log(`${index + 1}. ${strategy.strategy}`);
        console.log(`   Total Final Value: $${strategy.totalFinalValue}`);
        ...
        console.log(`   Best Performer: ${strategy.bestPerformer} (${strategy.bestReturn}%, $${strategy.bestFinalValue})`);
        console.log('');
    });
    
    // Save detailed comparison
    fs.writeFileSync('/home/memez/quantBot/weekly_rebalanced_comparison.json', JSON.stringify({
        strategies: comparison,
        riskManagement: RISK_MANAGEMENT_RULES,
        timePeriod: "July 30 - November, 2025 (~120+ days)",
        rebalancing: "Weekly with 5% of portfolio value per trade",
        summary: {
    ...
    console.log('Weekly rebalanced comparison saved to weekly_rebalanced_comparison.json');
    
    // Create CSV for easy analysis
    const csvWriter = createCsvWriter({
        path: '/home/memez/quantBot/weekly_rebalanced_comparison.csv',
        header: [
    ...
    csvWriter.writeRecords(comparison)
        .then(() => {
            console.log('Weekly rebalanced comparison CSV saved to weekly_rebalanced_comparison.csv');
        })
        .catch(console.error);
}
...
simulateWeeklyRebalancedProfitTargetVariations()
    .then((allResults) => {
        ...
        console.log('Time Period: July 30 - November, 2025 (~120+ days)');
        console.log('Position Size: 5% of portfolio value per trade (compounding)');
        console.log('Rebalancing: Weekly (every 7 days)');
