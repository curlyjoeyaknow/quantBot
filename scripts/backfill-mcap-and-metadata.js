...
exports.backfillMcapAndMetadata = backfillMcapAndMetadata;
const sqlite3_1 = require("sqlite3");
const util_1 = require("util");
const luxon_1 = require("luxon");
const path = __importStar(require("path"));
const mcap_calculator_1 = require("../packages/web/lib/services/mcap-calculator");
 ...
 */
async function fetchBirdeyeMetadata(tokenAddress, chain = 'solana') {
    if (!BIRDEYE_API_KEY) {
 ...
 */
async function getTokensNeedingUpdate(db) {
 ...
 */
async function updateTokenData(db, callId, entryMcap, metadata) {
 ...
 */
async function storeMetadataCache(db, tokenAddress, metadata) {
    const run = (0, util_1.promisify)(db.run.bind(db));
    try {
 ...
 */
async function processToken(db, call, stats) {
    stats.processed++;
    ...
    if (!entryMcap && call.price_at_alert) {
        // Try fallback chain
        const alertTime = luxon_1.DateTime.fromISO(call.alert_timestamp);
        // Check pump/bonk first (fastest)
        if ((0, mcap_calculator_1.isPumpOrBonkToken)(call.token_address)) {
            entryMcap = (0, mcap_calculator_1.calculatePumpBonkMcap)(call.price_at_alert);
            console.log(`  ✅ Pump/bonk MCAP: ${(0, mcap_calculator_1.formatMcap)(entryMcap)}`);
            stats.mcapFromPump++;
        }
        else {
            // Try Birdeye
            ...
            if (metadata?.mc) {
                ...
                if (metadata.price && call.price_at_alert) {
                    ...
                    console.log(`  ✅ Inferred MCAP: ${(0, mcap_calculator_1.formatMcap)(entryMcap)} (from current ${(0, mcap_calculator_1.formatMcap)(metadata.mc)})`);
                    stats.mcapFromInfer++;
                }
                else {
                    // Use current MCAP as approximation
                    entryMcap = metadata.mc;
                    console.log(`  ⚠️ Using current MCAP: ${(0, mcap_calculator_1.formatMcap)(entryMcap)} (no price data to infer)`);
                    stats.mcapFromBirdeye++;
                }
            }
            else if (call.alert_message) {
                // Try extracting from message
                const extractedMcap = (0, mcap_calculator_1.extractMcapFromMessage)(call.alert_message);
                if (extractedMcap) {
                    entryMcap = extractedMcap;
                    console.log(`  ✅ Extracted MCAP from message: ${(0, mcap_calculator_1.formatMcap)(entryMcap)}`);
 ...
 */
async function backfillMcapAndMetadata() {
...
//# sourceMappingURL=backfill-mcap-and-metadata.js.map