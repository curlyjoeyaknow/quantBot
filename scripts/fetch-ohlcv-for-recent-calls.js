...
const postgres_manager_1 = require("../packages/web/lib/db/postgres-manager");
/**
 * Get recent alerts from PostgreSQL database, ordered by newest first
 */
async function getRecentAlerts(limit = 100) {
    try {
        const result = await postgres_manager_1.postgresManager.query(`
      SELECT 
        a.id,
      ...
      `, [limit]);
        ...
        logger_1.logger.error('Error fetching recent alerts from database', error);
        throw error;
    }
}
/**
 ...
 */
async function fetchCandlesForAlert(alert) {
    ...
    logger_1.logger.info(`Fetching candles for alert ${alert.id}`, {
    ...
    try {
        const candles = await (0, candles_1.fetchHybridCandles)(alert.tokenAddress, // Full mint address - never concatenate!
        startTime, endTime, alert.chain, alertTime // Pass alertTime to ensure 52-period lookback and 1m candles
        );
        logger_1.logger.info(`✅ Successfully fetched ${candles.length} candles for alert ${alert.id}`, {
            token: alert.tokenAddress.substring(0, 20),
            symbol: alert.tokenSymbol,
        ...
        logger_1.logger.error(`❌ Failed to fetch candles for alert ${alert.id}`, error, {
            token: alert.tokenAddress.substring(0, 20),
            symbol: alert.tokenSymbol,
            chain: alert.chain,
            error: error.message,
...
async function main() {
    ...
    if (isNaN(limit) || limit <= 0) {
        logger_1.logger.error('Invalid limit provided. Must be a positive number.');
        process.exit(1);
    }
    logger_1.logger.info(`Starting OHLCV fetch for most recent ${limit} calls`, { limit });
    try {
        // Get recent alerts (newest first)
        const alerts = await getRecentAlerts(limit);
        if (alerts.length === 0) {
            logger_1.logger.warn('No alerts found in database');
            process.exit(0);
        }
        logger_1.logger.info(`Found ${alerts.length} recent alerts, processing from newest to oldest`);
        ...
        for (let i = 0; i < alerts.length; i++) {
            ...
            logger_1.logger.info(`${progress} Processing alert ${alert.id} (${alert.tokenSymbol || 'N/A'}) from ${alert.callerName || 'Unknown'}`);
            try {
                ...
                logger_1.logger.warn(`${progress} Failed to process alert ${alert.id}, continuing...`);
            }
        }
        // Summary
        logger_1.logger.info('✅ OHLCV fetch complete', {
            total: alerts.length,
        ...
        if (errors.length > 0) {
            logger_1.logger.warn('Failed alerts:', {
                count: errors.length,
...
//# sourceMappingURL=fetch-ohlcv-for-recent-calls.js.map