...
const OUTPUT_JSON_PATH = path.join(__dirname, 'original_strategy_influxdb_results.json');
const OUTPUT_CSV_PATH = path.join(__dirname, 'original_strategy_influxdb_results.csv');
const DETAILED_TRADES_PATH = path.join(__dirname, 'original_strategy_influxdb_detailed_trades.csv');
...
async function runOriginalStrategySimulation() {
    console.log('🚀 Running ORIGINAL STRATEGY simulation using InfluxDB...');
    ...
    try {
        // Initialize InfluxDB services
        await ohlcvIngestion.initialize();
        console.log('✅ InfluxDB services initialized');
        ...
        const summary = {
            strategyName: 'original_strategy_influxdb',
