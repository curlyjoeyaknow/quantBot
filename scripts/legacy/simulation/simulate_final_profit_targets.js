...
async function simulateFinalProfitTargetVariations() {
    const inputFile = '/home/memez/quantBot/final_complete_filtered_ca_drops.csv';
    const allResults = {};
    
    console.log('Testing PROFIT TARGET VARIATIONS on FINAL COMPLETE DATASET (July 30 - November, 2025)...');
    ...
    for (const [strategyKey, strategy] of Object.entries(PROFIT_TARGET_STRATEGIES)) {
        ...
        const results = await simulateStrategyFinal(inputFile, strategy);
    ...
    createFinalProfitTargetComparisonReport(allResults);
    
    return allResults;
}

async function simulateStrategyFinal(inputFile, strategy) {
    return new Promise((resolve, reject) => {
        ...
        fs.createReadStream(inputFile)
            ...
            .on('end', () => {
                ...
                Object.entries(callerData).forEach(([caller, calls]) => {
                    const simulation = simulateCallerWithStrategyFinal(caller, calls, strategy);
...
function simulateCallerWithStrategyFinal(caller, calls, strategy) {
    ...
    // Process up to 150 calls per caller (increased for longer period)
    const callsToProcess = calls.slice(0, 150);
    
    callsToProcess.forEach((call, index) => {
        const trade = simulateTradeWithStrategyFinal(call, index, strategy);
        if (trade) {
            trades.push(trade);
...
function simulateTradeWithStrategyFinal(call, tradeIndex, strategy) {
    ...
    let trade = attemptInitialEntryWithStrategyFinal(call, actualEntryPrice, positionSize, holdTime, tradeIndex, false, strategy);
    ...
    if (trade && trade.exitReason === 'stop_loss') {
        ...
        trade = attemptReentryWithStrategyFinal(call, actualReentryPrice, positionSize, holdTime, tradeIndex, true, strategy);
    }
    
    return trade;
}

function attemptInitialEntryWithStrategyFinal(call, entryPrice, positionSize, holdTime, tradeIndex, isReentry, strategy) {
...
function attemptReentryWithStrategyFinal(call, reentryPrice, positionSize, holdTime, tradeIndex, isReentry, strategy) {
    return attemptInitialEntryWithStrategyFinal(call, reentryPrice, positionSize, holdTime, tradeIndex, true, strategy);
}

function createFinalProfitTargetComparisonReport(allResults) {
    console.log('\n=== FINAL COMPLETE DATASET PROFIT TARGET STRATEGY COMPARISON ===');
    console.log('Time Period: July 30 - November, 2025 (~120+ days)');
    console.log('FINAL COMPLETE dataset with brook2 (Aug-Sep) + brook (Sep-Oct) + brook3 (Oct-Nov) data!');
    ...
    console.log('\nRanking by Total Return (FINAL COMPLETE ~120+ DAY DATASET):');
    ...
    fs.writeFileSync('/home/memez/quantBot/final_profit_target_comparison.json', JSON.stringify({
    ...
    console.log('Final comparison saved to final_profit_target_comparison.json');
    
    // Create CSV for easy analysis
    const csvWriter = createCsvWriter({
        path: '/home/memez/quantBot/final_profit_target_comparison.csv',
        header: [
    ...
    csvWriter.writeRecords(comparison)
        .then(() => {
            console.log('Final comparison CSV saved to final_profit_target_comparison.csv');
        })
        .catch(console.error);
}

// Run the final profit target variations simulation
simulateFinalProfitTargetVariations()
    .then((allResults) => {
        console.log('\n=== FINAL PROFIT TARGET VARIATIONS COMPLETE ===');
        console.log(`Tested ${Object.keys(allResults).length} different profit target strategies on FINAL COMPLETE dataset`);
        console.log('Time Period: July 30 - November, 2025 (~120+ days)');
        console.log('FINAL COMPLETE dataset with brook2 (Aug-Sep) + brook (Sep-Oct) + brook3 (Oct-Nov) data!');
