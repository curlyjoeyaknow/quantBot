...
const OUTPUT_JSON_PATH = path.join(__dirname, 'real_data_simulation_results.json');
const OUTPUT_CSV_PATH = path.join(__dirname, 'real_data_simulation_results.csv');
...
// Function to fetch real OHLCV data from Birdeye API
async function fetchRealOHLCVData(tokenAddress, alertTimestamp) {
    try {
        console.log(`Fetching real data for ${tokenAddress} at ${alertTimestamp.toISOString()}`);
        
        // Convert timestamp to Unix timestamp
        const unixTimestamp = Math.floor(alertTimestamp.getTime() / 1000);
        ...
        const response = await axios.get(`https://public-api.birdeye.so/public/v1/token/price_history`, {
            ...
            headers: {
                'X-API-KEY': 'dec8084b90724ffe949b68d0a18359d6'
            },
            timeout: 10000
        });
        
        if (response.data && response.data.items && response.data.items.length > 0) {
            const candles = response.data.items.map(candle => ({
            ...
            console.log(`✅ Got ${candles.length} real candles for ${tokenAddress}`);
            return candles;
        } else {
            console.log(`❌ No candle data returned for ${tokenAddress}`);
            return null;
        }
        
    } catch (error) {
        console.warn(`❌ Failed to fetch real data for ${tokenAddress}:`, error.message);
        return null;
    }
}

// Function to simulate a single trade with REAL data
async function simulateTradeWithRealData(call, strategyRules) {
    ...
    const candles = await fetchRealOHLCVData(tokenAddress, alertTimestamp);
    
    if (!candles || candles.length === 0) {
        console.log(`❌ No real data available for ${tokenAddress}`);
        return null;
    }
    ...
    console.log(`📊 Entry: $${entryPrice.toFixed(6)}, SL: $${stopLossPrice.toFixed(6)}, Re-entry: $${reentryPrice.toFixed(6)}`);
    console.log(`🎯 Take Profits: ${takeProfitLevels.map(tp => `${tp.multiplier}x@$${tp.price.toFixed(6)}`).join(', ')}`);
    ...
    let tradeResult = {
        ...
        exitPrice: 0,
        exitTimestamp: null,
        candleData: candles // Store actual candle data for verification
    };
    ...
    for (let i = 0; i < candles.length; i++) {
        ...
        if (candle.low <= stopLossPrice) {
            ...
            console.log(`🛑 STOP LOSS HIT at candle ${i}: $${exitPrice.toFixed(6)} (PnL: ${pnlSOL.toFixed(4)} SOL)`);
            
            // Check if re-entry should be attempted
            if (strategyRules.reentry.enabled) {
                console.log(`🔄 Attempting re-entry at $${reentryPrice.toFixed(6)}`);
                ...
                for (let j = i; j < candles.length; j++) {
                    ...
                    if (reentryCandle.low <= reentryPrice) {
                        ...
                        console.log(`✅ RE-ENTRY EXECUTED at candle ${j}: $${reentryEntryPrice.toFixed(6)}`);
                        ...
                        for (let k = j; k < candles.length; k++) {
                            ...
                            if (reentryCheckCandle.low <= reentryStopLossPrice) {
                                ...
                                console.log(`🛑 RE-ENTRY STOP LOSS HIT at candle ${k}: $${reentryExitPrice.toFixed(6)} (PnL: ${reentryPnLSOL.toFixed(4)} SOL)`);
                            ...
                            for (const tp of takeProfitLevels) {
                                if (reentryCheckCandle.high >= tp.price) {
                                    ...
                                    console.log(`🎯 RE-ENTRY TAKE PROFIT ${tp.multiplier}x HIT at candle ${k}: $${reentryExitPrice.toFixed(6)} (PnL: ${reentryPnLSOL.toFixed(4)} SOL)`);
                        ...
                        console.log(`⏰ RE-ENTRY TIMEOUT at last candle: $${reentryExitPrice.toFixed(6)} (PnL: ${reentryPnLSOL.toFixed(4)} SOL)`);
        ...
        for (const tp of takeProfitLevels) {
            if (candle.high >= tp.price) {
                ...
                console.log(`🎯 TAKE PROFIT ${tp.multiplier}x HIT at candle ${i}: $${exitPrice.toFixed(6)} (PnL: ${pnlSOL.toFixed(4)} SOL)`);
    ...
    console.log(`⏰ TIMEOUT at last candle: $${exitPrice.toFixed(6)} (PnL: ${pnlSOL.toFixed(4)} SOL)`);
    
    return tradeResult;
}

async function runRealDataSimulation() {
    console.log('🚀 Running REAL DATA simulation...');
    console.log('📊 Using actual 1-minute OHLCV data from Birdeye API');
    ...
    console.log('⚠️  NOTE: You need to add your Birdeye API key to fetch real data!');
    ...
    const strategyRules = {
        ...TRADING_RULES,
        takeProfit: TRADING_RULES.takeProfit.original
    };
    ...
    // Process each valid record (limit to 10 for testing with real API calls)
    for (let i = 0; i < Math.min(validRecords.length, 10); i++) {
        const call = validRecords[i];
        console.log(`\n📈 Processing trade ${i + 1}/${Math.min(validRecords.length, 10)}: ${call['Address']}`);
    ...
    console.log('✅ REAL 1-minute OHLCV data from Birdeye API');
    ...
    console.log('\n⚠️  To use with real data, add your Birdeye API key to the script!');
}

// Run the simulation
runRealDataSimulation().catch(console.error);
