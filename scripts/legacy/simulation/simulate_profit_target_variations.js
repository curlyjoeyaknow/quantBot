...
// RISK MANAGEMENT RULES (same for all strategies)
...
async function simulateProfitTargetVariations() {
    const inputFile = '/home/memez/quantBot/expanded_filtered_ca_drops.csv';
    const allResults = {};
    
    console.log('Testing PROFIT TARGET VARIATIONS with YOUR EXACT TRADING RULES...');
    ...
    for (const [strategyKey, strategy] of Object.entries(PROFIT_TARGET_STRATEGIES)) {
        ...
        const results = await simulateStrategy(inputFile, strategy);
    ...
    createProfitTargetComparisonReport(allResults);
    
    return allResults;
}

async function simulateStrategy(inputFile, strategy) {
        ...
        fs.createReadStream(inputFile)
            ...
            .on('end', () => {
                ...
                Object.entries(callerData).forEach(([caller, calls]) => {
                    const simulation = simulateCallerWithStrategy(caller, calls, strategy);
...
function simulateCallerWithStrategy(caller, calls, strategy) {
    ...
    callsToProcess.forEach((call, index) => {
        const trade = simulateTradeWithStrategy(call, portfolioValue, index, strategy);
        if (trade) {
            trades.push(trade);
...
function simulateTradeWithStrategy(call, currentPortfolioValue, tradeIndex, strategy) {
    ...
    let trade = attemptInitialEntryWithStrategy(call, actualEntryPrice, positionSize, holdTime, tradeIndex, false, strategy);
    ...
    if (trade && trade.exitReason === 'stop_loss') {
        ...
        trade = attemptReentryWithStrategy(call, actualReentryPrice, positionSize, holdTime, tradeIndex, true, strategy);
    }
    
    return trade;
}

function attemptInitialEntryWithStrategy(call, entryPrice, positionSize, holdTime, tradeIndex, isReentry, strategy) {
...
function attemptReentryWithStrategy(call, reentryPrice, positionSize, holdTime, tradeIndex, isReentry, strategy) {
    return attemptInitialEntryWithStrategy(call, reentryPrice, positionSize, holdTime, tradeIndex, true, strategy);
}

function createProfitTargetComparisonReport(allResults) {
    console.log('\n=== PROFIT TARGET STRATEGY COMPARISON ===');
    ...
    fs.writeFileSync('/home/memez/quantBot/profit_target_comparison.json', JSON.stringify({
    ...
    console.log('Detailed comparison saved to profit_target_comparison.json');
    
    // Create CSV for easy analysis
    const csvWriter = createCsvWriter({
        path: '/home/memez/quantBot/profit_target_comparison.csv',
        header: [
    ...
    csvWriter.writeRecords(comparison)
        .then(() => {
            console.log('Comparison CSV saved to profit_target_comparison.csv');
        })
        .catch(console.error);
}

// Run the profit target variations simulation
simulateProfitTargetVariations()
    .then((allResults) => {
        console.log('\n=== PROFIT TARGET VARIATIONS COMPLETE ===');
        console.log(`Tested ${Object.keys(allResults).length} different profit target strategies`);
