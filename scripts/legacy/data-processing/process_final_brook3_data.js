...
// Function to process all HTML files in brook3 directory
async function processBrook3Files() {
    const brook3Dir = '/home/memez/quantBot/messages/brook3';
    const files = fs.readdirSync(brook3Dir).filter(file => file.endsWith('.html'));
    
    console.log(`Found ${files.length} HTML files in brook3 directory:`, files);
    
    let allCaDrops = [];
    
    for (const file of files) {
        console.log(`Processing ${file}...`);
        const filePath = path.join(brook3Dir, file);
    ...
    console.log(`\nTotal CA drops found in brook3: ${allCaDrops.length}`);
    
    // Load existing complete data (brook2 + brook)
    const existingData = JSON.parse(fs.readFileSync('/home/memez/quantBot/complete_filtered_ca_drops.json', 'utf8'));
    console.log(`Existing complete CA drops: ${existingData.length}`);
    
    // Combine all CA drops
    const combinedCaDrops = [...existingData, ...allCaDrops];
    ...
    // Save filtered data
    fs.writeFileSync('/home/memez/quantBot/final_complete_filtered_ca_drops.json', JSON.stringify(filteredCaDrops, null, 2));
    console.log('Final complete filtered CA drops saved to final_complete_filtered_ca_drops.json');
    ...
    fs.writeFileSync('/home/memez/quantBot/final_complete_filtered_ca_drops.csv', csvContent);
    console.log('Final complete filtered CA drops saved to final_complete_filtered_ca_drops.csv');
    
    return {
        totalCaDrops: combinedCaDrops.length,
        filteredCaDrops: filteredCaDrops.length,
        filesProcessed: files.length,
        newCaDrops: allCaDrops.length
    };
}
...
processBrook3Files()
    .then((results) => {
        console.log('\n=== FINAL COMPLETE BROOK3 PROCESSING COMPLETE ===');
        console.log(`Brook3 files processed: ${results.filesProcessed}`);
        console.log(`New CA drops from brook3: ${results.newCaDrops}`);
        console.log(`Total CA drops: ${results.totalCaDrops}`);
        console.log(`Final filtered CA drops: ${results.filteredCaDrops}`);
        console.log('\nReady to run FINAL simulations with FULL August-November data!');
    })
    .catch(console.error);
