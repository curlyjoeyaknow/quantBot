  ...
  try {
    ...
    const client = createClient({
      host:
        ...
        `http://${process.env.CLICKHOUSE_HOST || 'localhost'}:${process.env.CLICKHOUSE_HTTP_PORT || '18123'}`,
      ...
      const intervalLabel =
        row.interval_seconds === 60
          ...
          : row.interval_seconds === 300
            ? '5m'
            : row.interval_seconds === 15
              ? '15s'
              : row.interval_seconds === 1
                ? '1s'
                : row.interval_seconds === 3600
                  ? '1h'
                  : `${row.interval_seconds}s`;
      console.log(
        `  ${row.chain}/${intervalLabel}: ${row.cnt.toLocaleString()} candles (${row.min_ts} to ${row.max_ts})`
      );
    }
