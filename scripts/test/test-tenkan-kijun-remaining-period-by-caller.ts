...
async function analyzeCaller(callerName: string, records: any[]): Promise<void> {
  ...
  const dailyPortfolioHistory: Array<{ date: string; reinvestment: number; noReinvestment: number }> = [];
  const weeklyPortfolioHistory: Array<{ week: string; reinvestment: number; noReinvestment: number; maxDrawdownReinvestment: number; maxDrawdownNoReinvestment: number }> = [];
