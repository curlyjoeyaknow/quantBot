 ...
 */
function consolidateTo1hCandles(candles) {
    if (candles.length === 0)
        return [];
    const oneHourCandles = [];
    const oneHourMs = 60 * 60 * 1000; // 1 hour in milliseconds
    let currentHourStart = null;
    let currentHourCandle = null;
 ...
 */
function simulateTenkanKijun1hStrategy(candles, alertTime) {
    ...
    for (let i = 0; i < oneHourCandles.length; i++) {
        const indicators = (0, indicators_1.calculateIndicators)(oneHourCandles, i, previousEMAs);
...
//# sourceMappingURL=test-tenkan-kijun-1h-candles-no-reinvestment.js.map