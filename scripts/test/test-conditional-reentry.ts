...
async function testConditionalReentry() {
  ...
  const strategies = [
    {
      name: '50% @ 2x, then 50% @ 5x on re-entry',
      strategy: [
    ...
    {
      name: '50% @ 2x, then 50% @ 10x on re-entry',
      strategy: [
        { percent: 0.5, target: 2 },
    ...
    {
      name: '100% @ 2x, then 100% @ 3x on re-entry',
      strategy: [
  ...
  for (const strat of strategies) {
    ...
    console.log(
      `${label} | ${String(r.winners).padStart(4)} | ${String(r.losers).padStart(4)} | ${netPnl.padStart(7)}x | ${String(r.hitFirstTarget).padStart(7)} | ${String(r.reEntered).padStart(10)}`
    );
  }
}
