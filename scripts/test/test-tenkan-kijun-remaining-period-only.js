    ...
    for (let i = startIndex; i < candles.length; i++) {
        ...
        if (i > startIndex && i > 0) {
            ...
            if (candleDurationMs <= 0)
                candleDurationMs = 60 * 60 * 1000; // Fallback to 1h
        }
...
async function testRemainingPeriodOnly() {
    ...
    const portfolioValues = [
        { week: 'start', value: initialPortfolio, date: luxon_1.DateTime.fromISO(sortedTrades[0]?.alertTime || '') }
    ];
    for (const [weekKey, weekTrades] of sortedWeeks) {
        ...
        const weekDate = luxon_1.DateTime.fromISO(weekKey);
    ...
    const firstTradeDate = luxon_1.DateTime.fromISO(sortedTrades[0]?.alertTime || '');
    const lastTradeDate = luxon_1.DateTime.fromISO(sortedTrades[sortedTrades.length - 1]?.alertTime || '');
    ...
    const weeklyReturnStdDev = Math.sqrt(weeklyReturns.reduce((sum, r) => sum + Math.pow(r - avgWeeklyReturn, 2), 0) / weeklyReturns.length);
...
//# sourceMappingURL=test-tenkan-kijun-remaining-period-only.js.map