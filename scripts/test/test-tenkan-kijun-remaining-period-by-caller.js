...
function simulateTenkanKijunFromAlert(candles, alertTime) {
...
async function analyzeCaller(callerName, records) {
    console.log(`\n${'='.repeat(80)}`);
    ...
    console.log(`   Total Cost per Round Trip: ~${(ENTRY_SLIPPAGE_PCT + EXIT_SLIPPAGE_PCT + TRADING_FEE_PCT * 2).toFixed(2)}%\n`);
    ...
    if (trades.length < 20) {
        console.log(`⏭️  [${callerName}] Skipping - only ${trades.length} trades (minimum 20 required)\n`);
        return;
    }
    // Calculate metrics
    ...
    if (CLAMP_MIN_PNL) {
        ...
        console.log(`⚠️  Loss-clamp is OFF - capping position size at ${(positionSizePercent * 100).toFixed(2)}% to maintain 2% max risk per trade`);
    }
    const movingAverageWindow = 10; // Use last 10 trades for moving average
    let portfolio = initialPortfolio;
    const portfolioHistory = [initialPortfolio]; // Track portfolio values for moving average
    // Track daily and weekly portfolio values for both reinvestment and non-reinvestment
    const dailyPortfolioHistory = [];
    const weeklyPortfolioHistory = [];
    ...
    const simplePortfolioHistory = [initialPortfolio];
    ...
    if (CLAMP_MIN_PNL) {
        console.log(`   Position Size %: ${(positionSizePercent * 100).toFixed(2)}% (risk: ${(maxRiskPerTrade * 100).toFixed(2)}%, stop loss: ${(stopLossPercent * 100).toFixed(2)}%)`);
    }
    else {
        console.log(`   Position Size %: ${(positionSizePercent * 100).toFixed(2)}% (risk: ${(maxRiskPerTrade * 100).toFixed(2)}%, max loss: 100%)`);
    }
    ...
    for (const trade of sortedTrades) {
        ...
        const movingAvgPortfolio = recentPortfolios.reduce((sum, p) => sum + p, 0) / recentPortfolios.length;
        ...
        const tradeDate = luxon_1.DateTime.fromISO(trade.exitTime).toISODate() || '';
        if (tradeDate) {
        ...
        if (tradeNum % 50 === 0 || Math.abs(tradeReturn) > portfolioBefore * 0.05 || tradeNum <= 10) {
            ...
            console.log(`      PnL: ${pnlPercent >= 0 ? '+' : ''}${pnlPercent.toFixed(2)}% (${trade.pnl.toFixed(4)}x)`);
            console.log(`      Portfolio: $${portfolioBefore.toFixed(2)} → $${portfolio.toFixed(2)} (${returnPercent >= 0 ? '+' : ''}${returnPercent.toFixed(2)}%)`);
            console.log(`      Position Size: $${currentPositionSize.toFixed(2)} (from MA: $${movingAvgPortfolio.toFixed(2)})`);
    ...
    const avgPerTradeMaxDrawdown = perTradeMaxDrawdowns.reduce((sum, d) => sum + d, 0) / perTradeMaxDrawdowns.length;
    const maxPerTradeMaxDrawdown = Math.max(...perTradeMaxDrawdowns);
    const avgRiskRatio = perTradeRiskRatios.reduce((sum, r) => sum + r, 0) / perTradeRiskRatios.length;
    const medianRiskRatio = perTradeRiskRatios.sort((a, b) => a - b)[Math.floor(perTradeRiskRatios.length / 2)];
    ...
    if (sortedTrades.length > 0) {
        ...
        const firstEntryMs = Math.min(...sortedTrades.map(t => luxon_1.DateTime.fromISO(t.entryTime).toMillis()));
        const lastExitMs = Math.max(...sortedTrades.map(t => luxon_1.DateTime.fromISO(t.exitTime).toMillis()));
    ...
    const { maxDrawdown: maxDrawdownReinvestment, maxDrawdownPct: maxDrawdownPctReinvestment } = computeMaxDrawdown(portfolioHistory);
    const { maxDrawdown: maxDrawdownNoReinvestment, maxDrawdownPct: maxDrawdownPctNoReinvestment } = computeMaxDrawdown(simplePortfolioHistory);
    ...
    const simpleFinalPortfolio = simplePortfolioHistory[simplePortfolioHistory.length - 1] || initialPortfolio;
    const simpleReturnPercent = ((simpleFinalPortfolio / initialPortfolio) - 1) * 100;
    // Generate weekly summary
    const weeklyMap = new Map();
    for (let i = 0; i < sortedTrades.length; i++) {
        const trade = sortedTrades[i];
        const exitDate = luxon_1.DateTime.fromISO(trade.exitTime);
        ...
        if (weekStart) {
            ...
            const reinvestmentValue = portfolioHistory[i + 1] || portfolioHistory[portfolioHistory.length - 1];
            const noReinvestmentValue = simplePortfolioHistory[i + 1] || simplePortfolioHistory[simplePortfolioHistory.length - 1];
    ...
    for (const [weekStart, values] of weeklyMap.entries()) {
        const weekReinvestment = values.reinvestment[values.reinvestment.length - 1] || initialPortfolio;
        const weekNoReinvestment = values.noReinvestment[values.noReinvestment.length - 1] || initialPortfolio;
        // Calculate max drawdown for this week's reinvestment values
        const weekMaxDrawdownReinvestment = values.reinvestment.length > 0
            ? computeMaxDrawdown(values.reinvestment).maxDrawdownPct
            : 0;
        const weekMaxDrawdownNoReinvestment = values.noReinvestment.length > 0
    ...
    await new Promise((resolve, reject) => {
        (0, csv_stringify_1.stringify)(tradeHistoryRows, { header: true }, (err, output) => {
            if (err)
                reject(err);
            else {
    ...
    await new Promise((resolve, reject) => {
        (0, csv_stringify_1.stringify)(dailySummaryRows, { header: true }, (err, output) => {
            if (err)
                reject(err);
            else {
    ...
    await new Promise((resolve, reject) => {
        (0, csv_stringify_1.stringify)(weeklySummaryRows, { header: true }, (err, output) => {
            if (err)
                reject(err);
            else {
    ...
    console.log(`   Total Return (sum of all trades): ${totalReturn >= 0 ? '+' : ''}${totalReturn.toFixed(2)}%`);
    ...
    console.log(`      Total Return: ${simpleReturnPercent >= 0 ? '+' : ''}${simpleReturnPercent.toFixed(2)}%`);
    ...
    console.log(`      Benefit %: ${reinvestmentBenefitPercent >= 0 ? '+' : ''}${reinvestmentBenefitPercent.toFixed(2)}%`);
    // Risk metrics
    console.log(`\n📉 RISK METRICS:`);
    console.log(`   WITH REINVESTMENT:`);
    console.log(`      Portfolio Max Drawdown: $${maxDrawdownReinvestment.toFixed(2)} (${maxDrawdownPctReinvestment.toFixed(2)}%)`);
    console.log(`      Peak Portfolio: $${Math.max(...portfolioHistory).toFixed(2)}`);
    console.log(`   WITHOUT REINVESTMENT:`);
    console.log(`      Portfolio Max Drawdown: $${maxDrawdownNoReinvestment.toFixed(2)} (${maxDrawdownPctNoReinvestment.toFixed(2)}%)`);
    ...
    console.log(`   Annualized TWR (compounded, theoretical): ${twrAnnualPct >= 0 ? '+' : ''}${twrAnnualPct.toFixed(2)}%`);
    if (twrAnnualDrawdownAdjusted > 0 && totalGrowth > 0) {
        ...
        console.log(`   Annualized TWR (realistic projection): ${twrAnnualDrawdownAdjusted >= 0 ? '+' : ''}${twrAnnualDrawdownAdjusted.toFixed(2)}%`);
        console.log(`      Reduction factor from ${daysActive.toFixed(1)}-day period: ${reductionFactor.toFixed(4)}`);
        console.log(`      (Theoretical: $${theoreticalValue.toFixed(2)}, Actual: $${actualValue.toFixed(2)})`);
        console.log(`   Calmar Ratio: ${calmarRatio.toFixed(2)} (annual return / max drawdown, higher = better)`);
    }
    console.log(`   ⚠️  Note: Compounded assumes same ${twrDailyPct.toFixed(2)}% daily return for 365 days (unrealistic).`);
    console.log(`      Realistic projection applies observed compounding efficiency to annual projection.`);
    // Risk-adjusted score
    const riskAdjustedScore = stdDevReturnsPct > 0 ? twrDailyPct / stdDevReturnsPct : 0;
    console.log(`\n📊 RISK-ADJUSTED METRICS:`);
    console.log(`   TWR/Volatility Score: ${riskAdjustedScore.toFixed(2)} (higher = better risk-adjusted returns)`);
    console.log(`\n📁 FILES SAVED:`);
...
async function createConsolidatedSummary() {
    ...
    for (const callerDir of callerDirs) {
        ...
        try {
            ...
            summaries.push({
                ...
                InitialPortfolio: summaryJson.initialPortfolio ? summaryJson.initialPortfolio.toFixed(2) : '100.00',
                FinalPortfolio_Reinvestment: summaryJson.finalPortfolio ? summaryJson.finalPortfolio.toFixed(2) : '100.00',
                TotalReturn_Reinvestment: summaryJson.finalPortfolio && summaryJson.initialPortfolio
                    ? (((summaryJson.finalPortfolio / summaryJson.initialPortfolio) - 1) * 100).toFixed(2)
                    : '0.00',
                FinalPortfolio_NoReinvestment: summaryJson.simpleFinalPortfolio ? summaryJson.simpleFinalPortfolio.toFixed(2) : '100.00',
                TotalReturn_NoReinvestment: summaryJson.simpleReturnPercent ? summaryJson.simpleReturnPercent.toFixed(2) : '0.00',
                MaxDrawdown_Reinvestment: summaryJson.maxDrawdownPct ? summaryJson.maxDrawdownPct.toFixed(2) : '0.00',
                MaxDrawdown_NoReinvestment: summaryJson.maxDrawdownPctNoReinvestment ? summaryJson.maxDrawdownPctNoReinvestment.toFixed(2) : '0.00',
                StdDevReturns: summaryJson.stdDevReturnsPct ? summaryJson.stdDevReturnsPct.toFixed(2) : '0.00',
                ...
                AnnualTWR_Theoretical: summaryJson.twrAnnualPct ? summaryJson.twrAnnualPct.toFixed(2) : '0.00',
                AnnualTWR_Realistic: summaryJson.twrAnnualDrawdownAdjusted ? summaryJson.twrAnnualDrawdownAdjusted.toFixed(2) : '0.00',
                CalmarRatio: summaryJson.calmarRatio ? summaryJson.calmarRatio.toFixed(2) : '0.00',
                RiskAdjustedScore: summaryJson.riskAdjustedScore ? summaryJson.riskAdjustedScore.toFixed(2) : '0.00',
                AvgPerTradeMaxDrawdown: summaryJson.avgPerTradeMaxDrawdown ? summaryJson.avgPerTradeMaxDrawdown.toFixed(2) : '0.00',
                MaxPerTradeMaxDrawdown: summaryJson.maxPerTradeMaxDrawdown ? summaryJson.maxPerTradeMaxDrawdown.toFixed(2) : '0.00',
                AvgRiskRatio: summaryJson.avgRiskRatio ? summaryJson.avgRiskRatio.toFixed(2) : '0.00',
                MedianRiskRatio: summaryJson.medianRiskRatio ? summaryJson.medianRiskRatio.toFixed(2) : '0.00',
            });
        }
        catch (error) {
    ...
    summaries.sort((a, b) => parseFloat(b.FinalPortfolio_Reinvestment) - parseFloat(a.FinalPortfolio_Reinvestment));
    // Write to CSV
    const summaryCsvPath = path.join(OUTPUT_DIR, 'all_callers_summary.csv');
    await new Promise((resolve, reject) => {
        (0, csv_stringify_1.stringify)(summaries, { header: true }, (err, output) => {
            if (err)
                reject(err);
            else {
...
//# sourceMappingURL=test-tenkan-kijun-remaining-period-by-caller.js.map