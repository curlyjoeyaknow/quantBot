...
# Fix ClickHouse include issues - handles <yandex> root tag
# Run with: sudo ./scripts/system/fix-clickhouse-includes-v3.sh
...
echo -e "${BLUE}=== Fixing ClickHouse Includes (v3) ===${NC}"
echo ""
...
# Check if root tag is <yandex> or <clickhouse>
ROOT_TAG=$(head -5 "$CFG" | grep -oE '<(yandex|clickhouse)' | head -1 | sed 's/<//')
echo "  Root tag detected: $ROOT_TAG"

# Remove any existing include_from lines
sed -i '/<include_from>/d' "$CFG"
sed -i '/<\/include_from>/d' "$CFG"

# Add include_from after the root tag using Python (most reliable)
python3 <<PYTHON
import re
...
# Find root tag (<yandex> or <clickhouse>) and add include_from after it
# Match the opening tag with any attributes
pattern = r'(<yandex[^>]*>|<clickhouse[^>]*>)'
...
if grep -q "include_from.*config.d" "$CFG"; then
  ...
  echo "  Showing first 10 lines of config:"
  head -10 "$CFG"
  exit 1
fi
