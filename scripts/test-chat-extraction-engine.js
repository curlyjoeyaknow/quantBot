...
async function main() {
    ...
    // Limit to last 200 messages for faster testing (remove this limit for full test)
    const messagesToProcess = messages.slice(-200);
    console.log(`📊 Processing last ${messagesToProcess.length} messages (to speed up test)\n`);
    // Process messages in batches (original + next 2 bot messages)
    for (let i = 0; i < messagesToProcess.length; i++) {
        const originalMsg = messagesToProcess[i];
        // Skip if original is a bot
        if (extractionEngine.isBot(originalMsg.sender)) {
            continue;
        }
        // Find next 2 bot messages (search in full messages array, not just processed slice)
        const botMessages = [];
        const originalIndex = messages.indexOf(originalMsg);
        for (let j = originalIndex + 1; j < Math.min(messages.length, originalIndex + 10); j++) {
        ...
        // Extract tokens
        try {
            const extracted = await extractionEngine.extract(originalMsg, botMessages, {
            ...
            utils_1.logger.warn('Error extracting from message', { error: error.message, sender: originalMsg.sender });
        }
    }
    // Summary
    console.log(`\n${'='.repeat(80)}`);
