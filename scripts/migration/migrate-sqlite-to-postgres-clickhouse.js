...
class DatabaseMigrator {
     ...
     */
    async migrateCallerAlerts(dbPath) {
      ...
      `);
            ...
            if (!this.dryRun) {
                ...
                for (const alert of alerts) {
                    try {
                        ...
                        const tokenResult = await client.query('SELECT id FROM tokens WHERE chain = $1 AND address = $2', [alert.chain || 'solana', alert.token_address]);
                        // Get caller_id
                        const callerResult = await client.query('SELECT id FROM callers WHERE source = $1 AND handle = $2', ['legacy', alert.caller_name]);
            ...
            `, [
                        ...
                        logger.error('Failed to migrate alert', error, { alertId: alert.id });
                    }
                }
            ...
            logger.error('Failed to migrate caller alerts', error);
            throw error;
        }
        finally {
            client.release();
    ...
    async migrateQuantbot(dbPath) {
        ...
        try {
            ...
            const tokens = await all('SELECT * FROM tokens');
          ...
          `, [
            ...
            const strategies = await all('SELECT * FROM strategies');
            ...
            `, [
                        ...
                        logger.error('Failed to migrate strategy', error, { strategyId: strategy.id });
                    }
                }
                await client.query('COMMIT');
            }
            ...
            const runs = await all('SELECT * FROM simulation_runs ORDER BY id ASC');
            ...
            if (!this.dryRun && runs.length > 0) {
                ...
                for (const run of runs) {
                    try {
                        // Get or create token
                        const tokenResult = await client.query('SELECT id FROM tokens WHERE chain = $1 AND address = $2', [run.chain || 'solana', run.mint]);
                        let tokenId;
              ...
              `, [run.chain || 'solana', run.mint, run.token_symbol, run.token_name]);
                        ...
                        if (run.strategy_name) {
                            const strategyResult = await client.query('SELECT id FROM strategies WHERE name = $1', [run.strategy_name]);
            ...
            `, [
                        ...
                        logger.error('Failed to migrate simulation run', error, { runId: run.id });
                    }
                }
                await client.query('COMMIT');
            }
            ...
            await client.query('ROLLBACK').catch(() => { });
            logger.error('Failed to migrate quantbot.db', error);
            throw error;
        }
        finally {
            client.release();
     ...
     */
    async migrateStrategyResults(dbPath) {
        const db = await this.openSqliteDb(dbPath);
        if (!db)
            return;
        const all = (0, util_1.promisify)(db.all.bind(db));
        const client = await this.pgPool.connect();
        try {
            const results = await all('SELECT * FROM strategy_results');
            ...
            if (!this.dryRun && results.length > 0) {
                ...
                for (const result of results) {
                    try {
                        // Get token_id
                        const tokenResult = await client.query('SELECT id FROM tokens WHERE chain = $1 AND address = $2', [result.chain || 'solana', result.token_address] // NEVER lowercase!
                        );
                        if (tokenResult.rows.length === 0) {
            ...
            `, [tokenId, result.alert_timestamp + '%']);
              ...
              `, [
                        ...
                        logger.error('Failed to migrate strategy result', error, {
                            resultId: result.id
                        });
                    }
                }
            ...
            await client.query('ROLLBACK').catch(() => { });
            logger.error('Failed to migrate strategy results', error);
            throw error;
        }
        finally {
            client.release();
     ...
     */
    async migrateDashboardMetrics(dbPath) {
        const db = await this.openSqliteDb(dbPath);
        if (!db)
            return;
        const all = (0, util_1.promisify)(db.all.bind(db));
        ...
        `);
            ...
            const metrics = await all('SELECT * FROM dashboard_metrics ORDER BY computed_at ASC');
          ...
          `, [
            ...
            await client.query('ROLLBACK').catch(() => { });
            logger.error('Failed to migrate dashboard metrics', error);
            throw error;
        }
        finally {
            client.release();
     ...
     */
    async migrateSimulationEvents(dbPath) {
        const db = await this.openSqliteDb(dbPath);
        if (!db)
            return;
        if (!this.clickhouse) {
      ...
      `);
            ...
            logger.error('Failed to migrate simulation events', error);
            this.stats.push({
                database: 'quantbot',
                table: 'simulation_events (ClickHouse)',
                rows: 0,
     ...
     */
    async migrateUnifiedCalls(dbPath) {
        const db = await this.openSqliteDb(dbPath);
        if (!db)
            return;
        const all = (0, util_1.promisify)(db.all.bind(db));
        const client = await this.pgPool.connect();
        try {
            const calls = await all('SELECT * FROM unified_calls ORDER BY alert_timestamp ASC');
            ...
            `, [call.chain || 'solana', call.token_address, call.token_symbol]); // NEVER lowercase!
                        // Get IDs
                        const tokenResult = await client.query('SELECT id FROM tokens WHERE chain = $1 AND address = $2', [call.chain || 'solana', call.token_address] // NEVER lowercase!
                        );
                        const callerResult = await client.query('SELECT id FROM callers WHERE source = $1 AND handle = $2', [call.source || 'legacy', call.caller_name]);
                        if (tokenResult.rows.length === 0 || callerResult.rows.length === 0) {
                            continue;
                        }
                        const tokenId = tokenResult.rows[0].id;
            ...
            `, [
              ...
              `, [
                        ...
                        logger.error('Failed to migrate unified call', error, { callId: call.id });
                    }
                }
                await client.query('COMMIT');
            }
            ...
            await client.query('ROLLBACK').catch(() => { });
            logger.error('Failed to migrate unified calls', error);
            throw error;
        }
        finally {
            client.release();
...
//# sourceMappingURL=migrate-sqlite-to-postgres-clickhouse.js.map