...
async function main(): Promise<void> {
  ...
  logger.info('This system listens to your personal Telegram chat for manually forwarded messages from Brook\'s channel.');

  // Validate required environment variables
  if (!PERSONAL_CHAT_ID) {
    logger.error('PERSONAL_CHAT_ID environment variable is required');
    logger.error('This is your personal Telegram chat ID where you forward messages from Brook\'s channel');
    process.exit(1);
  }
  ...
  try {
    ...
    logger.info('To use: Forward messages from Brook\'s channel to your personal chat, and the bot will automatically process them.');
  } catch (error) {
