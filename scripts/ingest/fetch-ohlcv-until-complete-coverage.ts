...
async function queryAllAlerts(duckdbPath: string): Promise<Alert[]> {
  ...
  // Query in batches (max limit is 10000)
  ...
  while (hasMore) {
    const result = await duckdbStorage.queryCalls(duckdbPath, batchSize, false);

    if (!result.success || !result.calls) {
      // Check if it's a lock error and retry
      if (result.error && result.error.includes('lock') && retries < maxRetries) {
        retries++;
        logger.warn(
          `Database locked, retrying in ${retryDelay / 1000}s (attempt ${retries}/${maxRetries})...`,
          {
            error: result.error,
          }
        );
    ...
    if (result.calls.length < batchSize) {
      hasMore = false;
    } else {
      offset += batchSize;
      // Note: queryCalls doesn't support offset, so we'll stop after first batch
      // In practice, this should get most alerts. For full coverage, we'd need to
      // modify the Python script to support pagination.
      hasMore = false;
      logger.warn(
        `Query limit reached. Got ${alerts.length} alerts. Consider pagination for full coverage.`
      );
    }
  }
 ...
 */
async function checkCoverage(alert: Alert, interval: '1s' | '1m' | '5m'): Promise<CoverageStatus> {
...
async function main() {
  ...
  while (true) {
    ...
    logger.info('Coverage Statistics:', {
      ...
      byInterval: {
        '1s': {
          ...
          percent: ((stats.byInterval['1s'].complete / stats.byInterval['1s'].total) * 100).toFixed(
            2
          ),
        },
        '1m': {
          ...
          percent: ((stats.byInterval['1m'].complete / stats.byInterval['1m'].total) * 100).toFixed(
            2
          ),
        },
        '5m': {
          ...
          percent: ((stats.byInterval['5m'].complete / stats.byInterval['5m'].total) * 100).toFixed(
            2
          ),
        },
      },
    ...
    if (coveragePercent >= TARGET_COVERAGE_PERCENT) {
      logger.info(
        `✅ Target coverage reached: ${coveragePercent.toFixed(2)}% >= ${TARGET_COVERAGE_PERCENT}%`
      );
      break;
    }
    ...
    for (let i = 0; i < needsFetch.length; i++) {
      ...
      const alert = alerts.find(
        (a) => a.mint === status.mint && a.alertTime.equals(status.alertTime)
      );
      ...
      if (result.success) {
        fetched++;
        logger.debug(
          `Fetched ${result.candlesFetched} candles for ${alert.mint} (${status.interval})`
        );
      } else {
        failed++;
  ...
  logger.info('Final Coverage:', {
    ...
    byInterval: {
      '1s': {
        ...
        percent: (
          (finalStats.byInterval['1s'].complete / finalStats.byInterval['1s'].total) *
          100
        ).toFixed(2),
      },
      '1m': {
        ...
        percent: (
          (finalStats.byInterval['1m'].complete / finalStats.byInterval['1m'].total) *
          100
        ).toFixed(2),
      },
      '5m': {
        ...
        percent: (
          (finalStats.byInterval['5m'].complete / finalStats.byInterval['5m'].total) *
          100
        ).toFixed(2),
      },
    },
