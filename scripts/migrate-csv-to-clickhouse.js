...
function parseCacheFilename(filename) {
    ...
    const startTime = luxon_1.DateTime.fromSQL(startDateStr, { zone: 'utc' });
    const endTime = luxon_1.DateTime.fromSQL(endDateStr, { zone: 'utc' });
 ...
 */
function detectInterval(candles) {
    if (candles.length < 2)
        return '5m';
...
async function main() {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            ...
            const exists = await (0, storage_1.hasCandles)(fileInfo.tokenAddress, fileInfo.chain, fileInfo.startTime, fileInfo.endTime);
            if (exists) {
            ...
            for (let j = 0; j < candles.length; j += BATCH_SIZE) {
                ...
                await (0, storage_1.insertCandles)(fileInfo.tokenAddress, fileInfo.chain, batch, interval);
            }
...
main();
//# sourceMappingURL=migrate-csv-to-clickhouse.js.map