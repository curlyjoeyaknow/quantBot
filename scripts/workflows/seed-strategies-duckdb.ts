 ...
 */
function convertStrategyConfig(
  preset: StrategyConfig,
  presetName: string
): {
  name: string;
  version: string;
  ...
  if (preset.stopLoss) {
    ...
    if (preset.stopLoss.trailing && preset.stopLoss.trailing !== 'none') {
      parts.push(
        `Trailing stop: ${preset.stopLoss.trailingPercent ? preset.stopLoss.trailingPercent * 100 : 20}%`
      );
    }
  }
  if (preset.holdHours) {
  ...
  if (preset.entry?.initialEntry && preset.entry.initialEntry !== 'none') {
    parts.push(`Entry: ${Math.abs((preset.entry.initialEntry as number) * 100)}% dip`);
  }
