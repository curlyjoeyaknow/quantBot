...
async function main() {
  ...
  console.log(
    `  Credit Usage: ${creditStats.creditsUsed.toLocaleString()}/${creditStats.totalCredits.toLocaleString()} (${creditStats.percentage.toFixed(2)}%)`
  );
  ...
  keyUsage.forEach((usage, index) => {
    ...
    console.log(
      `  Key ${index + 1}: ${status} ${usage.requestsUsed} requests, ${usage.estimatedCreditsUsed} credits, ${usage.consecutiveFailures} consecutive failures`
    );
  });
