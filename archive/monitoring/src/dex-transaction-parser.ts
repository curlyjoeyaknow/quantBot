          ...
          ? {
              writable:
                meta.loadedWritableAddresses?.map(
                  (address: string) => new PublicKey(Buffer.from(address, 'base64'))
                ) || [],
              readonly:
...
export class DexTransactionParser {
  ...
  private parseRaydiumTransaction(txn: VersionedTransactionResponse): SwapEvent | null {
    try {
      ...
      const solPre =
        preBalances.find((b: any) => b.mint === this.SOL_MINT)?.uiTokenAmount?.uiAmount || 0;
      ...
      const tokenPre =
        preBalances.find((b: any) => b.mint === tokenMint)?.uiTokenAmount?.uiAmount || 0;
      const tokenPost =
        postBalances.find((b: any) => b.mint === tokenMint)?.uiTokenAmount?.uiAmount || 0;

      const amountIn = type === 'Buy' ? Math.abs(solPre - solPost) : Math.abs(tokenPre - tokenPost);
      const amountOut =
        type === 'Buy' ? Math.abs(tokenPost - tokenPre) : Math.abs(solPost - solPre);
  ...
  private parseOrcaTransaction(txn: VersionedTransactionResponse): SwapEvent | null {
    try {
      ...
      const solPre =
        preBalances.find((b: any) => b.mint === this.SOL_MINT)?.uiTokenAmount?.uiAmount || 0;
      const solPost =
        postBalances.find((b: any) => b.mint === this.SOL_MINT)?.uiTokenAmount?.uiAmount || 0;
