...
import type { Candle, Strategy, StopLossConfig } from '@quantbot/core';
import type { Telegraf } from 'telegraf';
import type { IchimokuData, IchimokuSignal } from '@quantbot/simulation';
import { simulateStrategy } from '@quantbot/simulation';
import { calculateIchimoku, detectIchimokuSignals, formatIchimokuData } from '@quantbot/simulation';
...
interface CAMonitor {
  ...
  strategy: Strategy[]; // Array of profit targets per-strategy
  stopLossConfig: StopLossConfig; // Stop loss config
  ...
  ichimokuLeadingSpans?: {
    // Ichimoku leading spans for price alerts
    senkouA: number;
    senkouB: number;
    cloudTop: number;
    cloudBottom: number;
...
class HeliusMonitor {
  ...
  private bot: Telegraf; // Telegram bot instance
  ...
  private async loadActiveCAs(): Promise<void> {
    try {
      ...
      logger.info(
        'No auto-loaded CA tracking entries. Only manually flagged tokens will be monitored.'
      );
    } catch (error) {
  ...
  private connect(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        logger.error('Failed to create Helius WebSocket connection', err);
        settleReject(err);
        return;
      }
      ...
      ws.on('error', (error) => {
        const err = error as Error;
        logger.error('Helius WebSocket error', err);
  ...
  private subscribeToAllTrackedCAs(): void {
    ...
    const subscriptions = Array.from(this.activeCAs.keys()).map((key) => {
      ...
      return {
        ...
        id: key,
        method: 'subscribe',
        params: [`price-updates-${chain}`, { accounts: [mint] }],
      };
    });
  ...
  private async checkAlertsAndNotify(
    ...
    for (const alert of alerts) {
      try {
        ...
        await saveAlertSent(ca.id, alert.type, currentPrice, Math.floor(Date.now() / 1000));
   ...
   * Check for Ichimoku leading span crosses (immediate price alerts)
   */
  private async checkIchimokuLeadingSpanCrosses(
    ca: CAMonitor,
    currentPrice: number,
    timestamp: number
  ): Promise<void> {
    ...
    if (
      (lastPrice <= senkouA && currentPrice > senkouA) ||
      (lastPrice >= senkouA && currentPrice < senkouA)
    ) {
      ...
      const message =
        `${emoji} **Ichimoku Leading Span Cross!**\n\n` +
        `🪙 **${ca.tokenName}** (${ca.tokenSymbol})\n` +
    ...
    if (
      (lastPrice <= senkouB && currentPrice > senkouB) ||
      (lastPrice >= senkouB && currentPrice < senkouB)
    ) {
    ...
    if (
      (lastPrice <= cloudBottom && currentPrice > cloudTop) ||
      (lastPrice >= cloudTop && currentPrice < cloudBottom)
    ) {
      const direction = currentPrice > cloudTop ? 'above' : 'below';
      const emoji = currentPrice > cloudTop ? '🟢' : '🔴';

      const message =
        `${emoji} **Ichimoku Cloud Cross!**\n\n` +
        `🪙 **${ca.tokenName}** (${ca.tokenSymbol})\n` +
   ...
   */
  private async updateCandlesForIchimoku(
    ca: CAMonitor,
    price: number,
    timestamp: number
  ): Promise<void> {
    ...
    if (now - lastUpdate >= updateInterval) {
      try {
        ...
        if (isNearIchimokuLine) {
          logger.debug('Fast candle update', {
            tokenName: ca.tokenName,
            reason: 'near Ichimoku lines',
          });
        }
      } catch (error) {
        logger.error('Error updating candles from Birdeye', error as Error, {
          tokenName: ca.tokenName,
        });
      }
    }
  ...
  private async requestFrequentPriceUpdates(ca: CAMonitor): Promise<void> {
    ...
    if (now - lastRequest >= requestInterval) {
      try {
        ...
        logger.error('Error requesting frequent price updates', error as Error, {
          tokenName: ca.tokenName,
        });
      }
    }
      ...
      if (this.ws && this.ws.readyState === WebSocket.OPEN && this.activeCAs.size > 0) {
        ...
        for (const [key, ca] of this.activeCAs) {
          try {
            ...
            logger.error('Error requesting periodic updates', error as Error, {
              tokenName: ca.tokenName,
            });
          }
        }
    ...
    try {
      ...
      if (newCandles.length >= 52) {
        ...
        if (newIchimoku) {
          ...
          logger.debug('Updated candles and recalculated Ichimoku', {
            tokenName: ca.tokenName,
   ...
   */
  private async checkIchimokuSignals(
    ca: CAMonitor,
    currentPrice: number,
    timestamp: number
  ): Promise<void> {
  ...
  private async sendIchimokuAlert(
    ...
    const message =
      `${signalEmoji} **Ichimoku Signal Detected!**\n\n` +
      `🪙 **${ca.tokenName}** (${ca.tokenSymbol})\n` +
    ...
    try {
      ...
      logger.info('Sent Ichimoku alert', {
  ...
  private handleReconnect(): void {
    ...
    setTimeout(() => {
      this.connect().catch((error) => logger.error('Reconnect attempt failed', error as Error));
    }, delayMs);
  }
  ...
  private scheduleHourlySummaries(): void {
    setInterval(
      () => {
        this.sendHourlySummary().catch((e) => logger.error('Error in hourly summary', e as Error));
      },
      60 * 60 * 1000
    ); // Every hour
  }
  ...
  public async addCATracking(caData: any): Promise<void> {
    ...
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      const subscription = {
        ...
        id: key,
        method: 'subscribe',
        params: [`price-updates-${caData.chain}`, { accounts: [caData.mint] }],
      };
      this.ws.send(JSON.stringify(subscription));
    }
  }
  ...
  public async addCATrackingWithCandles(caData: any): Promise<void> {
    ...
    logger.info('Added CA tracking with historical candles', {
      candleCount: caData.historicalCandles?.length || 0,
      tokenName: caData.tokenName,
    });
  }
}
