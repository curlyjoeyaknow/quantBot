...
import { eventBus, EventFactory } from '@quantbot/utils';
...
export class CAMonitoringService extends EventEmitter {
  ...
  public removeCAMonitor(chain: string, mint: string): void {
    ...
    if (ca) {
      ...
      logger.info('Removed CA monitor', {
        tokenName: ca.tokenName,
        tokenSymbol: ca.tokenSymbol,
        mint: ca.mint,
      });
  ...
  private async checkIchimokuSignals(
    ...
    try {
      ...
      const previousIchimoku =
        previousIndex >= 51 ? calculateIchimoku(ca.candles, previousIndex) : null;
   ...
   * Removes all EventEmitter listeners and clears active CAs to prevent memory leaks
   */
  public shutdown(): void {
    // Remove all EventEmitter listeners
    this.removeAllListeners();

    // Clear active CAs map
    this.activeCAs.clear();

    logger.info('CA Monitoring Service shut down');
  }
}
