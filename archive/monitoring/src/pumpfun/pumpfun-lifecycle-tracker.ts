...
import {
  ...
  PUMP_FUN_PROGRAM_ID,
  upsertPumpfunToken,
  markPumpfunGraduated,
  type PumpfunTokenRecord,
  type TrackedToken,
} from '@quantbot/utils';
...
export class PumpfunLifecycleTracker {
  ...
  private async connectGrpc(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        'Failed to connect to Yellowstone gRPC. Falling back to Helius WebSocket.',
        error as Error
      );
      await this.connectWebSocket();
    }
  ...
  private buildGrpcSubscription(): any {
    const commitment =
      (CommitmentLevel && CommitmentLevel.PROCESSED) !== undefined ? CommitmentLevel.PROCESSED : 0;
  ...
  private handleGrpcMessage(message: any): void {
    ...
    const signature: string | undefined =
      txWrapper.signature || txWrapper.transaction?.signatures?.[0];
  ...
  private async processLaunch(signature: string, timestamp: number, tx?: any): Promise<void> {
    try {
      ...
      const tokenSymbol =
        metadata && typeof metadata.symbol === 'string' ? metadata.symbol : undefined;
  ...
  private extractMetadata(tx: any): Record<string, unknown> | null {
    ...
    if (tx?.meta?.postTokenBalances) {
      const balance = tx.meta.postTokenBalances.find(
        (entry: any) => entry?.mint && entry.mint !== 'So11111111111111111111111111111111111111112'
      );
      if (balance?.owner) {
