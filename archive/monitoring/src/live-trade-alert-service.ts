...
const ALERT_GROUP_IDS =
  process.env.LIVE_TRADE_ALERT_GROUP_IDS?.split(',')
...
export class LiveTradeAlertService extends EventEmitter {
  ...
  private telegramBot: import('telegraf').Telegraf | null = null;
              ...
              if (message.error) {
                ...
                if (this.authPromise) {
                  this.authPromise.reject(
                    new Error(
                      message.error.message || message.error.code || 'Authentication failed'
                    )
                  );
  ...
  private startRPCPolling(): void {
    this.rpcPollInterval = setInterval(async () => {
      ...
      const ethBscTokens = Array.from(this.activeMonitors.values()).filter(
        (m) =>
          m.chain === 'ethereum' || m.chain === 'bsc' || m.chain === 'eth' || m.chain === 'binance'
      );
  ...
  private async updatePricesBatch(monitors: TokenMonitor[]): Promise<void> {
    // Group by chain
    const solanaTokens = monitors.filter((m) => m.chain === 'solana').map((m) => m.tokenAddress);
  ...
  private formatEntryAlertMessage(alert: EntryAlert): string {
    const entryEmoji =
      alert.entryType === 'initial' ? '📉' : alert.entryType === 'trailing' ? '📈' : '🎯';
