...
const IDL_PATH = path.join(
  __dirname,
  '../../examples/solana-defi/PumpFun/Typescript/stream_pump_fun_bonding_curve_progress_accounts/Idl/pump_0.1.0.json'
);
  ...
  if (!accountCoder) {
    try {
      ...
      try {
        ...
        throw new Error(
          `Failed to load Pump.fun IDL from ${IDL_PATH} or ${fallbackPath}: ${error}`
        );
      }
    }
  }
 ...
 */
export function calculatePriceFromBondingCurve(
  account: BondingCurveAccount,
  solPriceUsd: number = 150
): number {
  // Get reserves - prefer real_reserves, fallback to virtual_reserves
  const solReserves =
    account.real_sol_reserves || account.virtual_sol_reserves || account.sol_reserves;
  const tokenReserves =
    account.real_token_reserves || account.virtual_token_reserves || account.token_reserves;
  ...
  const tokenStr =
    typeof tokenReserves === 'string' ? tokenReserves : (tokenReserves as BN).toString();
