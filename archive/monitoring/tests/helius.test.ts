...
const mockSimulateStrategy = simulateStrategy as unknown as jest.MockedFunction<
  typeof simulateStrategy
>;
const mockCalculateIchimoku = calculateIchimoku as unknown as jest.MockedFunction<
  typeof calculateIchimoku
>;
const mockDetectIchimokuSignals = detectIchimokuSignals as unknown as jest.MockedFunction<
  typeof detectIchimokuSignals
>;
const mockFormatIchimokuData = formatIchimokuData as unknown as jest.MockedFunction<
  typeof formatIchimokuData
>;
...
describe('HeliusMonitor', () => {
  ...
  beforeEach(() => {
    ...
    mockWebSocket = {
      get readyState() {
        return (WebSocket as any).OPEN;
      },
      send: vi.fn(),
      close: vi.fn(),
      on: vi.fn().mockImplementation(() => mockWebSocket), // For chaining
    } as any;

    (MockWebSocket as unknown as { mockImplementation: any }).mockImplementation(
      () => mockWebSocket
    );
  ...
  describe('WebSocket Connection', () => {
    it('should connect to WebSocket', async () => {
      ...
      expect(MockWebSocket).toHaveBeenCalledWith(
        expect.stringContaining('wss://mainnet.helius-rpc.com')
      );
    });
    ...
    it('should handle connection errors', async () => {
      ...
      // Ensure monitor.start() throws if WebSocket constructor throws
      try {
        await monitor.start();
        // If no error is thrown, this is a failure
        fail('Expected monitor.start() to throw "Connection failed"');
      } catch (err: unknown) {
    ...
    it('should handle authentication errors', async () => {
      ...
      await expect(monitor.start()).rejects.toThrow('401 Unauthorized');
