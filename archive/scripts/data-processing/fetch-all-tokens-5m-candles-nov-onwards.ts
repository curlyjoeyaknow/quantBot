...
const NOV_1_2025 = DateTime.fromISO('2025-11-01T00:00:00Z');
const END_TIME = DateTime.utc();
...
> {
  return new Promise((resolve, reject) => {
    ...
    db.all(query, [NOV_1_2025.toISO()], (err, rows: any[]) => {
      db.close();
      if (err) {
        reject(err);
        return;
...
async function main() {
  ...
  try {
    ...
    const noDataTokens: TokenResult[] = [];
    let processed = 0;
    ...
    for (let i = 0; i < tokens.length; i += BATCH_SIZE) {
      ...
      const batchPromises = batch.map(async (token) => {
        const result = await fetchCandlesForToken(
          token.tokenAddress,
          token.chain,
          token.firstAlertTime,
          engine
        );
    ...
    console.error('\n❌ Error:', error.message);
    logger.error('Failed to fetch candles', error as Error);
    process.exit(1);
  }
}
