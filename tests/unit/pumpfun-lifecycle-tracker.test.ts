...
import { PumpfunLifecycleTracker } from '../../src/services/pumpfun/pumpfun-lifecycle-tracker';
import { derivePumpfunBondingCurve, PUMP_FUN_PROGRAM_ID } from '../../src/utils/pumpfun';
import * as databaseModule from '../../src/utils/database';
import { heliusBackfillService } from '../../src/services/backfill/helius-backfill-service';
import { heliusStreamRecorder } from '../../src/services/stream/helius-recorder';

describe('PumpfunLifecycleTracker helpers', () => {
  const tracker = new PumpfunLifecycleTracker();
  const extractPrimaryMint = (tracker as any).extractPrimaryMint.bind(tracker) as (tx: any) => string | null;
  const extractMetadata = (tracker as any).extractMetadata.bind(tracker) as (tx: any) => Record<string, unknown> | null;
...
describe('PumpfunLifecycleTracker lifecycle flows', () => {
  ...
  it('handleLogsNotification delegates to launch handler when log matches create instruction', async () => {
    const launchSpy = vi
      .spyOn(tracker as any, 'processLaunch')
  ...
  it('handleGrpcMessage delegates to graduation handler with parsed transaction payload', async () => {
    const gradSpy = vi
      .spyOn(tracker as any, 'processGraduation')
      .mockResolvedValue(undefined);
