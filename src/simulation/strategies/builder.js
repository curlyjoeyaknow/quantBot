...
exports.buildStopLossConfig = buildStopLossConfig;
exports.buildEntryConfig = buildEntryConfig;
exports.buildReEntryConfig = buildReEntryConfig;
exports.buildFromPreset = buildFromPreset;
...
function buildStrategy(config) {
    ...
    return config.profitTargets.map((tp) => ({
        target: tp.target,
        percent: tp.percent,
    }));
}
 ...
 */
function buildStopLossConfig(config) {
    if (!config.stopLoss) {
        return undefined;
    }
    return {
 ...
 */
function buildEntryConfig(config) {
    if (!config.entry) {
        return undefined;
    }
    return {
 ...
 */
function buildReEntryConfig(config) {
    if (!config.reEntry) {
        return undefined;
    }
    return {
...
function buildFromPreset(presetName) {
    return (0, presets_1.getPreset)(presetName);
}
/**
