...
class InfluxDBOHLCVClient {
    ...
    async initialize() {
        try {
            logger_1.logger.info('Initializing InfluxDB...');
            ...
            if (!existingBucket) {
                logger_1.logger.info('Creating bucket', { bucket: this.bucket });
                ...
                logger_1.logger.info('Bucket created successfully', { bucket: this.bucket });
            }
            else {
                logger_1.logger.info('Bucket already exists', { bucket: this.bucket });
            }
            // Test write and read
            await this.testConnection();
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize InfluxDB', error);
            throw error;
        }
    }
    /**
    ...
    `;
        ...
        logger_1.logger.info('InfluxDB connection test successful');
    }
    /**
    ...
    async writeOHLCVData(tokenAddress, tokenSymbol, chain, data) {
        try {
            ...
            await this.writeApi.flush();
            logger_1.logger.info('Written OHLCV records', { recordCount: data.length, tokenSymbol, tokenAddress });
        }
        catch (error) {
            logger_1.logger.error('Failed to write OHLCV data', error, { tokenAddress });
            throw error;
        }
    }
    /**
    ...
    async getOHLCVData(tokenAddress, startTime, endTime, interval = '1m') {
      ...
      `;
            ...
            logger_1.logger.error('Failed to query OHLCV data', error, { tokenAddress });
            throw error;
        }
    }
    /**
    ...
    async getTokenRecordCount(tokenAddress) {
      ...
      `;
            ...
            logger_1.logger.error('Failed to get record count', error, { tokenAddress });
            return 0;
        }
    }
    /**
    ...
    async close() {
        ...
        logger_1.logger.info('InfluxDB connection closed');
    }
}
