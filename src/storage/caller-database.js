...
class CallerDatabase {
    constructor(dbPath = process.env.CALLER_DB_PATH || './caller_alerts.db') {
        this.dbPath = dbPath;
    ...
    async initDatabase() {
      ...
      `);
            logger_1.logger.info('Caller database initialized successfully');
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize caller database', error);
            throw error;
        }
    }
    /**
    ...
    async addCallerAlert(alert) {
      ...
      `, [
            ...
            logger_1.logger.error('Failed to add caller alert', error, { callerName: alert.callerName, tokenAddress: alert.tokenAddress });
            throw error;
        }
    }
    /**
            ...
            for (const alert of alerts) {
                try {
                    ...
                    if (!errorMessage.includes('UNIQUE constraint failed')) {
                        logger_1.logger.warn('Failed to add alert', { callerName: alert.callerName, error: errorMessage });
                    }
                }
            }
            stmt.finalize();
            logger_1.logger.info('Added caller alerts', { addedCount, totalCount: alerts.length });
            return addedCount;
        }
        catch (error) {
            logger_1.logger.error('Failed to batch add caller alerts', error);
            throw error;
        }
    }
    /**
    ...
    async getCallerAlerts(callerName, limit) {
        ...
        try {
            ...
            logger_1.logger.error('Failed to get caller alerts', error, { callerName });
            throw error;
        }
    }
    /**
    ...
    async getCallerAlertsInRange(callerName, startTime, endTime) {
      ...
      `, [callerName, startTime.toISOString(), endTime.toISOString()]);
            ...
            logger_1.logger.error('Failed to get caller alerts in range', error, { callerName, startTime: startTime?.toISOString(), endTime: endTime?.toISOString() });
            throw error;
        }
    }
    /**
    ...
    async getAllCallers() {
        ...
        try {
            ...
            logger_1.logger.error('Failed to get all callers', error);
            throw error;
        }
    }
    /**
    ...
    async getCallerStats(callerName) {
      ...
      `, [callerName]);
            ...
            logger_1.logger.error('Failed to get caller stats', error, { callerName });
            throw error;
        }
    }
    /**
    ...
    async getAllCallerStats() {
      ...
      `);
            ...
            logger_1.logger.error('Failed to get all caller stats', error);
            throw error;
        }
    }
    /**
    ...
    async getCallerTokens(callerName) {
      ...
      `, [callerName]);
            ...
            logger_1.logger.error('Failed to get caller tokens', error, { callerName });
            throw error;
        }
    }
    /**
    ...
    async updateCallerSuccessRate(callerName, successRate) {
      ...
      `, [successRate, callerName]);
            logger_1.logger.info('Updated caller success rate', { callerName, successRate: successRate.toFixed(2) });
        }
        catch (error) {
            logger_1.logger.error('Failed to update caller success rate', error, { callerName });
            throw error;
        }
    }
    /**
        ...
        return new Promise((resolve, reject) => {
            this.db.close((err) => {
                if (err)
                    ...
                    logger_1.logger.info('Caller database connection closed');
                    resolve();
                }
            });
        });
