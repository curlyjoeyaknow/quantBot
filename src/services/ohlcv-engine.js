...
exports.OHLCVEngine = void 0;
exports.getOHLCVEngine = getOHLCVEngine;
...
class OHLCVEngine {
    ...
    async initialize() {
        if (this.clickHouseEnabled) {
            try {
                ...
                logger_1.logger.info('OHLCV Engine: ClickHouse initialized');
            }
            catch (error) {
                logger_1.logger.warn('OHLCV Engine: ClickHouse initialization failed', { error: error.message });
     ...
     */
    async fetch(tokenAddress, startTime, endTime, chain = 'solana', options = {}) {
        const { cacheOnly = false, ensureIngestion = true, alertTime, interval = '5m' } = options;
        // Step 1: Check ClickHouse cache first (if enabled)
        if (this.clickHouseEnabled) {
            try {
                const cachedCandles = await (0, clickhouse_client_1.queryCandles)(tokenAddress, chain, startTime, endTime);
                if (cachedCandles.length > 0) {
                    logger_1.logger.debug(`OHLCV Engine: Using ClickHouse cache for ${tokenAddress.substring(0, 20)}... (${cachedCandles.length} candles)`);
                    return {
                ...
                logger_1.logger.warn('OHLCV Engine: ClickHouse query failed', { error: error.message, tokenAddress: tokenAddress.substring(0, 20) });
            }
        }
        // Step 2: If cache-only mode, return empty (no API calls)
        if (cacheOnly) {
            logger_1.logger.debug(`OHLCV Engine: Cache-only mode, no candles found for ${tokenAddress.substring(0, 20)}...`);
            return {
                candles: [],
                fromCache: false,
        ...
        try {
            const candles = await (0, candles_1.fetchHybridCandles)(tokenAddress, startTime, endTime, chain, alertTime);
            ...
            if (this.clickHouseEnabled) {
                try {
                    const existingCandles = await (0, clickhouse_client_1.queryCandles)(tokenAddress, chain, startTime, endTime);
                ...
                try {
                    ...
                    if (alertTime && candles.length > 1) {
                        ...
                        for (const candle of candles) {
                            const candleTime = luxon_1.DateTime.fromSeconds(candle.timestamp);
                        ...
                        if (candles5m.length > 0) {
                            await (0, clickhouse_client_1.insertCandles)(tokenAddress, chain, candles5m, '5m');
                        }
                        // Ingest 1m candles
                        if (candles1m.length > 0) {
                            await (0, clickhouse_client_1.insertCandles)(tokenAddress, chain, candles1m, '1m');
                        }
                    }
                    else {
                        // Ingest all as single interval
                        await (0, clickhouse_client_1.insertCandles)(tokenAddress, chain, candles, interval);
                    }
                    ingestedToClickHouse = true;
                    logger_1.logger.debug(`OHLCV Engine: Ingested ${candles.length} candles to ClickHouse for ${tokenAddress.substring(0, 20)}...`);
                }
                catch (error) {
                    logger_1.logger.warn(`OHLCV Engine: Failed to ingest to ClickHouse for ${tokenAddress.substring(0, 20)}...`, { error: error.message });
                }
            }
            return {
                candles,
     ...
     */
    async batchFetch(tokens, startTime, endTime, chain = 'solana', options = {}) {
        const results = new Map();
        logger_1.logger.info(`OHLCV Engine: Batch fetching ${tokens.length} tokens`);
        for (const token of tokens) {
            try {
                ...
                logger_1.logger.error(`OHLCV Engine: Failed to fetch ${token.substring(0, 20)}...`, error);
                results.set(token, {
                    candles: [],
                    fromCache: false,
                    ingestedToClickHouse: false,
...
exports.OHLCVEngine = OHLCVEngine;
// Singleton instance
let engineInstance = null;
/**
 * Get the singleton OHLCV Engine instance
 */
function getOHLCVEngine() {
    if (!engineInstance) {
        engineInstance = new OHLCVEngine();
    }
    return engineInstance;
}
//# sourceMappingURL=ohlcv-engine.js.map