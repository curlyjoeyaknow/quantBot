...
exports.fetchHistoricalCandlesForMonitoring = fetchHistoricalCandlesForMonitoring;
 ...
 */
async function fetchHistoricalCandlesForMonitoring(tokenAddress, chain = 'solana', alertTime) {
    try {
        ...
        logger_1.logger.info('Fetched historical candles', {
            tokenAddress: tokenAddress.substring(0, 20),
            '1m': candles1m.length,
            '5m': candles5m.length,
            '1h': candles1h.length,
        ...
        try {
            ...
            if (candles1m.length > 0) {
                await (0, clickhouse_client_1.insertCandles)(tokenAddress, chain, candles1m, '1m', true);
                logger_1.logger.debug('Stored 1m candles in ClickHouse', {
                    tokenAddress: tokenAddress.substring(0, 20),
            ...
            if (candles5m.length > 0) {
                await (0, clickhouse_client_1.insertCandles)(tokenAddress, chain, candles5m, '5m', true);
                logger_1.logger.debug('Stored 5m candles in ClickHouse', {
                    tokenAddress: tokenAddress.substring(0, 20),
            ...
            if (candles1h.length > 0) {
                await (0, clickhouse_client_1.insertCandles)(tokenAddress, chain, candles1h, '1H', true);
                logger_1.logger.debug('Stored 1h candles in ClickHouse', {
                    tokenAddress: tokenAddress.substring(0, 20),
            ...
            logger_1.logger.warn('Failed to store candles in ClickHouse', {
        ...
        if (alertTime && candles1m.length > 0) {
            ...
            const alertDateTime = luxon_1.DateTime.fromJSDate(alertTime);
            ...
            for (const candle1m of alertWindow1m) {
                const candle1mTime = luxon_1.DateTime.fromSeconds(candle1m.timestamp);
                const isCovered = candles5m.some(c5m => {
                    const c5mTime = luxon_1.DateTime.fromSeconds(c5m.timestamp);
        ...
        logger_1.logger.error('Failed to fetch historical candles', error, {
            tokenAddress: tokenAddress.substring(0, 20),
        });
 ...
 */
async function fetchCandles(tokenAddress, chain, startTime, endTime, interval) {
    try {
        const startUnix = Math.floor(startTime.toSeconds());
        const endUnix = Math.floor(endTime.toSeconds());
        const response = await birdeye_client_1.birdeyeClient.fetchOHLCVData(tokenAddress, new Date(startUnix * 1000), new Date(endUnix * 1000), interval);
...
//# sourceMappingURL=fetch-historical-candles.js.map