...
exports.initCallerDatabase = initCallerDatabase;
exports.findCallsForToken = findCallsForToken;
exports.getRecentCalls = getRecentCalls;
exports.getCallerStats = getCallerStats;
...
function initCallerDatabase() {
    return new Promise((resolve, reject) => {
        const db = new sqlite3_1.Database(CALLER_DB_PATH);
 ...
 */
async function findCallsForToken(tokenAddress) {
    ...
    try {
      ...
      `, [tokenAddress], (err, rows) => {
        ...
        logger_1.logger.error('Error finding calls for token', error, { tokenAddress });
        await new Promise((resolve, reject) => {
            db.close((err) => {
                if (err)
                    reject(err);
...
async function getRecentCalls(limit = 20) {
    ...
    try {
      ...
      `, [limit], (err, rows) => {
        ...
        logger_1.logger.error('Error getting recent calls', error, { limit });
        await new Promise((resolve, reject) => {
            db.close((err) => {
                if (err)
                    reject(err);
...
async function getCallerStats() {
    ...
    `);
        ...
        return { stats: stats[0], topCallers: topCallers };
    }
    catch (error) {
        logger_1.logger.error('Error getting caller stats', error);
        await new Promise((resolve, reject) => {
            db.close((err) => {
                if (err)
                    reject(err);
