...
exports.TenkanKijunAlertService = void 0;
...
const pump_idl_decoder_1 = require("./pump-idl-decoder");
const dex_transaction_parser_1 = require("./dex-transaction-parser");
...
try {
    ...
    logger_1.logger.warn('Yellowstone gRPC package not installed. Install with: npm install @triton-one/yellowstone-grpc');
}
class TenkanKijunAlertService extends events_1.EventEmitter {
    constructor(shyftApiKey, shyftWsUrl, shyftXToken, shyftGrpcUrl, rpcUrl) {
        super();
        this.activeMonitors = new Map();
        this.bondingCurveCache = new Map(); // Cache mint -> bonding curve address
        this.bondingCurveToMint = new Map(); // Reverse mapping: bonding curve address -> mint
        this.ws = null;
        ...
        this.jupiterPriceLastRequest = 0;
        this.jupiterPriceRequestCount = 0;
        this.jupiterPriceRequestWindowStart = Date.now();
        this.jupiterTokensLastRequest = 0;
        this.jupiterTokensRequestCount = 0;
        this.jupiterTokensRequestWindowStart = Date.now();
        this.JUPITER_MIN_REQUEST_INTERVAL = 1000; // 1 second between requests
        this.JUPITER_MAX_REQUESTS_PER_MINUTE = 60;
        this.MONITOR_WINDOW_DAYS = 7; // Monitor tokens from last 7 days
        this.CANDLE_INTERVAL_MS = 5 * 60 * 1000; // 5 minutes
        this.PRICE_UPDATE_INTERVAL_MS = 5 * 1000; // 5 seconds (fallback polling only - real-time gRPC/WebSocket updates are immediate)
        this.MIN_CANDLES_FOR_ENTRY = 52; // Need 52 candles for Ichimoku
        // DEX Program IDs
         ...
         */
        this.solPriceCache = { price: 150, timestamp: 0 };
        ...
        const rpcEndpoint = rpcUrl || process.env.SOLANA_RPC_URL || 'https://api.mainnet-beta.solana.com';
        this.rpcConnection = new web3_js_1.Connection(rpcEndpoint, 'confirmed');
        // Initialize DEX transaction parser
        this.dexParser = new dex_transaction_parser_1.DexTransactionParser();
    ...
    async start() {
        ...
        logger_1.logger.info('Alert service started', { tokenCount: this.activeMonitors.size });
    }
    /**
    ...
    stop() {
        ...
        logger_1.logger.info('Alert service stopped');
    }
    /**
            ...
            for (const callerName of callers) {
                try {
                    ...
                    for (const alert of alerts) {
                        ...
                        if (alert.chain.toLowerCase() !== 'solana')
                            continue;
                        ...
                        logger_1.logger.info('Monitoring token', { tokenSymbol: monitor.tokenSymbol, callerName: alert.callerName });
                        // Fetch initial price to start building candles
                        logger_1.logger.debug('Starting monitoring - fetching initial price', { tokenSymbol: monitor.tokenSymbol });
                        this.fetchJupiterPrice(monitor).then(() => {
                            logger_1.logger.debug('Initial price fetched, building candles', { tokenSymbol: monitor.tokenSymbol });
                        }).catch((error) => {
                            logger_1.logger.warn('Initial price fetch failed, will retry', { tokenSymbol: monitor.tokenSymbol, error: error.message });
                        });
                    }
                }
                catch (error) {
                    logger_1.logger.error('Error loading calls for caller', error, { callerName });
                }
            }
            logger_1.logger.info('Loaded tokens for monitoring', { tokenCount: this.activeMonitors.size });
        }
        catch (error) {
            logger_1.logger.error('Error loading recent calls', error);
        }
    }
    /**
    ...
    async connectGrpc() {
        if (!YellowstoneGrpcClient || !this.SHYFT_X_TOKEN) {
            logger_1.logger.warn('Yellowstone gRPC not available. Falling back to WebSocket.');
            await this.connectWebSocket();
            return;
        }
        try {
            logger_1.logger.info('Connecting to Yellowstone gRPC...');
            ...
            if (typeof YellowstoneGrpcClient === 'function') {
                ...
                this.grpcClient = new YellowstoneGrpcClient.YellowstoneGrpcClient(this.SHYFT_GRPC_URL, this.SHYFT_X_TOKEN);
            }
            else {
            ...
            this.grpcStream.on('data', (data) => {
                ...
                if (Math.random() < 0.01) { // Log 1% of updates to avoid spam
                    logger_1.logger.debug('gRPC received update', { monitorCount: this.activeMonitors.size });
                }
                this.handleGrpcUpdate(data);
            });
            this.grpcStream.on('error', (error) => {
                logger_1.logger.error('gRPC stream error', error);
                this.handleReconnect();
            });
            logger_1.logger.info('gRPC stream connected and listening for updates');
            ...
            logger_1.logger.info('Connected to Yellowstone gRPC. Stream ready.');
            ...
            logger_1.logger.error('Failed to connect to Yellowstone gRPC', error);
            logger_1.logger.info('Falling back to WebSocket...');
            await this.connectWebSocket();
        }
    }
    /**
    ...
    async connectWebSocket() {
        ...
        if (!this.SHYFT_API_KEY && !this.SHYFT_X_TOKEN) {
            logger_1.logger.warn('No Shyft API key configured. Using polling mode only.');
            return;
        }
        try {
            // Use Shyft WebSocket if available
            const wsUrl = this.SHYFT_WS_URL || 'wss://api.shyft.to/v1/stream';
            logger_1.logger.info('Connecting to Shyft WebSocket...');
            ...
            this.ws.on('open', () => {
                ...
                if (this.SHYFT_API_KEY || this.SHYFT_X_TOKEN) {
                    const authMsg = {
                        ...
                        id: 1,
                        method: 'auth',
                        params: [this.SHYFT_X_TOKEN || this.SHYFT_API_KEY]
                    };
                    if (this.ws) {
            ...
            this.ws.on('message', (data) => {
                try {
                    ...
                    // Log that we received data (but don't spam)
                    if (Math.random() < 0.01) { // Log 1% of updates to avoid spam
                        logger_1.logger.debug('WebSocket received update', { monitorCount: this.activeMonitors.size });
                    }
                    this.handlePriceUpdate(message);
                }
                catch (error) {
                    logger_1.logger.error('Error parsing price update', error);
                }
            });
            logger_1.logger.info('WebSocket connected and listening for updates');
            this.ws.on('close', () => {
            ...
            logger_1.logger.info('Falling back to polling mode');
        }
    }
    /**
    ...
    handleGrpcUpdate(data) {
        if (data.account) {
            ...
            try {
                if (account?.pubkey) {
                    accountAddress = new web3_js_1.PublicKey(account.pubkey).toBase58();
                }
                else if (Buffer.isBuffer(account) || account instanceof Uint8Array) {
                    accountAddress = new web3_js_1.PublicKey(account).toBase58();
                }
            ...
            for (const [key, monitor] of this.activeMonitors.entries()) {
                ...
                if (accountAddress === bondingCurveAddress) {
                    ...
                    if (accountData && accountData.length > 0) {
                        ...
                        const decoded = (0, pump_idl_decoder_1.decodeBondingCurveAccount)(dataBuffer);
                        if (decoded) {
                            const solPriceUsd = this.solPriceCache?.price || 150;
                            const price = (0, pump_idl_decoder_1.calculatePriceFromBondingCurve)(decoded, solPriceUsd);
                            // Get mint address for logging
                            const mintAddress = this.getMintFromBondingCurve(accountAddress);
                            const mintDisplay = mintAddress ? `${mintAddress.substring(0, 8)}...` : monitor.tokenAddress.substring(0, 8) + '...';
                            if (price > 0) {
                                logger_1.logger.debug('gRPC price update', { tokenSymbol: monitor.tokenSymbol, mint: mintDisplay, price });
                                const solReserves = decoded.real_sol_reserves || decoded.virtual_sol_reserves || decoded.sol_reserves;
                                const tokenReserves = decoded.real_token_reserves || decoded.virtual_token_reserves || decoded.token_reserves;
                                const solStr = typeof solReserves === 'string' ? solReserves : solReserves?.toString?.() || String(solReserves || 'N/A');
                                const tokenStr = typeof tokenReserves === 'string' ? tokenReserves : tokenReserves?.toString?.() || String(tokenReserves || 'N/A');
                                logger_1.logger.debug('Bonding curve reserves', { tokenSymbol: monitor.tokenSymbol, solReserve: solStr, tokenReserve: tokenStr });
                                const creatorDisplay = typeof decoded.creator === 'string'
                                    ? decoded.creator.substring(0, 8) + '...'
                                    : decoded.creator?.toBase58()?.substring(0, 8) + '...' || 'N/A';
                                logger_1.logger.debug('Bonding curve status', { tokenSymbol: monitor.tokenSymbol, complete: decoded.complete, creator: creatorDisplay });
                                this.updateTokenPrice(monitor, price, Date.now());
                            }
                        }
                        else {
                            logger_1.logger.warn('Failed to decode bonding curve', { tokenSymbol: monitor.tokenSymbol });
                        }
                    }
                    else {
                        ...
                        logger_1.logger.info('Token graduated, fetching DEX price', { tokenSymbol: monitor.tokenSymbol });
                        this.fetchJupiterPrice(monitor).catch((error) => {
                            logger_1.logger.error('Error fetching Jupiter price', error, { tokenSymbol: monitor.tokenSymbol });
                        });
                    }
                    return;
                }
        ...
        try {
            ...
            if (mint) {
                ...
                if (bondingCurveAddress) {
                    logger_1.logger.info('Pump.fun new token created', { mint: mint.substring(0, 8), bondingCurve: bondingCurveAddress.substring(0, 8) });
                }
            }
            ...
            if (swapEvent && swapEvent.mint) {
                ...
                if (monitor) {
                    ...
                    if (tokenAmount > 0) {
                        ...
                        logger_1.logger.debug('DEX swap price update', { tokenSymbol: monitor.tokenSymbol, price, swapType: swapEvent.type });
                        this.updateTokenPrice(monitor, price, Date.now());
                    }
                }
            }
     ...
     */
    async fetchMintFromBondingCurve(bondingCurveAddress) {
    ...
    deriveBondingCurveAddress(mint) {
        ...
        try {
            ...
            const [bondingCurve] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('bonding-curve'), mintPubkey.toBuffer()], PUMP_PROGRAM_ID);
            ...
            logger_1.logger.error('Failed to derive bonding curve address', error, { mint });
            return '';
        }
    }
    /**
     ...
     */
    getMintFromBondingCurve(bondingCurveAddress) {
     ...
     */
    parsePriceFromBondingCurve(accountData) {
        try {
            if (!accountData)
                return 0;
            // Decode using IDL structure
            const decoded = (0, pump_idl_decoder_1.decodeBondingCurveAccount)(accountData);
            if (!decoded) {
    ...
    subscribeToAllTokens() {
        ...
        else if (this.ws && this.ws.readyState === ws_1.default.OPEN) {
     ...
     */
    subscribeToBondingCurvesGrpc() {
        if (!this.grpcClient || !this.grpcStream)
            return;
        const accounts = [];
        for (const [key, monitor] of this.activeMonitors.entries()) {
        ...
        if (accounts.length === 0) {
            logger_1.logger.warn('No bonding curve addresses to subscribe to');
            return;
        }
        ...
        try {
            ...
            if (accounts.length > 0 && this.grpcStream) {
                ...
                logger_1.logger.info('gRPC subscribed', {
            ...
            logger_1.logger.error('Failed to subscribe via gRPC', error);
            // Fallback to polling if subscription fails
            if (!this.updateInterval) {
                logger_1.logger.info('Falling back to polling mode (Jupiter API)');
                this.startPeriodicUpdates();
            }
        }
    }
     ...
     */
    subscribeToBondingCurvesWebSocket() {
        ...
        logger_1.logger.info('Subscribed to accounts via WebSocket', { count: subscriptions.length });
    }
    /**
    ...
    handlePriceUpdate(message) {
        ...
        if (!monitor)
            return;
        logger_1.logger.debug('WebSocket price update', { tokenSymbol: monitor.tokenSymbol, price });
    ...
    updateTokenPrice(monitor, price, timestamp) {
        ...
        if (newCandleCreated) {
            logger_1.logger.debug('New 5m candle created', { tokenSymbol: monitor.tokenSymbol, totalCandles: candlesAfter });
        }
        ...
        if (monitor.candles.length >= this.MIN_CANDLES_FOR_ENTRY) {
            ...
            logger_1.logger.debug('Building candles', { tokenSymbol: monitor.tokenSymbol, current: monitor.candles.length, required: this.MIN_CANDLES_FOR_ENTRY });
        }
    }
    /**
    ...
    updateCandles(monitor, price, timestamp) {
        const now = luxon_1.DateTime.fromMillis(timestamp);
        ...
        const lastCandleTime = luxon_1.DateTime.fromMillis(lastCandle.timestamp * 1000);
    ...
    recalculateIndicators(monitor) {
        ...
        if (monitor.indicatorHistory.length > 0) {
            ...
            if (latest?.ichimoku) {
                ...
                logger_1.logger.debug('Indicators calculated', { tokenSymbol: monitor.tokenSymbol, tenkan, kijun });
            }
        }
    }
    /**
    ...
    checkSignals(monitor) {
        ...
        if (!current?.ichimoku || !previous?.ichimoku)
            return;
        ...
        if (!monitor.inPosition && !monitor.entrySignalSent) {
            ...
            if (crossedUp) {
                ...
                logger_1.logger.info('BUY SIGNAL', { tokenSymbol: monitor.tokenSymbol, entryPrice: monitor.entryPrice, callerName: monitor.callerName });
            }
        }
        ...
        if (monitor.inPosition && !monitor.exitSignalSent) {
            ...
            if (crossedDown || priceHitStop) {
                ...
                this.emit('alert', alert);
                logger_1.logger.info('SELL SIGNAL', { tokenSymbol: monitor.tokenSymbol, exitPrice, pnl: ((pnl - 1) * 100).toFixed(2) });
            }
        }
    }
    /**
        ...
        this.updateInterval = setInterval(async () => {
            ...
            for (const monitor of this.activeMonitors.values()) {
                try {
                    ...
                    logger_1.logger.error('Error fetching price', error, { tokenSymbol: monitor.tokenSymbol });
                }
            }
     ...
     */
    async fetchJupiterPrice(monitor) {
        try {
            ...
            const solAmount = 1000000000; // 1 SOL (9 decimals)
            const response = await axios_1.default.get(jupiterUrl, {
                params: {
            ...
            if (response.data?.outAmount && response.data?.inAmount) {
                ...
                if (outAmount > 0) {
                    ...
                    if (priceUsd > 0 && isFinite(priceUsd)) {
                        logger_1.logger.debug('Jupiter price fetched', { tokenSymbol: monitor.tokenSymbol, price: priceUsd });
    ...
    async getTokenDecimals(mint) {
        ...
        try {
            ...
            // Rate limits: 1 rps, 60 rpm (no API key required)
            const response = await axios_1.default.get(`https://lite-api.jup.ag/tokens/v2/token/${mint}`, {
                timeout: 3000,
            });
            ...
            const decimals = response.data?.decimals
                ...
                || 6; // Default to 6 for Pump.fun tokens
            // Cache the result
    ...
    async getSolPriceUsd() {
        ...
        try {
            ...
            // Rate limits: 1 rps, 60 rpm (no API key required)
            const response = await axios_1.default.get('https://lite-api.jup.ag/price/v3', {
                params: {
            ...
            const price = response.data?.[solMint]?.price
            ...
            if (error.response?.status === 429) {
                ...
                logger_1.logger.warn('Jupiter Price API rate limited, using cached price');
    ...
    handleReconnect() {
        if (this.reconnectAttempts >= this.maxReconnectAttempts) {
            logger_1.logger.error('Max reconnection attempts reached. Using polling mode.');
            return;
        }
        this.reconnectAttempts++;
        const delay = Math.min(1000 * Math.pow(2, this.reconnectAttempts), 30000);
        logger_1.logger.info('Reconnecting', { delayMs: delay, attempt: this.reconnectAttempts, maxAttempts: this.maxReconnectAttempts });
        setTimeout(async () => {
            await this.connect();
        }, delay);
    }
    ...
    logStatus() {
        ...
        logger_1.logger.info('Monitor Status', {
     ...
     */
    async removeMonitor(tokenAddress, chain) {
        const key = `${chain}:${tokenAddress}`.toLowerCase();
        if (this.activeMonitors.has(key)) {
            this.activeMonitors.delete(key);
            logger_1.logger.info('Removed monitor', { tokenAddress });
        }
    }
    /**
     * Add a token to monitor manually (alias for addMonitor)
     */
    async addMonitor(tokenAddress, tokenSymbol, chain, callerName, alertTime, alertPrice) {
        await this.addToken(tokenAddress, tokenSymbol, chain, callerName, luxon_1.DateTime.fromJSDate(alertTime), alertPrice);
    }
    /**
     * Add a token to monitor manually
     */
    async addToken(tokenAddress, tokenSymbol, chain, callerName, alertTime, alertPrice) {
        const key = `${chain}:${tokenAddress}`.toLowerCase();
        if (this.activeMonitors.has(key)) {
            logger_1.logger.warn('Already monitoring token', { tokenSymbol });
            return;
        }
        ...
        logger_1.logger.info('Added token to monitoring', { tokenSymbol, tokenAddress, chain });
    }
}
exports.TenkanKijunAlertService = TenkanKijunAlertService;
exports.default = TenkanKijunAlertService;
//# sourceMappingURL=tenkan-kijun-alert-service.js.map