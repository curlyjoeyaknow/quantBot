...
export class BrookCallIngestion {
   ...
   */
  private isForwardedFromBrook(message: any): boolean {
    // Check if message is forwarded
    const forwardFrom = (message as any).forward_from_chat;
    if (!forwardFrom) {
      return false;
    }
  ...
  private async handleMessage(ctx: Context): Promise<void> {
    ...
    if (!text) {
      logger.debug('No text found in message', { messageId });
      return;
    }
    ...
    if (isForwarded) {
      logger.info('Received forwarded message from Brook channel', {
        messageId,
        textPreview: text.substring(0, 100),
      });
    } else {
      logger.info('Received message in personal chat', {
        messageId,
        textPreview: text.substring(0, 100),
      });
    }
