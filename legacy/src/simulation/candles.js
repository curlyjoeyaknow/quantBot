 ...
 */
function getCacheFilename(mint, startTime, endTime, chain) {
 ...
 */
function saveCandlesToCache(candles, filename) {
    try {
        const csvContent = [
        ...
        logger_1.logger.debug('Cached candles', { count: candles.length, filename });
    }
    catch (error) {
        logger_1.logger.error('Failed to save cache', error, { filename });
    }
}
/**
 ...
 */
function loadCandlesFromCache(filename) {
    try {
        ...
        if (process.env.USE_CACHE_ONLY !== 'true') {
            ...
            if (ageHours > CACHE_EXPIRY_HOURS) {
                logger_1.logger.debug(`Cache expired (${ageHours.toFixed(1)}h old). Removing ${filename}...`);
        ...
        logger_1.logger.debug(`Loaded ${candles.length} candles from cache ${filename}`);
        return candles;
    }
    catch (error) {
        logger_1.logger.error('Failed to load cache', error, { filename });
        return null;
    }
}
/* ============================================================================
 ...
 */
function getAggregationIntervalSeconds(interval) {
    switch (interval) {
        case '5m':
            return 5 * 60;
        case '15m':
 ...
 */
async function fetchFreshCandles(mint, startTime, endTime, chain = 'solana', interval = '5m') {
 ...
 */
function mergeCandles(candles5m, candles1m, alertTime, windowMinutes = 30) {
    ...
    logger_1.logger.debug(`Merged candles: ${candles5m.length} 5m + ${candles1m.length} 1m = ${merged.length} total (removed ${candles5m.length - filtered5m.length} overlapping 5m candles)`);
    return merged;
}
/**
 ...
 */
async function fetchBirdeyeCandles(mint, interval, from, to, chain = 'solana') {
    try {
        ...
        if (estimatedCandles > MAX_CANDLES_PER_REQUEST) {
            logger_1.logger.debug(`Chunking request for ${mint.substring(0, 20)}... (${estimatedCandles} candles estimated, ${Math.ceil(estimatedCandles / MAX_CANDLES_PER_REQUEST)} chunks needed)`);
            const allCandles = [];
            let currentFrom = from;
            while (currentFrom < to) {
                // Calculate chunk end time (max 5000 candles worth)
                const chunkTo = Math.min(currentFrom + maxWindowSeconds, to);
                const chunkCandles = await fetchBirdeyeCandlesChunk(mint, interval, currentFrom, chunkTo, chain, apiKey);
 ...
 */
async function fetchBirdeyeCandlesDirect(mint, interval, from, to, chain = 'solana') {
 ...
 */
async function fetchBirdeyeCandlesChunk(mint, interval, from, to, chain, apiKey) {
    try {
        // Fetch by date range (up to 5000 candles)
        const response = await axios_1.default.get(BIRDEYE_ENDPOINT, {
            headers: {
                'X-API-KEY': apiKey,
                'x-chain': chain,
        ...
        if (items.length === 0) {
            logger_1.logger.debug(`No candles found for date range, trying limit approach for ${mint.substring(0, 20)}...`);
            const limitResponse = await axios_1.default.get(BIRDEYE_ENDPOINT, {
                headers: {
                    'X-API-KEY': apiKey,
                    'x-chain': chain,
                    accept: 'application/json',
            ...
            if (limitResponse.status === 200) {
                ...
                if (items.length > 0) {
                    logger_1.logger.debug(`Fetched ${items.length} candles using limit approach for ${mint.substring(0, 20)}...`);
 ...
 */
async function fetchOptimizedCandlesForAlert(mint, alertTime, endTime = luxon_1.DateTime.utc(), chain = 'solana') {
    ...
    logger_1.logger.debug('Fetching optimized candles for alert', {
    ...
    logger_1.logger.debug('Fetching 1m candles', {
    ...
    const oneMCandles = await fetchBirdeyeCandles(mint, '1m', oneMHistoricalStart, Math.min(oneMForwardEnd, endUnix), chain);
    ...
    if (fifteenSStart >= threeMonthsAgo) {
        logger_1.logger.debug('Fetching 15s candles', {
        ...
        const fifteenSCandles = await fetchBirdeyeCandles(mint, '15s', fifteenSStart, Math.min(fifteenSEnd, endUnix), chain);
        allCandles.push(...fifteenSCandles);
    }
    else {
        logger_1.logger.debug('Skipping 15s candles (alert >3 months ago)');
    }
    ...
    if (current5mFrom < endUnix) {
        logger_1.logger.debug('Fetching 5m candles', {
    ...
    logger_1.logger.debug('Optimized fetch complete', {
        mint: mint.substring(0, 20),
 ...
 */
async function fetchHybridCandles(mint, startTime, endTime, chain = 'solana', alertTime) {
    ...
    if (alertTime) {
        ...
        if (ichimokuLookback < startTime) {
            ...
            logger_1.logger.debug(`Extended start time back by 260 minutes for Ichimoku: ${actualStartTime.toISO()} (original: ${startTime.toISO()})`);
        }
    }
    ...
    if (process.env.USE_CLICKHOUSE === 'true' || process.env.CLICKHOUSE_HOST) {
        try {
            const { queryCandles } = await Promise.resolve().then(() => __importStar(require('../storage/clickhouse-client')));
            const clickhouseCandles = await queryCandles(mint, chain, actualStartTime, endTime);
            if (clickhouseCandles.length > 0) {
                logger_1.logger.debug(`Using ClickHouse candles for ${mint} (${clickhouseCandles.length} candles, from ${actualStartTime.toISO()})`);
                ...
                if (actualStartTime < startTime) {
                    ...
                    logger_1.logger.debug(`Filtered ClickHouse candles to requested range: ${filtered.length} candles (had ${clickhouseCandles.length} with lookback)`);
                    return filtered;
                }
            ...
            logger_1.logger.warn('ClickHouse query failed, falling back to CSV cache', { error: error.message, mint });
        }
    }
    ...
    if (cachedCandles) {
        logger_1.logger.debug(`Using cached candles for ${mint} (${cachedCandles.length} candles)`);
        ...
        if (process.env.USE_CACHE_ONLY !== 'true' && (process.env.USE_CLICKHOUSE === 'true' || process.env.CLICKHOUSE_HOST)) {
            try {
                const { insertCandles } = await Promise.resolve().then(() => __importStar(require('../storage/clickhouse-client')));
                ...
                logger_1.logger.debug(`✅ Synced ${cachedCandles.length} cached candles to ClickHouse for ${mint.substring(0, 20)}...`);
            }
            catch (error) {
    ...
    if (cacheFiles.length > 0) {
        ...
        if (cachedData && cachedData.length > 0) {
            const lastCachedTime = luxon_1.DateTime.fromSeconds(cachedData[cachedData.length - 1].timestamp);
            if (endTime.diff(lastCachedTime, 'hours').hours < 1) {
                logger_1.logger.debug(`Extending recent cache from ${lastCachedTime.toFormat('yyyy-MM-dd HH:mm')} to ${endTime.toFormat('yyyy-MM-dd HH:mm')}`);
    ...
    if (process.env.USE_CACHE_ONLY === 'true') {
        logger_1.logger.debug(`⚠️ No cached candles found for ${mint}: ${startTime.toISO()} — ${endTime.toISO()}`);
        logger_1.logger.debug(`   USE_CACHE_ONLY=true, returning empty array (no API calls)`);
        return [];
    }
    logger_1.logger.debug(`Fetching fresh candles for ${mint}: ${startTime.toISO()} — ${endTime.toISO()}`);
    ...
    if (actualStartTime < startTime) {
        ...
        logger_1.logger.debug(`Fetched ${candles5m.length} 5m candles with lookback (${candles5mFiltered.length} in requested range)`);
    }
    ...
    if (alertTime) {
        ...
        logger_1.logger.debug(`Fetching 1m candles for alert window: ${windowStart.toISO()} — ${windowEnd.toISO()}`);
    ...
    if (alertTime && actualStartTime < startTime) {
        ...
        logger_1.logger.debug(`Returning ${finalCandles.length} candles (${lookbackCandles.length} lookback + ${finalCandles.length - lookbackCandles.length} requested range) for Ichimoku`);
    }
    if (finalCandles.length > 0) {
        ...
        if (actualStartTime < startTime && candles5m.length > candles5mFiltered.length) {
            ...
            logger_1.logger.debug(`Saved lookback candles to cache: ${lookbackCacheFilename} (${candles5m.length} candles)`);
        }
            ...
            try {
                ...
                if (alertTime) {
                    ...
                    const candles1m = finalCandles.filter(c => {
                        const candleTime = luxon_1.DateTime.fromSeconds(c.timestamp);
                ...
                logger_1.logger.debug(`✅ Saved candles to ClickHouse for ${mint.substring(0, 20)}...`);
            }
            catch (error) {
                logger_1.logger.error('Failed to save to ClickHouse', error, { mint: mint.substring(0, 20) });
        ...
        if (process.env.DEBUG_CANDLES === 'true') {
            logger_1.logger.debug(`⚠️ No candles returned for ${mint.substring(0, 20)}... (token may not exist or have no data)`);
        }
    }
    return finalCandles;
}
