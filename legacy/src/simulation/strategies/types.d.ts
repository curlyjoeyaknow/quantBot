 ...
 */
/**
 * Single take-profit level (legacy representation).
 */
...
export type IndicatorName = 'rsi' | 'macd' | 'sma' | 'ema' | 'vwma' | 'bbands' | 'atr' | 'ichimoku_cloud' | 'price_change' | 'volume_change' | 'custom';
...
export interface LadderLegConfig {
    id?: string;
    sizePercent: number;
    priceOffset?: number;
    multiple?: number;
...
export interface StrategyConfig {
    /** Strategy name */
    name: string;
    /** Take profit levels (legacy representation) */
    ...
    minExitPrice?: number;
    /** Optional indicator-based entry and exit signals */
    entrySignal?: SignalGroup;
    exitSignal?: SignalGroup;
    /** Optional laddered entry and exit configuration */
 ...
 */
export type StrategyPresetName = 'basic-6h-20pct-sl' | 'conservative-24h' | 'aggressive-multi-tp' | 'trailing-stop-20pct' | 'buy-the-dip-30pct' | string;
