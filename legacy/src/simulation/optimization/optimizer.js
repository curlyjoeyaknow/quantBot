...
const builder_1 = require("../strategies/builder");
const engine_1 = require("../engine");
const candles_1 = require("../candles");
const loaders_1 = require("../../data/loaders");
...
class StrategyOptimizer {
    ...
    async optimize(config) {
        ...
        const strategies = (0, grid_1.generateParameterCombinations)(config.parameterGrid, config.baseStrategy);
        // Limit strategies if specified
        const strategiesToTest = config.maxStrategies
            ? strategies.slice(0, config.maxStrategies)
            : strategies;
        // Load data
        const dataRecords = await (0, loaders_1.loadData)({
        ...
        for (let i = 0; i < strategiesToTest.length; i += maxConcurrent) {
            ...
            const batchResults = await Promise.all(batch.map(strategy => this.testStrategy(strategy, dataRecords)));
     ...
     */
    async testStrategy(strategy, dataRecords) {
        // Validate strategy
        const validation = (0, builder_1.validateStrategy)(strategy);
        if (!validation.valid) {
        ...
        for (const record of dataRecords) {
            try {
                ...
                const timestamp = record.timestamp instanceof luxon_1.DateTime
                    ? record.timestamp
                    : luxon_1.DateTime.fromJSDate(record.timestamp);
                if (!mint || !timestamp.isValid) {
                    skipped++;
                    continue;
                }
                const endTime = record.endTime || timestamp.plus({ days: 60 });
                // Pass timestamp as alertTime for 1m candles around alert time
                const candles = await (0, candles_1.fetchHybridCandles)(mint, timestamp, endTime, chain, timestamp);
                if (candles.length < 10) {
                    skipped++;
                    continue;
                }
                // Build strategy parameters
                const strategyParams = (0, builder_1.buildStrategy)(strategy);
                const stopLossConfig = (0, builder_1.buildStopLossConfig)(strategy);
                const entryConfig = (0, builder_1.buildEntryConfig)(strategy);
                const reEntryConfig = (0, builder_1.buildReEntryConfig)(strategy);
                // Run simulation
                const result = (0, engine_1.simulateStrategy)(candles, strategyParams, stopLossConfig, entryConfig, reEntryConfig);
     ...
     */
    calculateTimeToAth(candles, events) {
