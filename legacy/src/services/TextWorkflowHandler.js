...
exports.TextWorkflowHandler = void 0;
const telegraf_1 = require("telegraf");
const luxon_1 = require("luxon");
const axios_1 = __importDefault(require("axios"));
const caller_database_1 = require("../utils/caller-database");
const logger_1 = require("../utils/logger");
class TextWorkflowHandler {
    constructor(sessionService, simulationService, strategyService, ichimokuWorkflowService, caDetectionService, repeatHelper) {
        this.sessionService = sessionService;
        this.simulationService = simulationService;
        this.strategyService = strategyService;
        this.ichimokuWorkflowService = ichimokuWorkflowService;
        this.caDetectionService = caDetectionService;
    ...
    async handleCallbackQuery(ctx) {
        try {
            ...
            if (!userId) {
                logger_1.logger.debug('No userId in callback query');
                return;
            }
            // Only respond to direct messages
            if (callbackQuery.message && 'chat' in callbackQuery.message && callbackQuery.message.chat.type !== 'private') {
                logger_1.logger.debug('Ignoring non-private callback query', { userId });
                return;
            }
            await ctx.answerCbQuery();
            const data = callbackQuery.data;
            if (!data) {
                logger_1.logger.debug('No data in callback query', { userId });
                return;
            }
            logger_1.logger.debug('Processing callback', { userId, callbackData: data });
            // Handle add_curlyjoe callbacks (doesn't require session)
            if (data.startsWith('add_curlyjoe:')) {
                const { AddCurlyJoeCommandHandler } = await Promise.resolve().then(() => __importStar(require('../commands/AddCurlyJoeCommandHandler')));
            ...
            if (data.startsWith('watchlist:')) {
                const { WatchlistCommandHandler } = await Promise.resolve().then(() => __importStar(require('../commands/WatchlistCommandHandler')));
                await WatchlistCommandHandler.handleCallback(ctx, data);
                return;
            }
            const session = this.sessionService.getSession(userId);
            if (!session) {
                logger_1.logger.debug('No session found for user when handling callback', { userId, callbackData: data });
                await ctx.reply('❌ No active session found. Please start a new backtest with /backtest');
                return;
            }
            logger_1.logger.debug('Processing callback', { userId, callbackData: data, sessionType: session.type });
            // Handle backtest source selection
            if (data.startsWith('backtest_source:')) {
                const source = data.replace('backtest_source:', '');
                logger_1.logger.debug('Handling backtest source selection', { userId, source });
            ...
            logger_1.logger.error('Error in handleCallbackQuery', error, { userId: ctx.from?.id });
    ...
    async handleTextInput(ctx, session, text, stepHint) {
    ...
    async handleBacktestSourceSelection(ctx, session, source) {
        try {
            const userId = ctx.from?.id;
            if (!userId) {
                logger_1.logger.debug('No userId in handleBacktestSourceSelection');
                return;
            }
            logger_1.logger.debug('handleBacktestSourceSelection', { userId, source });
            if (source === 'recent_backtests') {
                // Get recent backtests
                const { getUserSimulationRuns } = await Promise.resolve().then(() => __importStar(require('../utils/database')));
                ...
                const buttons = recentRuns.slice(0, 10).map((run, idx) => {
                    const label = `${idx + 1}. ${run.tokenSymbol || 'N/A'} (${run.chain}) - ${(run.finalPnl * 100).toFixed(1)}%`;
                    return [telegraf_1.Markup.button.callback(label, `backtest_select:run:${run.id}`)];
                });
                await ctx.reply('📊 **Select a Recent Backtest:**\n\nChoose one to rerun with new parameters:', telegraf_1.Markup.inlineKeyboard(buttons));
                if (!session.data)
                    session.data = {};
                const data = this.ensureSessionData(session);
                ...
                // Get recent calls
                const { getRecentCalls } = await Promise.resolve().then(() => __importStar(require('../utils/caller-database')));
                ...
                const buttons = recentCalls.slice(0, 15).map((call, idx) => {
                    ...
                    return [telegraf_1.Markup.button.callback(label, `backtest_select:call:${call.token_address}:${call.chain}`)];
                });
                await ctx.reply('📞 **Select a Recent Call:**\n\nChoose a call to backtest:', telegraf_1.Markup.inlineKeyboard(buttons));
                const data = this.ensureSessionData(session);
                ...
                // Get top callers
                const { getCallerStats } = await Promise.resolve().then(() => __importStar(require('../utils/caller-database')));
                ...
                const buttons = topCallers.slice(0, 10).map((caller, idx) => {
                    const label = `${idx + 1}. ${caller.caller_name} (${caller.alert_count} calls)`;
                    return [telegraf_1.Markup.button.callback(label, `backtest_caller:${caller.caller_name}`)];
                });
                await ctx.reply('👤 **Select a Caller:**\n\nChoose a caller to see their calls:', telegraf_1.Markup.inlineKeyboard(buttons));
                const data = this.ensureSessionData(session);
            ...
            logger_1.logger.error('Error in handleBacktestSourceSelection', error, { userId: ctx.from?.id });
            await ctx.reply(`❌ Error: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async handleBacktestSelection(ctx, session, selection) {
        const userId = ctx.from?.id;
        if (!userId)
            return;
        ...
        if (type === 'run') {
            ...
            const { getSimulationRun } = await Promise.resolve().then(() => __importStar(require('../utils/database')));
            ...
            this.sessionService.setSession(userId, session);
            // Show strategy menu
            await ctx.reply(`✅ **Selected:** ${run.tokenSymbol || 'N/A'} (${run.chain})\n\n**📈 Select Take Profit Strategy:**`, telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('✅ Default (50%@2x, 30%@5x, 20%@10x)', 'strategy:default')],
                [telegraf_1.Markup.button.callback('📊 Aggressive (30%@3x, 40%@5x, 30%@10x)', 'strategy:aggressive')],
                [telegraf_1.Markup.button.callback('🛡️ Conservative (60%@2x, 30%@3x, 10%@5x)', 'strategy:conservative')],
                [telegraf_1.Markup.button.callback('🚀 Moonshot (10%@3x, 10%@5x, 80% ride)', 'strategy:moonshot')],
                [telegraf_1.Markup.button.callback('✍️ Manual Entry', 'strategy:manual')]
            ]));
        }
        else if (type === 'call') {
            ...
            this.sessionService.setSession(userId, session);
            // Show datetime menu
            await ctx.reply(`✅ **Selected:** ${tokenAddress.substring(0, 20)}... (${chain})\n\n**📅 Select Simulation Start Date/Time:**`, telegraf_1.Markup.inlineKeyboard([
                [
                    telegraf_1.Markup.button.callback('🕐 Now', 'datetime:now'),
                    telegraf_1.Markup.button.callback('📅 Yesterday', 'datetime:yesterday')
                ],
                [
                    telegraf_1.Markup.button.callback('📆 7 Days Ago', 'datetime:7days'),
                    telegraf_1.Markup.button.callback('📆 30 Days Ago', 'datetime:30days')
                ],
                [telegraf_1.Markup.button.callback('✍️ Manual Entry', 'datetime:manual')]
            ]));
        }
    }
    async handleCallerSelection(ctx, session, callerName) {
        const userId = ctx.from?.id;
        if (!userId)
            return;
        // Get calls by this caller
        const { getCACallsByCaller } = await Promise.resolve().then(() => __importStar(require('../utils/database')));
        ...
        const buttons = calls.slice(0, 15).map((call, idx) => {
            const date = new Date(call.call_timestamp * 1000).toISOString().split('T')[0];
            const label = `${idx + 1}. ${call.token_symbol || 'N/A'} (${date})`;
            return [telegraf_1.Markup.button.callback(label, `backtest_select:call:${call.mint}:${call.chain}`)];
        });
        await ctx.reply(`👤 **Calls by ${callerName}:**\n\nSelect a call to backtest:`, telegraf_1.Markup.inlineKeyboard(buttons));
        const data = this.ensureSessionData(session);
    ...
    async handleText(ctx) {
        ...
        logger_1.logger.debug('TextWorkflowHandler processing', { userId: ctx.from?.id, sessionType: session.type, text: text.substring(0, 50) });
        await this.handleSimulationWorkflow(ctx, session, text);
    }
    async handleRunSelection(ctx, session, text) {
        const userId = ctx.from?.id;
        if (!userId)
            return;
    ...
    async handleSimulationWorkflow(ctx, session, text) {
        const userId = ctx.from?.id;
        if (!userId)
            return;
        logger_1.logger.debug('handleSimulationWorkflow', { sessionType: session.type });
        switch (session.type) {
            case 'backtest':
                logger_1.logger.debug('Routing to handleBacktestWorkflow');
    ...
    async handleBacktestWorkflow(ctx, session, text, stepHint = '') {
        const userId = ctx.from?.id;
        if (!userId)
            return;
        const data = this.ensureSessionData(session);
        logger_1.logger.debug('handleBacktestWorkflow called', { mint: data.mint, text });
        ...
        if (!data.mint) {
            ...
            if (data.waitingManualInput === 'mint') {
                delete data.waitingManualInput;
                data.mint = text;
                logger_1.logger.debug('Setting mint to', { mint: text });
            }
            else {
            ...
            try {
                ...
                logger_1.logger.error('Error checking database for calls', error, { mint: text });
            }
        ...
        if (!data.datetime) {
            ...
            if (stepHint === 'datetime' || text.startsWith('datetime:')) {
                ...
                let dt;
                if (preset === 'now') {
                    dt = luxon_1.DateTime.utc();
                }
                else if (preset === 'yesterday') {
                    dt = luxon_1.DateTime.utc().minus({ days: 1 });
                }
                else if (preset === '7days') {
                    dt = luxon_1.DateTime.utc().minus({ days: 7 });
                }
                else if (preset === '30days') {
                    dt = luxon_1.DateTime.utc().minus({ days: 30 });
                }
                    ...
                    // Try parsing as ISO
                    dt = luxon_1.DateTime.fromISO(preset, { zone: 'utc' });
                    if (!dt.isValid) {
                        dt = luxon_1.DateTime.fromISO(text, { zone: 'utc' });
                    }
                }
                if (!dt.isValid) {
                ...
                if (!dt.isValid) {
                    ...
                    await ctx.reply('📅 **Select Simulation Start Date/Time:**\n\nChoose a preset or enter manually:', telegraf_1.Markup.inlineKeyboard([
                        [
                            telegraf_1.Markup.button.callback('🕐 Now', 'datetime:now'),
                            telegraf_1.Markup.button.callback('📅 Yesterday', 'datetime:yesterday')
                        ],
            ...
            try {
                ...
                logger_1.logger.debug('Fetching metadata for mint', { mint: data.mint });
                ...
                logger_1.logger.debug('Metadata response', { mint: data.mint, hasData: !!meta.data });
                ...
                const err = e;
                logger_1.logger.error('Token metadata error', e, { status: err.response?.status, mint: data.mint });
                if (err.response?.status === 404) {
                    data.metadata = { name: 'Unknown', symbol: 'N/A' };
                    await ctx.reply(`⚠️ **Token not found on Birdeye:** ${data.mint}\n\n**📈 Select Take Profit Strategy:**`, telegraf_1.Markup.inlineKeyboard([
                        [telegraf_1.Markup.button.callback('✅ Default (50%@2x, 30%@5x, 20%@10x)', 'strategy:default')],
                        [telegraf_1.Markup.button.callback('📊 Aggressive (30%@3x, 40%@5x, 30%@10x)', 'strategy:aggressive')],
                        [telegraf_1.Markup.button.callback('🛡️ Conservative (60%@2x, 30%@3x, 10%@5x)', 'strategy:conservative')],
                        [telegraf_1.Markup.button.callback('🚀 Moonshot (10%@3x, 10%@5x, 80% ride)', 'strategy:moonshot')],
        ...
        if (!data.strategy) {
            ...
            this.sessionService.setSession(userId, session);
            // Show stop loss menu
            await ctx.reply('✅ **Take profit strategy set!**\n\n**🛑 Select Stop Loss Configuration:**', telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('✅ Default (-50% initial, 50% trailing)', 'stoploss:default')],
                [telegraf_1.Markup.button.callback('🛡️ Tight (-20% initial, 30% trailing)', 'stoploss:tight')],
                [telegraf_1.Markup.button.callback('📊 Moderate (-30% initial, 50% trailing)', 'stoploss:moderate')],
                [telegraf_1.Markup.button.callback('🚀 Wide (-50% initial, 100% trailing)', 'stoploss:wide')],
                [telegraf_1.Markup.button.callback('✍️ Manual Entry', 'stoploss:manual')]
            ]));
            return;
        }
        ...
        if (!data.stopLossConfig) {
            ...
            this.sessionService.setSession(userId, session);
            // Show re-entry menu
            await ctx.reply('✅ **Stop loss configured!**\n\n**🔄 Select Re-entry Configuration:**', telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('❌ No Re-entry (Default)', 'reentry:no')],
                [telegraf_1.Markup.button.callback('✅ Allow Re-entry', 'reentry:yes')],
                [telegraf_1.Markup.button.callback('✍️ Manual Entry', 'reentry:manual')]
            ]));
            return;
        }
        // Step 5: Re-entry configuration
        if (!data.reEntryConfig) {
            const defaultReEntry = { trailingReEntry: 'none', maxReEntries: 0 };
    ...
    async runBacktestSimulation(ctx, session) {
        const userId = ctx.from?.id;
        if (!userId)
            return;
        try {
            ...
            const startTime = luxon_1.DateTime.fromISO(datetimeStr);
            ...
            const err = error;
            const data = this.ensureSessionData(session);
            logger_1.logger.error('Backtest simulation error', error, { userId, mint: data.mint });
            await ctx.reply(`❌ **Simulation Failed**\n\nError: ${err.message || 'Unknown error'}\n\nPlease try again with a different token or timeframe.`);
            this.sessionService.clearSession(userId);
        }
    }
}
exports.TextWorkflowHandler = TextWorkflowHandler;
//# sourceMappingURL=TextWorkflowHandler.js.map