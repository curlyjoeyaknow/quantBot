...
import type { StrategyLeg, StopLossConfig, EntryConfig, ReEntryConfig, CostConfig } from '../../src/types/index.js';
import { getCandleCloseTime, getCandleCloseTimeFromInterval, type CandleInterval } from '../../src/types/causal-accessor.js';
 ...
 */
function createTestCandles(count: number, startTimestamp: number, intervalSeconds: number = 300): Candle[] {
...
describe('Gate 3: Future-Scramble Test', () => {
  ...
  it('works with different split points', async () => {
    ...
    const scrambledCandles = scrambleCandlesAfterTime(originalCandles, earlySplitTime, intervalSeconds);
    const scrambledResult = await simulateStrategy(
      scrambledCandles,
      strategy,
      stopLoss,
  ...
  it('works with late split points', async () => {
    ...
    const scrambledCandles = scrambleCandlesAfterTime(originalCandles, lateSplitTime, intervalSeconds);
    const scrambledResult = await simulateStrategy(
      scrambledCandles,
      strategy,
      stopLoss,
