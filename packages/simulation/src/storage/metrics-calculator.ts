...
export function calculateResultMetrics(result: SimulationResult): {
  ...
  const entryEvents = events.filter(e => 
    e.type === 'entry' || e.type === 'trailing_entry_triggered' || e.type === 're_entry'
  );
  const exitEvents = events.filter(e => 
    e.type === 'target_hit' || e.type === 'stop_loss' || e.type === 'final_exit'
  );
  ...
  if (events.length >= 2) {
    const firstEntry = events.find(e => 
      e.type === 'entry' || e.type === 'trailing_entry_triggered'
    );
  ...
  const averageHoldingMinutes = holdingCount > 0 
    ? totalHoldingMinutes / holdingCount 
    : undefined;
  ...
  for (const event of events) {
    if (event.type === 'entry' || event.type === 'trailing_entry_triggered' || event.type === 're_entry') {
      lastEntryPrice = event.price;
    } else if (event.type === 'target_hit' || event.type === 'stop_loss' || event.type === 'final_exit') {
      if (lastEntryPrice > 0) {
  ...
  const medianTradeReturn = sortedReturns.length > 0
