    ...
    try {
      ...
      try {
        ...
        for (const parquetFile of manifest.parquetFiles) {
          ...
          // Read parquet file via Python (heavy data lifting)
          const result = await client.query(
            `SELECT * FROM read_parquet('${filePath.replace(/'/g, "''")}')`
          );

          if (result.error) {
            logger.warn('Failed to read parquet file from catalog', {
              path: filePath,
              error: result.error,
            });
            continue;
          ...
          for (const row of rows) {
            ...
            if (typeof row.timestamp === 'bigint') {
              candleTimestamp = Number(row.timestamp);
            } else if (typeof row.timestamp === 'string') {
              candleTimestamp = DateTime.fromISO(row.timestamp).toUnixInteger();
            } else {
              candleTimestamp = row.timestamp;
            }
    ...
    try {
      ...
      for (let i = 0; i < allCandles.length; i += batchSize) {
        ...
        for (const c of batch) {
          ...
          const escapedCallId = c.call_id.replace(/'/g, "''");
          const escapedToken = c.token_address.replace(/'/g, "''");
          const escapedChain = c.chain.replace(/'/g, "''");

          await client.execute(
            `INSERT INTO candles VALUES ('${escapedCallId}', '${escapedToken}', '${escapedChain}', ${c.timestamp}, ${c.open}, ${c.high}, ${c.low}, ${c.close}, ${c.volume})`
          );
        }
      }

      // Export to parquet
      const escapedPath = slicePath.replace(/'/g, "''");
      await client.execute(`COPY candles TO '${escapedPath}' (FORMAT PARQUET)`);
    } finally {
      // Clean up Python connection
      await client.close();
    }
