...
type DuckDbDatabase = InstanceType<DuckDbModule['Database']>;
type DuckDbNativeConnection = ReturnType<DuckDbDatabase['connect']>;
 ...
 */
export async function openCentralDuckDb(): Promise<DuckDbDatabase> {
  const path = getCentralDuckDbPath();
  const duckdb = await getDuckdbModule();
  return new duckdb.Database(path) as DuckDbDatabase;
}

/**
 * Create adapter from DuckDB connection to DuckDbConnection interface
 */
function createAdapter(db: DuckDbNativeConnection): DuckDbConnection {
  return {
 ...
 */
export async function ensureCentralBacktestSchema(db: DuckDbDatabase): Promise<void> {
...
export async function upsertRunMetadata(metadata: BacktestRunMetadata): Promise<void> {
  const db = await openCentralDuckDb();
