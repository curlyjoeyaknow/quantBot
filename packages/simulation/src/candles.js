...
function saveCandlesToCache(candles, filename) {
    try {
        ...
        utils_1.logger.debug('Cached candles', { count: candles.length, filename });
    }
    catch (error) {
        utils_1.logger.error('Failed to save cache', error, { filename });
    }
}
/**
    ...
    try {
        ...
        if (process.env.USE_CACHE_ONLY !== 'true') {
            ...
            if (ageHours > CACHE_EXPIRY_HOURS) {
                utils_1.logger.debug(`Cache expired (${ageHours.toFixed(1)}h old). Removing ${filename}...`);
        ...
        utils_1.logger.debug(`Loaded ${candles.length} candles from cache ${filename}`);
        return candles;
    }
    catch (error) {
        utils_1.logger.error('Failed to load cache', error, { filename });
        return null;
    }
}
/* ============================================================================
...
function mergeCandles(candles5m, candles1m, alertTime, windowMinutes = 30) {
    ...
    utils_1.logger.debug(`Merged candles: ${candles5m.length} 5m + ${candles1m.length} 1m = ${merged.length} total (removed ${candles5m.length - filtered5m.length} overlapping 5m candles)`);
    return merged;
}
/**
...
async function fetchBirdeyeCandles(mint, interval, from, to, chain = 'solana') {
    try {
        ...
        if (estimatedCandles > MAX_CANDLES_PER_REQUEST) {
            utils_1.logger.debug(`Chunking request for ${mint.substring(0, 20)}... (${estimatedCandles} candles estimated, ${Math.ceil(estimatedCandles / MAX_CANDLES_PER_REQUEST)} chunks needed)`);
            const allCandles = [];
...
async function fetchBirdeyeCandlesChunk(mint, interval, from, to, chain, apiKey) {
    try {
        ...
        if (items.length === 0) {
            utils_1.logger.debug(`No candles found for date range, trying limit approach for ${mint.substring(0, 20)}...`);
            ...
            if (limitResponse.status === 200) {
                ...
                if (items.length > 0) {
                    utils_1.logger.debug(`Fetched ${items.length} candles using limit approach for ${mint.substring(0, 20)}...`);
...
async function fetchOptimizedCandlesForAlert(mint, alertTime, endTime = luxon_1.DateTime.utc(), chain = 'solana') {
    ...
    utils_1.logger.debug('Fetching optimized candles for alert', {
    ...
    utils_1.logger.debug('Fetching 1m candles', {
    ...
    if (fifteenSStart >= threeMonthsAgo) {
        utils_1.logger.debug('Fetching 15s candles', {
        ...
        utils_1.logger.debug('Skipping 15s candles (alert >3 months ago)');
    }
    ...
    if (current5mFrom < endUnix) {
        utils_1.logger.debug('Fetching 5m candles', {
    ...
    utils_1.logger.debug('Optimized fetch complete', {
        mint: mint.substring(0, 20),
...
async function fetchHybridCandles(mint, startTime, endTime, chain = 'solana', alertTime) {
    ...
    if (alertTime) {
        ...
        if (ichimokuLookback < startTime) {
            ...
            utils_1.logger.debug(`Extended start time back by 260 minutes for Ichimoku: ${actualStartTime.toISO()} (original: ${startTime.toISO()})`);
        }
    }
    ...
    if (process.env.USE_CLICKHOUSE === 'true' || process.env.CLICKHOUSE_HOST) {
        try {
            ...
            utils_1.logger.warn('ClickHouse query failed, falling back to CSV cache', { error: error.message, mint });
        }
    }
    ...
    if (cachedCandles) {
        utils_1.logger.debug(`Using cached candles for ${mint} (${cachedCandles.length} candles)`);
    ...
    if (cacheFiles.length > 0) {
        ...
        if (cachedData && cachedData.length > 0) {
            ...
            if (endTime.diff(lastCachedTime, 'hours').hours < 1) {
                utils_1.logger.debug(`Extending recent cache from ${lastCachedTime.toFormat('yyyy-MM-dd HH:mm')} to ${endTime.toFormat('yyyy-MM-dd HH:mm')}`);
    ...
    if (process.env.USE_CACHE_ONLY === 'true') {
        utils_1.logger.debug(`⚠️ No cached candles found for ${mint}: ${startTime.toISO()} — ${endTime.toISO()}`);
        utils_1.logger.debug(`   USE_CACHE_ONLY=true, returning empty array (no API calls)`);
        return [];
    }
    ...
    if (alertTime && actualStartTime < startTime) {
        ...
        utils_1.logger.debug(`Returning ${finalCandles.length} candles (${lookbackCandles.length} lookback + ${finalCandles.length - lookbackCandles.length} requested range) for Ichimoku`);
    }
    if (finalCandles.length > 0) {
        ...
        if (actualStartTime < startTime && candles5m.length > candles5mFiltered.length) {
            ...
            utils_1.logger.debug(`Saved lookback candles to cache: ${lookbackCacheFilename} (${candles5m.length} candles)`);
        }
        ...
        if (process.env.USE_CLICKHOUSE === 'true' || process.env.CLICKHOUSE_HOST) {
            try {
                ...
                utils_1.logger.debug(`✅ Saved candles to ClickHouse for ${mint.substring(0, 20)}...`);
            }
            catch (error) {
                utils_1.logger.error('Failed to save to ClickHouse', error, { mint: mint.substring(0, 20) });
