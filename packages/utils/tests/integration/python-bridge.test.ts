...
describe('Python Bridge Test - Telegram Ingestion', () => {
  ...
  const pythonToolPath = join(
    workspaceRoot,
    'packages/ingestion/python/telegram/duckdb_punch_pipeline.py'
  );
  const testFixturePath = join(
    workspaceRoot,
    'packages/ingestion/python/telegram/tests/fixtures/sample_telegram.json'
  );
  const outputDbPath = join(
    workspaceRoot,
    'packages/ingestion/python/telegram/tests/fixtures/test_output.duckdb'
  );
  ...
  it('runs Python tool on tiny fixture and validates output schema', async () => {
    ...
    try {
      ...
      const validated = await engine.runTelegramPipeline(
        ...
        {
          cwd: join(workspaceRoot, 'packages/ingestion/python/telegram'),
          env: { PYTHONPATH: join(workspaceRoot, 'packages/ingestion/python/telegram') },
        }
      );
  ...
  it('validates Python tool handles invalid input gracefully', async () => {
    ...
    const invalidPath = join(
      workspaceRoot,
      'packages/ingestion/python/telegram/tests/fixtures/invalid.json'
    );
