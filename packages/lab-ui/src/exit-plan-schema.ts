...
const LadderLevel = z.union([
  z.object({ kind: z.literal("multiple"), multiple: z.number().positive(), fraction: z.number().min(0).max(1) }),
...
export function validateExitPlanJson(config_json: string): { ok: true } | { ok: false; error: string } {
  try {
