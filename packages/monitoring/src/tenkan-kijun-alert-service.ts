...
import { CallerDatabase } from '@quantbot/data';
...
try {
  ...
  logger.warn('Yellowstone gRPC package not installed. Install with: npm install @triton-one/yellowstone-grpc');
}
...
export class TenkanKijunAlertService extends EventEmitter {
  ...
  constructor(shyftApiKey?: string, shyftWsUrl?: string, shyftXToken?: string, shyftGrpcUrl?: string, rpcUrl?: string) {
    super();
  ...
  private async loadRecentCalls(): Promise<void> {
    try {
      ...
      for (const callerName of callers) {
        try {
          const alerts = await this.callerDb.getCallerAlertsInRange(
            callerName,
            startTime,
            endTime
          );
          ...
          for (const alert of alerts) {
            ...
            logger.info('Monitoring token', { tokenSymbol: monitor.tokenSymbol, callerName: alert.callerName });
            
            // Fetch initial price to start building candles
            logger.debug('Starting monitoring - fetching initial price', { tokenSymbol: monitor.tokenSymbol });
            this.fetchJupiterPrice(monitor).then(() => {
              logger.debug('Initial price fetched, building candles', { tokenSymbol: monitor.tokenSymbol });
            }).catch((error) => {
              logger.warn('Initial price fetch failed, will retry', { tokenSymbol: monitor.tokenSymbol, error: (error as Error).message });
            });
          }
        } catch (error) {
      ...
      this.ws.on('message', (data: WebSocket.RawData) => {
        try {
          ...
          if (this.isAuthenticated) {
            ...
            logger.debug('Received message before authentication, ignoring', { messageId: message.id });
          }
        } catch (error) {
        ...
        if (accountAddress === bondingCurveAddress) {
          ...
          if (accountData && accountData.length > 0) {
            ...
            if (decoded) {
              ...
              if (price > 0) {
                logger.debug('gRPC price update', { tokenSymbol: monitor.tokenSymbol, mint: mintDisplay, price });
                ...
                const solStr = typeof solReserves === 'string' ? solReserves : (solReserves as any)?.toString?.() || String(solReserves || 'N/A');
                const tokenStr = typeof tokenReserves === 'string' ? tokenReserves : (tokenReserves as any)?.toString?.() || String(tokenReserves || 'N/A');
                logger.debug('Bonding curve reserves', { tokenSymbol: monitor.tokenSymbol, solReserve: solStr, tokenReserve: tokenStr });
                ...
                logger.debug('Bonding curve status', { tokenSymbol: monitor.tokenSymbol, complete: decoded.complete, creator: creatorDisplay });
            ...
            logger.info('Token graduated, fetching DEX price', { tokenSymbol: monitor.tokenSymbol });
            this.fetchJupiterPrice(monitor).catch((error) => {
              logger.error('Error fetching Jupiter price', error as Error, { tokenSymbol: monitor.tokenSymbol });
            });
          }
          return;
        }
    ...
    try {
      ...
      if (mint) {
        ...
        if (bondingCurveAddress) {
          logger.info('Pump.fun new token created', { mint: mint.substring(0, 8), bondingCurve: bondingCurveAddress.substring(0, 8) });
        }
      }
      ...
      if (swapEvent && swapEvent.mint) {
        ...
        if (monitor) {
          ...
          if (tokenAmount > 0) {
            ...
            logger.debug('DEX swap price update', { tokenSymbol: monitor.tokenSymbol, price, swapType: swapEvent.type });
            this.updateTokenPrice(monitor, price, Date.now());
          }
        }
      }
  ...
  private updateTokenPrice(monitor: TokenMonitor, price: number, timestamp: number): void {
    ...
    if (newCandleCreated) {
      logger.debug('New 5m candle created', { tokenSymbol: monitor.tokenSymbol, totalCandles: candlesAfter });
    }
    ...
    if (monitor.candles.length >= this.MIN_CANDLES_FOR_ENTRY) {
      ...
      logger.debug('Building candles', { tokenSymbol: monitor.tokenSymbol, current: monitor.candles.length, required: this.MIN_CANDLES_FOR_ENTRY });
    }
  }
  ...
  private checkSignals(monitor: TokenMonitor): void {
    ...
    if (!monitor.inPosition && !monitor.entrySignalSent) {
      ...
      if (crossedUp) {
        ...
        logger.info('BUY SIGNAL', { tokenSymbol: monitor.tokenSymbol, entryPrice: monitor.entryPrice, callerName: monitor.callerName });
      }
    }
    ...
    if (monitor.inPosition && !monitor.exitSignalSent) {
      ...
      if (crossedDown || priceHitStop) {
        ...
        this.emit('alert', alert);
        logger.info('SELL SIGNAL', { tokenSymbol: monitor.tokenSymbol, exitPrice, pnl: ((pnl - 1) * 100).toFixed(2) });
      }
    }
  }
    ...
    this.updateInterval = setInterval(async () => {
      ...
      for (const monitor of this.activeMonitors.values()) {
        try {
          ...
          logger.error('Error fetching price', error as Error, { tokenSymbol: monitor.tokenSymbol });
        }
      }
  ...
  private async fetchJupiterPrice(monitor: TokenMonitor): Promise<void> {
    try {
      ...
      if (priceUsd > 0 && isFinite(priceUsd)) {
        logger.debug('Jupiter price fetched', { tokenSymbol: monitor.tokenSymbol, price: priceUsd });
