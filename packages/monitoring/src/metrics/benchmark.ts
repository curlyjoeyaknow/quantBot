 ...
 */
export function printBenchmarkComparison(current: BenchmarkResult, baseline: BenchmarkResult): void {
  ...
  const driftThroughput = ((current.tokensPerSec - baseline.tokensPerSec) / baseline.tokensPerSec) * 100;
  
  console.log(`Total Time:  ${baseline.totalMs}ms → ${current.totalMs}ms (${driftTotal >= 0 ? '+' : ''}${driftTotal.toFixed(1)}%)`);
  console.log(`Simulation:  ${baseline.avgSimMs}ms → ${current.avgSimMs}ms (${driftSim >= 0 ? '+' : ''}${driftSim.toFixed(1)}%)`);
  console.log(`Throughput:  ${baseline.tokensPerSec.toFixed(1)} → ${current.tokensPerSec.toFixed(1)} tok/s (${driftThroughput >= 0 ? '+' : ''}${driftThroughput.toFixed(1)}%)`);
  console.log('─'.repeat(50));
