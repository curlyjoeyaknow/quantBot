...
export function registerMetadataCommands(program: Command): void {
  ...
  metadataCmd
    ...
    .option('--use-clickhouse', 'Query ClickHouse for EVM tokens', true)
    .option('--use-duckdb', 'Query DuckDB for EVM tokens', true)
    .option('--limit <number>', 'Limit number of tokens to resolve')
    .option('--dry-run', 'Show what would be resolved without updating', false)
    .option('--format <format>', 'Output format', 'table')
    .action(async (options) => {
      const { execute } = await import('../core/execute.js');
      const commandDef = commandRegistry.getCommand('metadata', 'resolve-evm');
      if (!commandDef) {
        throw new NotFoundError('Command', 'metadata.resolve-evm');
      }
