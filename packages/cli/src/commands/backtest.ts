...
export function registerBacktestCommands(program: Command): void {
  const backtestCmd = program
    .command('backtest')
    .description(
      'Trading strategy backtest operations (deterministic replay over historical data)'
    );

  // Run command
  const runCmd = backtestCmd
    .command('run')
    .description('Run backtest on calls')
  ...
  const runsCmd = backtestCmd.command('runs').description('List last 20 backtest runs');
  ...
  const listStrategiesCmd = backtestCmd
  ...
  const storeStrategyCmd = backtestCmd
  ...
  // Run DuckDB command
  const runDuckdbCmd = backtestCmd
    .command('run-duckdb')
    .description('Run backtest using DuckDB Python engine')
    .requiredOption('--duckdb <path>', 'Path to DuckDB file')
  ...
  const generateReportCmd = backtestCmd
  ...
  const clickhouseQueryCmd = backtestCmd
