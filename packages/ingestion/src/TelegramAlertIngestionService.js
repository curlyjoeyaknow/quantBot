...
exports.TelegramAlertIngestionService = void 0;
...
function isBot(sender) {
    ...
    if (/^rick$/i.test(cleanSender))
        return true;
    if (/^phanes$/i.test(cleanSender))
        return true;
    // Also check if it contains bot names (in case of extra characters)
    if (/rick/i.test(cleanSender) && cleanSender.length < 20)
        return true;
    if (/phanes/i.test(cleanSender) && cleanSender.length < 20)
        return true;
    return false;
}
/**
 ...
 */
function detectChain(botText, address) {
 ...
 */
function extractFromBotResponse(botText) {
    const result = {};
    // Extract CA address using new validation module
    const extracted = (0, addressValidation_1.extractAddresses)(botText);
    ...
    const nameMatch2 = botText.match(/(?:🟣|🐶|🟢|🔷|🪙|💊)\s*([A-Z][a-zA-Z0-9\s-.']+?)(?:\s*\(|\s*\[|\s*\$)/);
    ...
    if (candidateName) {
        ...
        const isInvalid = invalidPatterns.some((p) => p.test(candidateName)) ||
    ...
    if (mcapMatch1 || mcapMatch2 || mcapMatch3) {
        ...
        if (!isNaN(value) && value > 0) {
            if (unit === 'K')
                ...
                mcapMultiplier = 1000000;
            else if (unit === 'B')
                mcapMultiplier = 1000000000;
 ...
 */
async function _validateContractAddress(address, chain) {
    try {
        ...
        utils_1.logger.warn('Failed to validate contract address', {
...
class TelegramAlertIngestionService {
    callersRepo;
    tokensRepo;
    alertsRepo;
    callsRepo;
    ...
    async ingestExport(params) {
        utils_1.logger.info('Starting Telegram export ingestion', {
        ...
        const caller = await this.callersRepo.getOrCreateCaller(params.chain.toLowerCase(), // source
        params.callerName, // handle
        params.callerName // displayName
        );
        utils_1.logger.info('Resolved caller', { id: caller.id, name: params.callerName });
        ...
        utils_1.logger.info('Processing messages', { totalMessages: messages.length });
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            try {
                ...
                if (botMessageCount <= 5 || processedBotMessages < 10) {
                    utils_1.logger.debug('Found bot message', {
                        index: i,
                        from: message.from,
                ...
                if (processedBotMessages < 10) {
                    utils_1.logger.debug('Extracted bot data', {
                ...
                if (!botData.caAddress) {
                    if (processedBotMessages < 10) {
                        utils_1.logger.debug('Skipping bot message - no CA address found', {
                            from: message.from,
                ...
                if (validationCount < VALIDATION_SAMPLE_SIZE) {
                    validationCount++;
                    utils_1.logger.info(`Validating contract address with multi-chain fallback (${validationCount}/${VALIDATION_SAMPLE_SIZE})`, {
                        address: botData.caAddress.substring(0, 20),
                        chainHint: detectedChain,
                    });
                    // Use multi-chain metadata fetching as definitive fallback
                    const multiChainResult = await (0, MultiChainMetadataService_1.fetchMultiChainMetadata)(botData.caAddress, detectedChain);
                    if (multiChainResult.primaryMetadata) {
                        ...
                        utils_1.logger.info('Multi-chain validation successful', {
                            address: botData.caAddress.substring(0, 20),
                            chain: detectedChain,
                        ...
                        utils_1.logger.warn('Invalid contract address detected - not found on any chain', {
                            address: botData.caAddress.substring(0, 20),
                            chainHint: detectedChain,
                            addressKind: multiChainResult.addressKind,
                ...
                const token = await this.tokensRepo.getOrCreateToken(detectedChain, (0, core_1.createTokenAddress)(botData.caAddress), {
                ...
                const alertId = await this.alertsRepo.insertAlert({
                    ...
                    callerId: caller.id,
                    side: 'buy',
                    alertTimestamp: luxon_1.DateTime.fromJSDate(callerMessage.timestamp).toJSDate(),
                ...
                await this.callsRepo.insertCall({
                    ...
                    side: 'buy',
                    signalType: 'entry',
                    signalTimestamp: luxon_1.DateTime.fromJSDate(callerMessage.timestamp).toJSDate(),
                    metadata: {
                        messageId: callerMessage.messageId,
                ...
                if (processedBotMessages <= 10) {
                    utils_1.logger.info('✅ Extracted alert from bot response', {
                        caller: callerName,
                    ...
                    utils_1.logger.debug('Extracted alert from bot response', {
                        caller: callerName,
                        token: botData.caAddress.substring(0, 20),
                        ticker: botData.ticker,
                        name: botData.name,
        ...
        utils_1.logger.info('Completed Telegram export ingestion', {
            ...result,
...
exports.TelegramAlertIngestionService = TelegramAlertIngestionService;
//# sourceMappingURL=TelegramAlertIngestionService.js.map