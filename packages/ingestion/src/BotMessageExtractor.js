...
class BotMessageExtractor {
    ...
    extract(textOrHtml) {
        ...
        if (dexscreenerLink) {
            ...
            if (match) {
                ...
                if (result.chain === 'solana') {
                    try {
                        const pubkey = new web3_js_1.PublicKey(extractedAddress);
                        ...
                        if (result.contractAddress !== extractedAddress) {
                            utils_1.logger.warn('Solana address case mismatch', {
                        ...
                        utils_1.logger.warn('Invalid Solana address extracted', {
        ...
        if (!result.contractAddress) {
            if ($) {
                ...
                if (codeBlock) {
                    const extracted = (0, addressValidation_1.extractAddresses)(codeBlock);
            ...
            if (!result.contractAddress) {
                const extracted = (0, addressValidation_1.extractAddresses)(plainText);
        ...
        let vol1hMatch = textOrHtml.match(/📈\s*1H[:\s]*(?:<code>)?[^<]*?(?:<\/code>)?\s*⋅\s*(?:<code>)?\$?([0-9,]+\.?[0-9]*)([KMB]?)(?:<\/code>)?/);
        if (!vol1hMatch) {
        ...
        if ($) {
            ...
            const websiteLink = $('a[href^="http"]')
                .not('a[href*="t.me"], a[href*="x.com"], a[href*="twitter.com"], a[href*="dexscreener"], a[href*="solscan"], a[href*="etherscan"], a[href*="pump.fun"]')
                .first()
                .attr('href');
            if (websiteLink) {
     ...
     */
    parseNumberWithSuffix(valueStr, suffix) {
    ...
    normalizeChain(chain) {
        ...
        if (normalized === 'base')
            return 'base';
        if (normalized === 'bsc' || normalized === 'binance')
            return 'bsc';
     ...
     */
    detectChainFromText(text, address, callerHistory) {
        ...
        if (lowerText.includes('baseswap') ||
...
exports.BotMessageExtractor = BotMessageExtractor;
//# sourceMappingURL=BotMessageExtractor.js.map