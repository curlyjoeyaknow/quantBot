...
import type { Candle, Call, StrategyConfig, SimulationResult, SimulationEvent } from '@quantbot/core';
...
const mockCallsRepoInstance = {
  ...
  findByCaller: vi.fn().mockResolvedValue([]),
};
...
vi.mock('../src/clickhouse/repositories/OhlcvRepository', () => ({
  OhlcvRepository: vi.fn().mockImplementation(() => mockOhlcvRepoInstance),
}));

vi.mock('../src/clickhouse/repositories/IndicatorsRepository', () => ({
  IndicatorsRepository: vi.fn().mockImplementation(() => mockIndicatorsRepoInstance),
}));

vi.mock('../src/clickhouse/repositories/SimulationEventsRepository', () => ({
  SimulationEventsRepository: vi.fn().mockImplementation(() => mockSimulationEventsRepoInstance),
}));

vi.mock('../src/postgres/repositories/CallsRepository', () => ({
  CallsRepository: vi.fn().mockImplementation(() => mockCallsRepoInstance),
}));

vi.mock('../src/postgres/repositories/StrategiesRepository', () => ({
  StrategiesRepository: vi.fn().mockImplementation(() => mockStrategiesRepoInstance),
}));

vi.mock('../src/postgres/repositories/SimulationResultsRepository', () => ({
  SimulationResultsRepository: vi.fn().mockImplementation(() => mockSimulationResultsRepoInstance),
}));

vi.mock('../src/postgres/repositories/TokensRepository', () => ({
  TokensRepository: vi.fn().mockImplementation(() => ({})),
}));

vi.mock('../src/postgres/repositories/AlertsRepository', () => ({
  AlertsRepository: vi.fn().mockImplementation(() => ({})),
}));

vi.mock('../src/postgres/repositories/CallersRepository', () => ({
  CallersRepository: vi.fn().mockImplementation(() => ({})),
}));
...
describe('StorageEngine', () => {
  ...
  describe('OHLCV Candles', () => {
    ...
    it('should handle errors gracefully', async () => {
      ...
      await expect(
        engine.storeCandles(FULL_MINT, 'solana', mockCandles, '5m')
  ...
  describe('Token Calls', () => {
    ...
    it('should store a call', async () => {
      ...
      expect(mockCallsRepo.create).toHaveBeenCalledWith({
        alertId: mockCall.alertId,
        tokenId: mockCall.tokenId,
        callerId: mockCall.callerId,
        strategyId: mockCall.strategyId,
        side: mockCall.side,
        signalType: mockCall.signalType,
        signalStrength: mockCall.signalStrength,
        signalTimestamp: mockCall.signalTimestamp,
        metadata: mockCall.metadata,
      });
    ...
    it('should retrieve calls by token', async () => {
      ...
      mockCallsRepoInstance.findByToken.mockResolvedValue(mockCalls);

      const result = await engine.getCallsByToken(1);

      expect(mockCallsRepoInstance.findByToken).toHaveBeenCalledWith(1, undefined);
    ...
    it('should retrieve calls by caller', async () => {
      ...
      mockCallsRepoInstance.findByCaller.mockResolvedValue(mockCalls);

      const result = await engine.getCallsByCaller(2);

      expect(mockCallsRepoInstance.findByCaller).toHaveBeenCalledWith(2, undefined);
      expect(result).toEqual(mockCalls);
    });
    ...
    it('should cache calls', async () => {
      ...
      expect(mockCallsRepoInstance.findByToken).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('Indicators', () => {
    ...
    it('should retrieve indicators', async () => {
      const startTime = DateTime.fromSeconds(1000);
      const endTime = DateTime.fromSeconds(2000);
      const indicatorsMap = new Map<number, IndicatorValue[]>([
        [1000, mockIndicators],
      ]);
  ...
  describe('Simulation Results', () => {
    ...
    it('should handle empty events array', async () => {
      ...
      expect(mockSimulationEventsRepo.insertEvents).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Error Handling', () => {
    it('should propagate errors from repositories', async () => {
      ...
      await expect(
        engine.storeCandles(FULL_MINT, 'solana', [{ timestamp: 1000, open: 1, high: 1, low: 1, close: 1, volume: 1 }], '5m')
    ...
    it('should handle repository errors gracefully in get operations', async () => {
      ...
      await expect(
        engine.getCandles(FULL_MINT, 'solana', startTime, endTime)
