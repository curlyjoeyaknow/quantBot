...
function getPostgresPool() {
    ...
    pool.on('error', (error) => {
        utils_1.logger.error('Postgres pool error', error, {
            host: config.host,
            database: config.database,
        });
    });
    utils_1.logger.info('Postgres pool created', {
        host: config.host,
        database: config.database,
    });
    return pool;
...
async function withPostgresTransaction(handler) {
    ...
    try {
        ...
        await client.query('ROLLBACK').catch((rollbackError) => {
            utils_1.logger.error('Postgres rollback failed', rollbackError);
        });
        throw error;
    }
    finally {
...
async function closePostgresPool() {
    ...
    await pool.end();
    pool = null;
    utils_1.logger.info('Postgres pool closed');
}
