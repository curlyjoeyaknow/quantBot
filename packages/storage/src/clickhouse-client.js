...
async function initClickHouse() {
    ...
    try {
        ...
        utils_1.logger.info('ClickHouse database and tables initialized');
    }
    catch (error) {
        utils_1.logger.error('Error initializing ClickHouse', error);
...
async function insertCandles(tokenAddress, chain, candles, interval = '5m', isBackfill = false) {
    ...
    try {
        ...
        if (process.env.USE_CACHE_ONLY !== 'true') {
            ...
            utils_1.logger.error('Error inserting candles', error, { tokenAddress: displayAddr });
        }
...
async function insertTicks(tokenAddress, chain, ticks) {
    ...
    try {
        ...
        utils_1.logger.error('Error inserting ticks', error, { tokenAddress: tokenAddress.substring(0, 20) });
        throw error;
    }
}
/**
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            if (process.env.DEBUG_CLICKHOUSE === 'true') {
                utils_1.logger.debug('ClickHouse query', { tokenAddress: tokenAddress.substring(0, 20), chain, startUnix, endUnix, attempt: attempt + 1 });
            }
            ...
            if (process.env.USE_CACHE_ONLY !== 'true' && !isSocketError) {
                utils_1.logger.error('Error querying candles', error, { tokenAddress });
            }
            return [];
        }
    }
...
async function hasCandles(tokenAddress, chain, startTime, endTime) {
    ...
    try {
      ...
      `,
        ...
        utils_1.logger.error('Error checking candles', error, { tokenAddress });
        return false;
    }
}
/**
...
async function closeClickHouse() {
    if (client) {
        await client.close();
        client = null;
        utils_1.logger.info('ClickHouse connection closed');
    }
}
