...
export class CallerDatabase {
  ...
  constructor(
    dbPath: string = process.env.CALLER_DB_PATH ||
      path.join(process.cwd(), 'data', 'databases', 'caller_alerts.db')
  ) {
    this.dbPath = dbPath;
      ...
      for (const alert of alerts) {
        try {
          ...
          if (!errorMessage.includes('UNIQUE constraint failed')) {
            logger.warn('Failed to add alert', {
              callerName: alert.callerName,
              error: errorMessage,
            });
          }
   ...
   */
  async getCallerTokens(
    callerName: string
  ): Promise<
    Array<{ tokenAddress: string; tokenSymbol: string; chain: string; alertCount: number }>
  > {
    const all = promisify(this.db.all.bind(this.db)) as (
      sql: string,
      params?: Array<unknown>
  ...
  async updateCallerSuccessRate(callerName: string, successRate: number): Promise<void> {
    ...
    try {
      ...
      logger.info('Updated caller success rate', {
        callerName,
