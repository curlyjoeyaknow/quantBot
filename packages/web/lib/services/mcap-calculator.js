...
exports.calculateMcapFromPriceChange = calculateMcapFromPriceChange;
exports.calculateMcapMultiple = calculateMcapMultiple;
exports.inferEntryMcap = inferEntryMcap;
exports.formatMcap = formatMcap;
exports.calculateMcap = calculateMcap;
exports.validateMcapCalculations = validateMcapCalculations;
exports.isPumpOrBonkToken = isPumpOrBonkToken;
exports.calculatePumpBonkMcap = calculatePumpBonkMcap;
exports.extractMcapFromMessage = extractMcapFromMessage;
exports.fetchMcapAtTime = fetchMcapAtTime;
exports.getEntryMcapWithFallback = getEntryMcapWithFallback;
/**
 ...
 */
function calculateMcapFromPriceChange(entryMcap, entryPrice, currentPrice) {
    if (entryPrice <= 0)
        return entryMcap;
 ...
 */
function calculateMcapMultiple(entryMcap, peakMcap) {
    if (entryMcap <= 0)
        return 1.0;
 ...
 */
function inferEntryMcap(currentMcap, currentPrice, entryPrice) {
    if (currentPrice <= 0)
        return currentMcap;
 ...
 */
function calculateMcap(price, supply) {
    return price * supply;
}
/**
 * Example usage and validation
 */
function validateMcapCalculations() {
    // Test 1: Basic MCAP calculations
    const entryPrice = 0.001;
    const entryMcap = 100000;
    const peakPrice = 0.010;
    const peakMcap = calculateMcapFromPriceChange(entryMcap, entryPrice, peakPrice);
    console.assert(peakMcap === 1000000, 'Peak MCAP should be $1M');
    const multiple = calculateMcapMultiple(entryMcap, peakMcap);
    console.assert(multiple === 10, 'Multiple should be 10x');
    const inferredEntryMcap = inferEntryMcap(1000000, 0.010, 0.001);
    console.assert(inferredEntryMcap === 100000, 'Inferred entry MCAP should be $100K');
    ...
    console.assert(pumpMcap === 10000, 'Pump token at $0.00001 should have $10K MCAP');
    // Test 4: Message extraction
    const mcapFromMsg1 = extractMcapFromMessage('This is a great token, mcap: $500k');
    console.assert(mcapFromMsg1 === 500000, 'Should extract $500K from message');
    const mcapFromMsg2 = extractMcapFromMessage('Trading at 2.5m mc right now');
    console.assert(mcapFromMsg2 === 2500000, 'Should extract $2.5M from message');
 ...
 */
function calculatePumpBonkMcap(price) {
 ...
 */
async function fetchMcapFromBirdeye(tokenAddress, chain = 'solana') {
    try {
        ...
        const response = await fetch(`https://public-api.birdeye.so/defi/v3/token/meta-data/single?address=${tokenAddress}`, {
            headers: {
                'X-API-KEY': apiKey,
            },
        });
 ...
 */
function extractMcapFromMessage(messageText) {
    if (!messageText)
        return null;
 ...
 */
async function fetchMcapAtTime(tokenAddress, chain, timestamp, entryPrice, messageText) {
 ...
 * 5. Return null
 */
async function getEntryMcapWithFallback(tokenAddress, chain, alertTimestamp, entryPrice, messageText, currentMcap, currentPrice) {
    // Try the full fallback chain (includes pump/bonk detection)
    const fetchedMcap = await fetchMcapAtTime(tokenAddress, chain, alertTimestamp, entryPrice, messageText);
    if (fetchedMcap)
        return fetchedMcap;
...
//# sourceMappingURL=mcap-calculator.js.map