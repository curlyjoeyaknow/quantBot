...
describe('Birdeye API Ingestion Scenarios', () => {
    ...
    describe('API Response Handling', () => {
        it('should handle valid Birdeye API response with items', async () => {
            ...
            vi.mocked(birdeyeClient.fetchOHLCVData).mockResolvedValue(mockResponse);
            const result = await birdeyeClient.fetchOHLCVData(TEST_MINT, TEST_ALERT_TIME.minus({ minutes: 52 }).toJSDate(), TEST_ALERT_TIME.toJSDate(), '1m', TEST_CHAIN);
    ...
    describe('Error Handling', () => {
        it('should handle 400 Bad Request (invalid token)', async () => {
            ...
            await expect(birdeyeClient.fetchOHLCVData(TEST_MINT, TEST_ALERT_TIME.minus({ minutes: 52 }).toJSDate(), TEST_ALERT_TIME.toJSDate(), '1m', TEST_CHAIN)).rejects.toThrow();
        });
        ...
        it('should handle network timeout', async () => {
            ...
            vi.mocked(birdeyeClient.fetchOHLCVData).mockRejectedValue(error);
            await expect(birdeyeClient.fetchOHLCVData(TEST_MINT, TEST_ALERT_TIME.minus({ minutes: 52 }).toJSDate(), TEST_ALERT_TIME.toJSDate(), '1m', TEST_CHAIN)).rejects.toThrow('timeout');
        });
    ...
    describe('Time Range Filtering', () => {
        it('should filter candles to requested time range', () => {
            ...
            const filtered = birdeyeItems.filter((item) => item.unixTime >= startUnix && item.unixTime <= endUnix);
...
//# sourceMappingURL=birdeye-ingestion-simple.test.js.map