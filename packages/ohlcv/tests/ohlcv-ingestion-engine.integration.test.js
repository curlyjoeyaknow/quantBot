...
describe('OhlcvIngestionEngine - Integration Tests', () => {
    ...
    beforeEach(() => {
        ...
        mocks.getCandles.mockImplementation(async (mint, chain, startTime, endTime, options) => {
        ...
        mocks.storeCandles.mockImplementation(async (mint, chain, candles, interval) => {
            const key = `${mint}:${chain}:${interval}`;
        ...
        mocks.getOrCreateToken.mockImplementation(async (chain, address, metadata) => {
    ...
    describe('End-to-End Data Flow', () => {
        it('should fetch metadata, store it, then fetch and store candles', async () => {
            ...
            expect(getMocks().getOrCreateToken).toHaveBeenCalledWith(TEST_CHAIN, TEST_MINT, expect.objectContaining({ name: mockMetadata.name, symbol: mockMetadata.symbol }));
    ...
    describe('Storage Engine Integration', () => {
        it('should preserve full mint address in all storage operations', async () => {
            ...
            expect(getMocks().storeCandles).toHaveBeenCalledWith(fullMint, // Full address preserved
            TEST_CHAIN, expect.any(Array), expect.any(String));
            // Verify metadata storage
            expect(getMocks().getOrCreateToken).toHaveBeenCalledWith(TEST_CHAIN, fullMint, // Full address preserved
            expect.any(Object));
        });
...
//# sourceMappingURL=ohlcv-ingestion-engine.integration.test.js.map