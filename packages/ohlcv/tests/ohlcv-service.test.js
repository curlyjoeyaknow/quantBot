...
vi.mock('@quantbot/storage', async () => {
    ...
    globalThis.__ohlcvServiceStorageMocks__ = { mockStorage, mockStorageEngine };
    return mockStorage;
});
...
describe('OHLCVService', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        const storageMocks = globalThis.__ohlcvServiceStorageMocks__;
    ...
    describe('ingestCandles', () => {
        ...
        it('should ingest candles to ClickHouse', async () => {
            ...
            expect(mockStorageEngine.storeCandles).toHaveBeenCalledWith(FULL_MINT, // Full address, case-preserved
            'solana', mockCandles, '5m');
        ...
        it('should preserve exact case of mint address', async () => {
            ...
            expect(mockStorageEngine.storeCandles).toHaveBeenCalledWith(FULL_MINT_LOWERCASE, // Exact case preserved
            'solana', mockCandles, '5m');
        });
        ...
        it('should handle ingestion errors', async () => {
            ...
            await expect(service.ingestCandles(FULL_MINT, 'solana', mockCandles)).rejects.toThrow('Database error');
        });
    });
    describe('getCandles', () => {
        const mockCandles = [
        ...
        it('should use ClickHouse cache when available', async () => {
            mockStorageEngine.getCandles.mockResolvedValue(mockCandles);
            const result = await service.getCandles(FULL_MINT, 'solana', startTime, endTime);
            expect(mockStorageEngine.getCandles).toHaveBeenCalledWith(FULL_MINT, 'solana', startTime, endTime, { interval: '5m' });
            expect(result).toEqual(mockCandles);
        });
...
//# sourceMappingURL=ohlcv-service.test.js.map