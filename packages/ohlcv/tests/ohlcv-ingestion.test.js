...
vi.mock('@quantbot/storage', async () => {
    ...
    globalThis.__ohlcvIngestionStorageMocks__ = {
        mockInfluxClient,
        mockCache,
    };
    return {
...
vi.mock('@quantbot/api-clients', async () => {
    ...
    globalThis.__ohlcvIngestionApiMocks__ = { mockBirdeyeClient };
    return {
...
describe('OHLCVIngestionService', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        const storageMocks = globalThis.__ohlcvIngestionStorageMocks__;
        mockInfluxClient = storageMocks.mockInfluxClient;
        mockCache = storageMocks.mockCache;
        const apiMocks = globalThis.__ohlcvIngestionApiMocks__;
    ...
    describe('fetchAndStoreOHLCV', () => {
        ...
        it('should use cached data when available', async () => {
            ...
            const result = await service.fetchAndStoreOHLCV(FULL_MINT, startTime, endTime, 'TEST', 'solana');
            expect(mockCache.get).toHaveBeenCalledWith(FULL_MINT, startTime, endTime, '1m');
            expect(mockInfluxClient.writeOHLCVData).toHaveBeenCalledWith(FULL_MINT, // Full address, case-preserved
            'TEST', 'solana', cachedData);
        ...
        it('should preserve exact case of mint address', async () => {
            ...
            expect(mockBirdeyeClient.fetchOHLCVData).toHaveBeenCalledWith(FULL_MINT_LOWERCASE, // Exact case preserved
            startTime, endTime);
            expect(mockInfluxClient.writeOHLCVData).toHaveBeenCalledWith(FULL_MINT_LOWERCASE, 'UNKNOWN', 'solana', expect.any(Array));
        });
        ...
        it('should cache data with 2 hour TTL', async () => {
            ...
            expect(mockCache.set).toHaveBeenCalledWith(FULL_MINT, startTime, endTime, expect.any(Array), '1m', 120 // 2 hours TTL
            );
        });
    });
});
//# sourceMappingURL=ohlcv-ingestion.test.js.map