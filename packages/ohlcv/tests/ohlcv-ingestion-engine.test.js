...
import { initClickHouse, TokensRepository } from '@quantbot/storage';
...
describe('OhlcvIngestionEngine', () => {
    ...
    describe('fetchCandles - Metadata', () => {
        it('should fetch and store metadata before candles', async () => {
            ...
            expect(mockGetOrCreateToken).toHaveBeenCalledWith(TEST_CHAIN, TEST_MINT, expect.objectContaining({ name: mockMetadata.name, symbol: mockMetadata.symbol }));
        });
    ...
    describe('fetchCandles - 1m Candles', () => {
        it('should fetch 1m candles from -52 minutes before alert', async () => {
            ...
            if (fetchCall) {
                const startTime = DateTime.fromJSDate(fetchCall[1]);
        ...
        it('should store 1m candles immediately after fetching', async () => {
            ...
            expect(mockStorageEngine.storeCandles).toHaveBeenCalledWith(TEST_MINT, TEST_CHAIN, expect.arrayContaining([expect.objectContaining({ timestamp: mockCandles[0].timestamp })]), '1m');
        });
    });
    describe('fetchCandles - 5m Candles', () => {
        ...
        it('should store each 5m chunk immediately', async () => {
            ...
            await engine.fetchCandles(TEST_MINT, TEST_CHAIN, TEST_ALERT_TIME);
            expect(mockStorageEngine.storeCandles).toHaveBeenCalledWith(TEST_MINT, TEST_CHAIN, expect.arrayContaining([expect.objectContaining({ timestamp: mockCandles[0].timestamp })]), '5m');
        });
    });
    describe('Caching', () => {
    ...
    describe('Mint Address Preservation', () => {
        it('should preserve full mint address in all operations', async () => {
            ...
            expect(birdeyeClient.fetchOHLCVData).toHaveBeenCalledWith(fullMint, expect.any(Date), expect.any(Date), expect.any(String), TEST_CHAIN);
            expect(mockStorageEngine.storeCandles).toHaveBeenCalledWith(fullMint, TEST_CHAIN, expect.any(Array), expect.any(String));
        });
    });
...
//# sourceMappingURL=ohlcv-ingestion-engine.test.js.map