...
describe('exportAndAnalyzeSlice', () => {
  it("orchestrates export then analyze (pure workflow)", async () => {
    ...
    const analyzer: SliceAnalyzer = {
      analyze: async ({ manifest }) => ({
        status: "ok",
        summary: { exportedFiles: manifest.parquetFiles.length, exportedRows: manifest.summary.totalRows ?? null },
      }),
    };
  ...
  it("enforces maxFiles limit", async () => {
    const exporter: SliceExporter = {
      exportSlice: async () =>
        makeStubManifest({
          parquetFiles: Array.from({ length: 3 }, (_, i) => ({ path: `file:///tmp/part-${i}.parquet` })),
