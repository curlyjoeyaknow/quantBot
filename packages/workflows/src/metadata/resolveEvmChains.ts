 ...
 */
async function updateChainInDuckDB(duckdbPath: string, address: string, newChain: string): Promise<void> {
  const { PythonEngine } = await import('@quantbot/utils');
  const engine = new PythonEngine();
...
export async function resolveEvmChains(
  ...
  const uniqueTokens = Array.from(
    new Map(evmTokens.map(t => [t.address, t])).values()
  );
  
  ctx.logger.info(`Total unique EVM tokens to resolve: ${uniqueTokens.length}`);
  
  // Apply limit if specified
  const tokensToResolve = validated.limit 
    ? uniqueTokens.slice(0, validated.limit)
    : uniqueTokens;
  ...
  for (const [index, token] of tokensToResolve.entries()) {
    ctx.logger.info(`[${index + 1}/${tokensToResolve.length}] Resolving ${token.address.substring(0, 20)}...`);
    
    try {
      const { chain, metadata } = await resolveTokenChain(token.address, ctx.birdeyeClient, ctx.logger);
