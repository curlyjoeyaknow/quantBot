...
// Configure ClickHouse connection before any imports that use it
// Set environment variables if not already set (allows override via .env or command line)
// These must be set BEFORE any module imports that use ClickHouse client
if (!process.env.CLICKHOUSE_HOST) {
  process.env.CLICKHOUSE_HOST = '127.0.0.1';
}
if (!process.env.CLICKHOUSE_HTTP_PORT && !process.env.CLICKHOUSE_PORT) {
...
describe('Slice Export & Analyze E2E Tests', () => {
  ...
  beforeAll(async () => {
    // Check if ClickHouse is available using current environment configuration
    // Don't override environment variables - use what's already set
    try {
      const { getClickHouseClient } = await import('@quantbot/storage');
      const ch = getClickHouseClient();
      // Use a simple query with timeout to test connectivity
      const result = await Promise.race([
        ch.query({ query: 'SELECT 1 as test', format: 'JSONEachRow' }),
      ...
      clickHouseAvailable = false;
      // Only log if we're in verbose mode to avoid cluttering test output
      if (process.env.VERBOSE_TESTS) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.warn('ClickHouse not available, skipping E2E tests:', errorMessage);
      }
    }
  ...
  it.skipIf(!clickHouseAvailable)(
    'E2E: should complete full pipeline: export → manifest → analyze → validate',
    async () => {
    ...
    120000
  ); // 2 minute timeout for full E2E test

  it.skipIf(!clickHouseAvailable)(
    'E2E: should handle complex SQL analysis queries',
    async () => {
      const exporter = createClickHouseSliceExporterAdapterImpl();
      const analyzer = createDuckDbSliceAnalyzerAdapterImpl();
  ...
  it.skipIf(!clickHouseAvailable)(
    'E2E: should handle empty result set with analysis',
    async () => {
      const exporter = createClickHouseSliceExporterAdapterImpl();
      const analyzer = createDuckDbSliceAnalyzerAdapterImpl();
  ...
  it.skipIf(!clickHouseAvailable)(
    'E2E: should enforce limits correctly',
    async () => {
      const exporter = createClickHouseSliceExporterAdapterImpl();
      const analyzer = createDuckDbSliceAnalyzerAdapterImpl();
  ...
  it.skipIf(!clickHouseAvailable)(
    'E2E: should handle empty result sets with improved diagnostics',
    async () => {
      const exporter = createClickHouseSliceExporterAdapterImpl();
      const analyzer = createDuckDbSliceAnalyzerAdapterImpl();
  ...
  it.skipIf(!clickHouseAvailable)(
    'E2E: should handle DuckDB analysis errors gracefully',
    async () => {
      const exporter = createClickHouseSliceExporterAdapterImpl();
      const analyzer = createDuckDbSliceAnalyzerAdapterImpl();
  ...
  it.skipIf(!clickHouseAvailable)(
    'E2E: should handle missing Parquet files gracefully',
    async () => {
      const exporter = createClickHouseSliceExporterAdapterImpl();
      const analyzer = createDuckDbSliceAnalyzerAdapterImpl();
