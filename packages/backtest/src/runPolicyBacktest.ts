...
import type { CallRecord, Interval, PolicyResultRow } from './types.js';
...
export async function runPolicyBacktest(
  ...
  const medianReturnBps =
    returnsBps.length > 0 ? returnsBps.sort((a, b) => a - b)[Math.floor(returnsBps.length / 2)] : 0;

  const stopOutRate = policyResults.length > 0 ? stopOutCount / policyResults.length : 0;

  const avgTimeExposedMs = policyResults.length > 0 ? totalTimeExposedMs / policyResults.length : 0;

  const avgTailCapture =
    tailCaptures.length > 0 ? tailCaptures.reduce((a, b) => a + b, 0) / tailCaptures.length : null;
