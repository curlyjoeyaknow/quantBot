...
export interface SimulationEngineDeps {
  ...
  defaults?: PartialDeep<ScenarioDefaults>;
}
...
export interface ScenarioRunRequest {
  ...
  candlesMap: Map<string, Candle[]> | Record<string, Candle[]>;
  runOptions?: Partial<RunOptions>;
  overrides?: PartialDeep<ScenarioDefaults>;
}
...
export class SimulationEngine {
  ...
  private mergeScenarioConfigs(
    ...
    const baseStopLoss = this.defaults.stopLoss ?? DEFAULT_STOP_LOSS;
    const baseEntry = this.defaults.entry ?? DEFAULT_ENTRY;
    const baseReEntry = this.defaults.reEntry ?? DEFAULT_REENTRY;
    const baseCosts = this.defaults.costs ?? DEFAULT_COSTS;
