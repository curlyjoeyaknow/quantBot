...
def check_coverage_at_horizon(
    ...
    query = f"""
        ...
        WHERE (token_address = '{escaped_mint}'
               ...
               OR token_address LIKE '{escaped_mint}%'
               OR lower(token_address) LIKE lower('{escaped_mint}%'))
    ...
    try:
        ...
        # Silently fail - return False if query fails
        return False
...
def main():
    ...
    matrix, total_alerts = calculate_coverage_matrix(conn, ch_client, database, args.interval)
