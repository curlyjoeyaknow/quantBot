...
Migrated from tools/telegram/simulation/contracts.py to tools/backtest/lib/simulation/contracts.py
as part of simulation migration to backtest package.
"""
...
class SimInput:
    ...
    contractVersion: str = CURRENT_CONTRACT_VERSION
    seed: Optional[int] = None
    dataVersion: Optional[str] = None
    strategyVersion: Optional[str] = None
    
    # Execution model (no perfect fills)
    executionModel: Optional[Dict[str, Any]] = None
    
    # Risk model
    riskModel: Optional[Dict[str, Any]] = None
    
    # Data snapshot hash (for reproducibility)
    dataSnapshotHash: Optional[str] = None
    
    # Clock resolution (milliseconds, seconds, minutes, hours)
    clockResolution: str = 'm'  # Default to minutes
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            'clockResolution': self.clockResolution,
        }
        ...
        if self.dataVersion:
            result['dataVersion'] = self.dataVersion
        if self.strategyVersion:
            result['strategyVersion'] = self.strategyVersion
        if self.executionModel:
            result['executionModel'] = self.executionModel
        if self.riskModel:
            result['riskModel'] = self.riskModel
        if self.dataSnapshotHash:
            result['dataSnapshotHash'] = self.dataSnapshotHash
        return result
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SimInput':
        ...
        return cls(
            ...
            contractVersion=data.get('contractVersion', CURRENT_CONTRACT_VERSION),
            seed=data.get('seed'),
            dataVersion=data.get('dataVersion'),
            strategyVersion=data.get('strategyVersion'),
            executionModel=data.get('executionModel'),
            riskModel=data.get('riskModel'),
            dataSnapshotHash=data.get('dataSnapshotHash'),
            clockResolution=data.get('clockResolution', 'm'),
        )
